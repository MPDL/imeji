...
public class RegistrationBusinessControllerTest extends SuperServiceTest {
	...
	public void registerAfterInvitation() throws Exception {
		...
		invitationBusinessController
				.invite(new Invitation(user.getEmail(), collectionBasic.getId().toString(), ShareRoles.READ.name()));
		// Register
		Registration registration = registrationBC.register(user);
	...
	/**
	 * Register with an email which have an expired registration (still not removed
	 * by clear job) <br/>
