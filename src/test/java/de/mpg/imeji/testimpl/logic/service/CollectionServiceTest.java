...
public class CollectionServiceTest extends SuperServiceTest {
	...
	public static void specificSetup() {
		...
		try {
			sysadmin = ImejiFactory.newUser().setEmail("admin3@test.org").setPerson("admin3", "admin3", "org")
					.setPassword("password").setQuota(Long.MAX_VALUE).build();
			defaultUser = ImejiFactory.newUser().setEmail("default@test.org").setPerson("default", "default", "org")
					.setPassword("password").setQuota(Long.MAX_VALUE).build();
			restrictedUser = ImejiFactory.newUser().setEmail("restricted@test.org")
					.setPerson("restricted", "restricted", "org").setPassword("password").setQuota(Long.MAX_VALUE)
					.build();
			userReadGrant = ImejiFactory.newUser().setEmail("read2@test.org").setPerson("read", "read", "org")
					.setPassword("password").setQuota(Long.MAX_VALUE).build();
			userEditGrant = ImejiFactory.newUser().setEmail("edit2@test.org").setPerson("edit", "edit", "org")
					.setPassword("password").setQuota(Long.MAX_VALUE).build();
	...
	private void retrieve_Test(String msg, URI uri, User user, String expectedIDString, String expectedName,
			Class exception) {
		CollectionService service = new CollectionService();
	...
	public void update() {
		...
		update_Test("private collection, user read grant", collectionPrivate, userReadGrant, "Private Collection",
				NotAllowedError.class);
		update_Test("private collection, user edit grant", collectionPrivate, userEditGrant, "Private Collection",
				null);
	}
	...
	public void updateLogo() {
		try {
			updateLogo_Test("collectionPrivate, edit grant user", collectionPrivate, ImejiTestResources.getTest1Jpg(),
					userEditGrant, null);

			updateLogo_Test("collectionPrivate, read grant user", collectionPrivate, ImejiTestResources.getTest2Jpg(),
	...
	public void delete() {
		...
		CollectionImeji collectionToDelete = ImejiFactory.newCollection().setTitle("Delete").setPerson("p", "p", "o")
				.build();
		try {
			...
			Locks.lock(lock);
			delete_Test("private Collection, collection admin user,itemLocked", collectionToDelete, defaultUser,
	...
	public void release() {
		CollectionService service = new CollectionService();
		CollectionImeji collectionToRelease = ImejiFactory.newCollection().setTitle("To Release")
				.setPerson("m", "p", "g").build();
		try {
			...
			release_Test("Unauthorized user", collectionToRelease, itemToRelease, userEditGrant, getDefaultLicense(),
					getDefaultLicense(), NotAllowedError.class);
			collectionToRelease = service.retrieve(collectionToRelease.getId(), sysadmin);

			Lock lock = new Lock(itemToRelease.getId().toString(), null);
			Locks.lock(lock);
			release_Test("Locked item", collectionToRelease, itemToRelease, defaultUser, getDefaultLicense(),
					getDefaultLicense(), UnprocessableError.class);
			Locks.unLock(lock);
			collectionToRelease = service.retrieve(collectionToRelease.getId(), sysadmin);

			release_Test("pending collection, collection admin", collectionToRelease, itemToRelease, defaultUser,
	...
	private void release_Test(String msg, CollectionImeji col, Item item, User user, License defaultLicense,
	...
	public void withdraw() {
		CollectionService service = new CollectionService();
		CollectionImeji collectionToWithdraw = ImejiFactory.newCollection().setTitle("To Withdraw")
				.setPerson("m", "p", "g").build();
	...
	private void withdraw_Test(String msg, CollectionImeji collection, User user, Class exception) {
		...
		try {
			...
			if (exception == null) {
				...
				Assert.assertEquals(msg + ": Collection Status should still be unchanged", collectionPreviousStatus,
						retrievedCol.getStatus());
			}
		} catch (ImejiException e1) {
