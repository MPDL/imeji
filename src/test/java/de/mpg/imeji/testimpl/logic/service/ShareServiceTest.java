...
public class ShareServiceTest extends SuperServiceTest {
  ...
  public void shareAndUnshareSysadmin() {
    ...
    try {
      ...
      Assert.assertFalse("User 1 should not have sysadmin grant", defaultUser1.getGrants().contains(sysadminGrant.toGrantString()));
      ...
      defaultUser1 = (new UserService()).retrieve(defaultUser1.getId(), sysadmin);
      Assert.assertTrue("User 1 should have sysadmin grant", defaultUser1.getGrants().contains(sysadminGrant.toGrantString()));
      ...
      defaultUser1.setGrants(AuthorizationPredefinedRoles.defaultUser(defaultUser1.getId().toString()));
      try {
  ...
  private void shareToUser_Test(String msg, User fromUser, User toUser, Grant grant, Class exception) {
    ShareService service = new ShareService();
    try {
    ...
    try {
      ...
      if (exception == null) {
        Assert.assertTrue(msg + ": User should have grant", retUser.getGrants().contains(grant.toGrantString()));
      } else {
        Assert.assertFalse(msg + ": User should not have grant", retUser.getGrants().contains(grant.toGrantString()));
      }
    } catch (ImejiException e) {
      Assert.fail(e.getMessage());
    }
  ...
  public void shareToUserGroup() throws ImejiException {
    ...
    try {
      defaultUser2.getGrants().add(editGrant.toGrantString());
      shareToUserGroup_Test("User 2 not allowed, read grant", defaultUser2, group, readGrant, NotAllowedError.class);
      shareToUserGroup_Test("User 2 not allowed, edit grant", defaultUser2, group, editGrant, NotAllowedError.class);
  ...
  private void shareToUserGroup_Test(String msg, User fromUser, UserGroup group, Grant grant, Class exception) {
    ShareService service = new ShareService();
    try {
    ...
    try {
      ...
      if (exception == null) {
        Assert.assertTrue(msg + ": User should have grant", retGroup.getGrants().contains(grant.toGrantString()));
      } else {
        Assert.assertFalse(msg + ": User should not have grant", retGroup.getGrants().contains(grant.toGrantString()));
      }
    } catch (ImejiException e) {
      Assert.fail(e.getMessage());
    }
