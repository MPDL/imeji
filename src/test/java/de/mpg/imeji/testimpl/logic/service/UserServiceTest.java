...
public class UserServiceTest extends SuperServiceTest {
	...
	public static void specificSetup() {
		...
		try {
			...
			adminUser = ImejiFactory.newUser().setEmail("admin3@test.org").setPerson("admin3", "admin3", "org")
	...
	public void create() {
		...
		Assert.assertEquals("Admin grants should be as expected",
				AuthorizationPredefinedRoles.imejiAdministrator(adminUser.getId().toString()), adminUser.getGrants());
		Assert.assertEquals("Default grants should be as expected",
				AuthorizationPredefinedRoles.defaultUser(defaultUser.getId().toString()), defaultUser.getGrants());
	...
	public void retrive() {
		...
		try {
			...
			User defaultByBatch = (User) service.retrieveBatch(Arrays.asList(defaultUser.getId().toString()), 10)
					.toArray()[0];
			User defaultByBatchLazy = (User) service
					.retrieveBatchLazy(Arrays.asList(defaultUser.getId().toString()), 10).toArray()[0];
			Assert.assertEquals("ID should be as expected for email retrive", defaultUser.getId().toString(),
					defaultByEmail.getId().toString());
			Assert.assertEquals("ID should be as expected for ID retrive", defaultUser.getId().toString(),
					defaultById.getId().toString());
			Assert.assertEquals("email should be as expected for email retrive", defaultUser.getEmail(),
					defaultByEmail.getEmail());
			Assert.assertEquals("email should be as expected for ID retrive", defaultUser.getEmail(),
					defaultById.getEmail());
			Assert.assertEquals("ID should be as expected for batch retrive", defaultUser.getId(),
					defaultByBatch.getId());
			Assert.assertEquals("ID should be as expected for batch retrive lazy", defaultUser.getId(),
					defaultByBatchLazy.getId());
	...
	public void retrieveAll() {
		...
		try {
			...
			Assert.assertTrue("All users should have been retrived",
					containsAdmin && containsDefault && containsRestricted);
		} catch (ImejiException e) {
			Assert.fail(e.getMessage());
		}
	}
	...
	public void getCompleteName() {
		...
		try {
			service.update(defaultUser, adminUser);
			Assert.assertEquals("User name", "Planck, Max",
					service.getCompleteName(defaultUser.getId(), Locale.ENGLISH));
		} catch (ImejiException e) {
		}
	}
	...
	public void retrieveAllAdmins() {
		UserService service = new UserService();
		try {
			User adminUser2 = ImejiFactory.newUser().setEmail("admin4@test.org").setPerson("admin4", "admin4", "org")
					.setPassword("password").setQuota(Long.MAX_VALUE).build();
