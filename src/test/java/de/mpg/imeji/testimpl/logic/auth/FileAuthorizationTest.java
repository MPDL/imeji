...
public class FileAuthorizationTest extends SuperServiceTest {
  ...
  public void loggedInReadPrivateItemOfOwnCollection() throws ImejiException {
    createCollection();
    createItemWithFile();
    Assert.assertTrue(StorageUtil.isAllowedToViewFile(getContent(item).getFull(), JenaUtil.testUser));
    Assert.assertTrue(StorageUtil.isAllowedToViewFile(getContent(item).getThumbnail(), JenaUtil.testUser));
    Assert.assertTrue(StorageUtil.isAllowedToViewFile(getContent(item).getPreview(), JenaUtil.testUser));
  }
  ...
  public void loggedInReadPrivateItemOfForbiddenCollection() throws ImejiException {
    createCollection();
    createItemWithFile();
    Assert.assertFalse(StorageUtil.isAllowedToViewFile(getContent(item).getFull(), JenaUtil.testUser2));
    Assert.assertFalse(StorageUtil.isAllowedToViewFile(getContent(item).getThumbnail(), JenaUtil.testUser2));
    Assert.assertFalse(StorageUtil.isAllowedToViewFile(getContent(item).getPreview(), JenaUtil.testUser2));
  }
  ...
  @Test
  public void loggedInReadPublicItemOfReleasedCollectionInPrivateMode() throws ImejiException, IOException, URISyntaxException {
    createCollection();
    createItemWithFile();
    releaseCollection();
    enablePrivateMode();
