...
public class HttpAuthenticationTest {
  ...
  public void loginWithEmailAndPassword() {
    try {
      User user =
          AuthenticationFactory.factory(generateBasicAuthenticationHeader(JenaUtil.TEST_USER_EMAIL, JenaUtil.TEST_USER_PWD)).doLogin();
  ...
  public void loginWithWrongPassword() {
    try {
      AuthenticationFactory.factory(generateBasicAuthenticationHeader(JenaUtil.TEST_USER_EMAIL, JenaUtil.TEST_USER_PWD + "abc")).doLogin();
      Assert.fail();
    } catch (AuthenticationError e) {
      // Correct
    }
  ...
  public void loginWithAPIKey() throws ImejiException, JoseException {
    ...
    try {
      User user = AuthenticationFactory.factory(generateAPIKEYAuthenticationHeader(usertest.getApiKey())).doLogin();
      Assert.assertNotNull(user);
    } catch (AuthenticationError e) {
      Assert.fail(e.getMessage());
    }
  ...
  public void loginUserWithoutKey() throws ImejiException {
    ...
    controller.update(usertest, usertest);
    AdminProcess.login(generateBasicAuthenticationHeader(JenaUtil.TEST_USER_EMAIL, JenaUtil.TEST_USER_PWD));
    String key = controller.retrieve(JenaUtil.TEST_USER_EMAIL, Imeji.adminUser).getApiKey();
  ...
  public void logout() throws ImejiException {
    ...
    Assert.assertFalse(key.equals(controller.retrieve(JenaUtil.TEST_USER_EMAIL, Imeji.adminUser).getApiKey()));
    try {
  ...
  private String generateBasicAuthenticationHeader(String email, String password) {
    return "Basic " + new String(Base64.encodeBase64(new String(email + ":" + password).getBytes()));
  }
