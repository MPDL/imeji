...
/**
 * Tests ContentService. Currently not tested: create ContentVO for external
 * file, update (b/c it already gets tested in ItemServiceTest.updateFile)
 * 
 * @author jandura
 *
 */
public class ContentServiceTest extends SuperServiceTest {
	...
	public void create() {
		...
		// ItemService.create
		Assert.assertEquals("Item id should match id of item stored in content", item.getId().toString(),
				content.getItemId());
		try {
	...
	public void retrieveBatch() {
		try {
			ContentVO contentRetrive = (new ContentService()).retrieveBatch(Arrays.asList(content.getId().toString()))
					.get(0);
			Assert.assertEquals("ID should be as exoected", content.getId().toString(),
					contentRetrive.getId().toString());
			Assert.assertEquals("URL of original should be as expected", content.getOriginal(),
