...
public class UsergroupServiceTest extends SuperServiceTest {
  ...
  public static void specificSetup() {
    ...
    try {
      ...
      userGroup = ImejiFactory.newUserGroup().setName("UserGroup")
          .addUsers(defaultUser1, defaultUser2).addGrants(FOO_GRANT).build();
  ...
  public void create() {
    ...
    UserGroup userGroup2 = ImejiFactory.newUserGroup().setName("UserGroup2")
        .addUsers(defaultUser1, defaultUser2).build();
    try {
  ...
  public void retrive() {
    ...
    try {
      ...
      UserGroup ret3 =
          service.retrieveBatch(Arrays.asList(userGroup.getId().toString()), restrictedUser).get(0);
      Assert.assertEquals("ID", userGroup.getId(), ret3.getId());
      Assert.assertEquals("Name", userGroup.getName(), ret3.getName());
      UserGroup ret4 = service
          .retrieveBatchLazy(Arrays.asList(userGroup.getId().toString()), restrictedUser).get(0);
  ...
  public void update() {
    try {
      User defaultUser3 = ImejiFactory.newUser().setEmail("default3@test.org")
  ...
  public void delete() {
    ...
    try {
      ...
      Assert.assertFalse("defaultUser1 should have lost his FOO grant",
          defaultUser1.getGrants().contains(FOO_GRANT));
      // Recreate the usergroup
      service.create(userGroup, adminUser);

      /*
       * try { service.delete(userGroup, defaultUser1); Assert.fail("No exception has been thown");
       * } catch (NotAllowedError e) { // That is correct }
       */
  ...
  public void retrieveAll_searchByUser_serachByName() {
    ...
    UserGroup userGroup2 =
        ImejiFactory.newUserGroup().setName("SearchName").addUsers(defaultUser1).build();
    ...
    try {
      ...
      Assert.assertTrue("All users",
          doesCollectionConsistOfUserGroups(all, userGroup, userGroup2, userGroup3));
      Assert.assertTrue("By User",
          doesCollectionConsistOfUserGroups(byUser, userGroup, userGroup2));
  ...
  public void removeUserFromAllGroups() {
    ...
    try {
       ...
       * Assert.fail("No exception was thrown"); } catch (NotAllowedError e) { // correct }
       * 
      ...
      Assert.assertEquals("User2 should be in no user group", 0,
          service.searchByUser(defaultUser2, adminUser).size());
      Assert.assertFalse("User2 should have lost his foo grant",
          defaultUser2.getGrants().contains(FOO_GRANT));
    } catch (ImejiException e) {
      Assert.fail(e.getMessage());
    }
  ...
  private boolean doesCollectionConsistOfUserGroups(Collection<UserGroup> coll,
      UserGroup... groups) {
