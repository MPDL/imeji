...
public class UsergroupServiceTest extends SuperServiceTest {
	...
	public static void specificSetup() {
		...
		try {
			...
			restrictedUser = ImejiFactory.newUser().setEmail("restricted@test.org")
					...
					.build();
			defaultUser1 = ImejiFactory.newUser().setEmail("default1@test.org").setPerson("default1", "default1", "org")
					.setPassword("password").setQuota(Long.MAX_VALUE).build();
			defaultUser2 = ImejiFactory.newUser().setEmail("default2@test.org").setPerson("default2", "default2", "org")
					.setPassword("password").setQuota(Long.MAX_VALUE).build();
			...
			userGroup = ImejiFactory.newUserGroup().setName("UserGroup").addUsers(defaultUser1, defaultUser2)
					.addGrants(FOO_GRANT).build();
	...
	public void create() {
		...
		UserGroup userGroup2 = ImejiFactory.newUserGroup().setName("UserGroup2").addUsers(defaultUser1, defaultUser2)
				.build();
		try {
	...
	public void retrive() {
		...
		try {
			...
			UserGroup ret4 = service.retrieveBatchLazy(Arrays.asList(userGroup.getId().toString()), restrictedUser)
					.get(0);
	...
	public void update() {
		try {
			User defaultUser3 = ImejiFactory.newUser().setEmail("default3@test.org")
					.setPerson("default3", "default3", "org").setPassword("password").setQuota(Long.MAX_VALUE).build();
