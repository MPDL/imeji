...
public class ItemIntegration extends ItemTestBase {
	...
	public void createItemWithFile_NullAsExtensionInFileUrl_Bug1023() throws IOException {
		...
		multiPart.field("json",
				itemJSON.replace("___COLLECTION_ID___", collectionId)
						.replaceAll("\\s*\"filename\":\\s*\"___FILENAME___\"\\s*,", "").replace("___FILENAME___", "")
						.replace("___REFERENCE_URL___", "").replace("___FETCH_URL___", ""));
	...
	public void createItem_NotLoggedIn() throws IOException {
		...
		Response response2 = target(PATH_PREFIX).register(authAsUserFalse).register(MultiPartFeature.class)
	...
	public void createItem_WithNotAllowedUser() throws Exception {
		...
		Response response = target(PATH_PREFIX).register(authAsUser2).register(MultiPartFeature.class)
				.register(JacksonFeature.class).request(MediaType.APPLICATION_JSON_TYPE)
				.post(Entity.entity(multiPart, multiPart.getMediaType()));
	...
	public void createItem_SyntaxInvalidJSONFile() throws Exception {
		...
		multiPart.field("json",
				wrongJSON.replace("___COLLECTION_ID___", collectionId).replace("___FILENAME___", "test.png"));
	...
	public void createItem_InvalidFetchURL() throws IOException {
		...
		multiPart.field("json",
				itemJSON.replace("___COLLECTION_ID___", collectionId).replace("___FETCH_URL___", "invalid url")
	...
	public void createItem_FetchURL_NoFile() throws IOException {
		...
		multiPart.field("json",
				itemJSON.replace("___COLLECTION_ID___", collectionId).replace("___FETCH_URL___", "www.google.de")
	...
	public void createItemExtensionsTest() throws IOException, JSONException {
		...
		multiPart.bodyPart(filePart);
		multiPart.field("json",
				itemJSON.replace("___COLLECTION_ID___", collectionId).replace("___FILENAME___", "test.exe"));
	...
	public void test_7_ReadItemsWithQuery() throws Exception {
		...
		Response response = (target(PATH_PREFIX).queryParam("q", "\"" + itemTO.getFilename() + "\"")
				.register(authAsUser).register(MultiPartFeature.class).request(MediaType.APPLICATION_JSON_TYPE)).get();

		assertEquals(Status.OK.getStatusCode(), response.getStatus());
		SearchResultTO<DefaultItemTO> resultTO = RestProcessUtils.buildTOFromJSON(response.readEntity(String.class),
	...
	public void test_1_UpdateItem_2_WithFile_Fetched() throws ImejiException, IOException {
		...
		FormDataMultiPart multiPart = new FormDataMultiPart();
		multiPart.field("json",
				getStringFromPath(updateJSON).replace("___FILE_NAME___", UPDATED_FILE_NAME)
	...
	public void test_1_UpdateItem_4_WithFile_Attached_Fetched() throws IOException, ImejiException {
		...
		multiPart.bodyPart(filePart);
		multiPart.field("json",
				getStringFromPath(updateJSON).replace("___FILE_NAME___", newFile.getName())
						.replace("___FETCH_URL___", fileURL).replaceAll("\"id\"\\s*:\\s*\"__ITEM_ID__\",", "")
						.replace("___REFERENCE_URL___", ""));
	...
	public void test_1_UpdateItem_7_WithFile_Attached_Fetched_Referenced() throws IOException, ImejiException {
		...
		multiPart.bodyPart(filePart);
		multiPart.field("json",
				getStringFromPath(updateJSON).replaceAll("\"id\"\\s*:\\s*\"__ITEM_ID__\",", "")
	...
	public void test_1_deleteItem_WithNonAuth() throws Exception {
		...
		Response response2 = target(PATH_PREFIX).register(authAsUserFalse).path("/" + itemId)
	...
	public void test_2_deleteItem_NotAllowed() throws Exception {
		...
		Form form = new Form();
		form.param("id", itemId);
		Response response = target(PATH_PREFIX).register(authAsUser2).path("/" + itemId)
				.request(MediaType.APPLICATION_JSON_TYPE).delete();
	...
	public void test_3_deleteItem_NotExist() throws Exception {
		...
		Response response = target(PATH_PREFIX).register(authAsUser).path("/" + itemId + "i_do_not_exist")
				.request(MediaType.APPLICATION_JSON_TYPE).delete();
	...
	public void test_2_deleteItem_Released() throws Exception {
		...
		Form form = new Form();
		form.param("id", itemId);
		Response response = target(PATH_PREFIX).register(authAsUser).path("/" + itemId)
				.request(MediaType.APPLICATION_JSON_TYPE).delete();
