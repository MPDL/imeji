...
/**
 * The class overrides TestContainerFactory with GrizzlyWebContainerFactory. The GrizzlyWebContainer
 * fully supports HttpServlet, that is needed to test @Context injection variables in tests
 *
 * @author vmakarenko
 *
 */
public class MyTestContainerFactory implements TestContainerFactory {
      ...
      public void start() {
        try {
          this.server = create(baseUri, ServletContainer.class, null,
              ImmutableMap.of("jersey.config.server.provider.packages", "de.mpg.imeji.rest.resources",
  ...
  private static HttpServer create(URI u, Class<? extends Servlet> c, Servlet servlet, Map<String, String> initParams,
      Map<String, String> contextInitParams) throws IOException {
    if (u == null) {
    ...
    server.getServerConfiguration().addHttpHandler(new StaticHttpHandler(STATIC_CONTEXT_REST, STATIC_CONTEXT_STORAGE), STATIC_CONTEXT_PATH);
