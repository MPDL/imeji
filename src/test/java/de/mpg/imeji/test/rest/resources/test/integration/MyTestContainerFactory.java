...
/**
 * The class overrides TestContainerFactory with GrizzlyWebContainerFactory. The
 * GrizzlyWebContainer fully supports HttpServlet, that is needed to
 * test @Context injection variables in tests
 *
 * @author vmakarenko
 *
 */
public class MyTestContainerFactory implements TestContainerFactory {
	...
	public TestContainer create(final URI baseUri, final DeploymentContext deploymentContext)
		...
		return new TestContainer() {
			...
			public void start() {
				try {
					this.server = create(baseUri, ServletContainer.class, null, ImmutableMap.of(
