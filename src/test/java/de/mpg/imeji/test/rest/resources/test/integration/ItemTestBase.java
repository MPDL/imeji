...
public class ItemTestBase extends ImejiTestBase {
  ...
  protected static void createItem() throws Exception {
    ...
    CollectionImeji coll = cc.retrieve(ObjectHelper.getURI(CollectionImeji.class, collectionId), JenaUtil.testUser);
  ...
  public void test_5_defaultSyntax_badTypedValues(String itemId, String jSon) throws IOException {
    ...
    Response response = itemId.equals("") ? getCreateTargetAuth().post(Entity.entity(multiPart, multiPart.getMediaType()))
    ...
    response = itemId.equals("") ? getCreateTargetAuth().post(Entity.entity(multiPart, multiPart.getMediaType()))
        : getUpdateTargetAuth(itemId).put(Entity.entity(multiPart, multiPart.getMediaType()));
    assertEquals(HttpStatus.SC_UNPROCESSABLE_ENTITY, response.getStatus());
  ...
  private Invocation.Builder getCreateTargetAuth() {
    return target(TARGET_PATH_PREFIX).register(authAsUser).register(MultiPartFeature.class).register(JacksonFeature.class)
