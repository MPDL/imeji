var datasourceUrl,result,servlet,offset=2,id;function getDatasourceUrl(a,c){datasourceUrl=a;this.servlet=c;offset=2}function getDatasourceUrlWithFixedDelay(a,c,f){getDatasourceUrl(a,c);offset=f}function split(a){return a.split(/,\s*/)}function extractLast(a){return split(a).pop()}function setInputValue(a,c){null!=document.getElementById(a)&&(null!=c?document.getElementById(a).value=c:""==document.getElementById(a).value)}
$(function(){$(".autocomplete_js").bind("keydown",function(a){null!=datasourceUrl&&""!=datasourceUrl&&a.keyCode===$.ui.keyCode.TAB&&$(this).data("autocomplete").menu.active&&a.getPreventDefault()}).autocomplete({source:function(a,c){$.getJSON(servlet,{searchkeyword:a.term,datasource:datasourceUrl},function(a){result=a;c(result)})},minLength:0,messages:{noResults:"",results:function(){return""}},search:function(){if(null==datasourceUrl||""==datasourceUrl||extractLast(this.value).length<offset)return!1},
focus:function(){return!1},select:function(a,c){a=this.id.split(":");for(var f="",v=0;v<a.length-1;v++)f=f+a[v]+":";console.log(f);setInputValue(this.id,c.item.value);setInputValue(f+"identifier",c.item.id);setInputValue(f+"given",c.item.givenname);setInputValue(f+"family",c.item.family);setInputValue(f+"identifier",c.item.id);setInputValue(f+"organization:0:name",c.item.organization);setInputValue(f+"latitude",c.item.latitude);setInputValue(f+"longitude",c.item.longitude);return!1}}).focus(function(){0==
offset&&null!=datasourceUrl&&""!=datasourceUrl&&(this.value=" ",$(this).autocomplete("search"),this.value="",event.getPreventDefault())})});$(function(){$(".datepicker").datepicker({changeMonth:!0,changeYear:!0,dateFormat:"yy-mm-dd",firstDay:1})});function endsWith(a,c){return-1!==a.indexOf(c,a.length-c.length)}function formatNumber(a){return a.replace(",",".").replace(" ","")}function isNumber(a){a=formatNumber(a);return!isNaN(parseFloat(a))&&isFinite(a)}
function sortOptionsByText(){var a,c;if(a=jQuery('select[data-sort\x3d"sort"]'))c=jQuery(a.find("option")),c=jQuery.makeArray(c).sort(function(a,c){return a.innerHTML>c.innerHTML?1:-1}),a.html(c)}function loadContent(a,c,f){$.ajax({type:"GET",url:a,cache:!1,success:function(a){c&&$(c).html(a);f&&setTimeout(f,15,c,a)}})}function validateInputNumber(a){a.value=formatNumber(a.value);isNumber(a.value)||(a.value="")}
function parseId(a,c){(c?new RegExp(c+"(\\S*)"):/id_(\S*)/).exec(a);return""!==RegExp.$1?RegExp.$1:!1}function checkForChilds(a){var c;a=jQuery(".parent_"+a);0<a.length&&a.each(function(a,v){jQuery(this).addClass("imj_highlightDependencies");(c=parseId(jQuery(this).attr("class")))&&checkForChilds(c)})}function highlight(a,c){(c?$("."+c+a):$(".id_"+a)).addClass("imj_highlightDependencies")}function reset_highlight(){$(".imj_highlightDependencies").removeClass("imj_highlightDependencies")}
function highlighter(){var a=$(".highlight_area");a.off("mouseover");a.off("mouseout");a.mouseover(function(){var a=parseId(jQuery(this).attr("class")),f=parseId(jQuery(this).attr("class"),"parent_");highlight(a);checkForChilds(a);f&&highlight(f,"id_")}).mouseout(function(){reset_highlight()})}function submitPanel(a,c){a=document.getElementById(a);null!=a&&(a.innerHTML='\x3ch2\x3e\x3cspan class\x3d"free_area0_p8 xTiny_marginLExcl"\x3e'+c+"\x3c/span\x3e\x3c/h2\x3e")}
function openDialog(a){a=$(document.getElementById(a));a.css("left",Math.max(0,Math.round(($(window).width()-$(a).outerWidth())/2)+$(window).scrollLeft())+"px");a.show();$(".imj_modalDialogBackground").show()}function closeDialog(a){a=$(document.getElementById(a));$(".imj_modalDialogBackground").hide();a.hide()}$(window).resize(function(a){a=$(".imj_modalDialogBox:visible");0<a.length&&a.css("left",Math.max(0,Math.round(($(window).width()-$(a).outerWidth())/2)+$(window).scrollLeft())+"px")});
var swcObject={};
function initSWC(a){swcObject.data=$(a).text();swcObject.json=swc_parser(swcObject.data);a=document.createElement("canvas");window.WebGLRenderingContext&&(a.getContext("webgl")||a.getContext("experimental-webgl"))?(a=$("*[id*\x3d"+swcObject.placeholderID+"]"),a.get(0).style.display="none",a=new SharkViewer({swc:swcObject.json,dom_element:swcObject.displayID,WIDTH:swcObject.width,HEIGHT:swcObject.height,center_node:-1,show_stats:!1,screenshot:!1}),a.init(),a.animate()):document.getElementById(swcObject.failedMsgID).style.display="block"}
function loadSWC(a,c){var f;f=$(a);swcObject={domSource:a,dataURL:f.data("swc-source")||void 0,serviceURL:f.data("swc-service")||void 0,elementID:c,displayID:"#"===f.data("target-id")[0]?f.data("target-id").substring(1):f.data("target-id"),width:f.data("target-width"),height:f.data("target-height"),placeholderID:"#"===f.data("placeholder-id")[0]?f.data("placeholder-id").substring(1):f.data("placeholder-id"),failedMsgID:"#"===f.data("failed-msg-id")[0]?f.data("failed-msg-id").substring(1):f.data("failed-msg-id")};
initSWC("#"+swcObject.elementID)}function handleDisableButton(a){if("submit"===a.source.type)switch(a.status){case "begin":a.source.disabled=!0;break;case "complete":a.source.disabled=!1}}"undefined"!==typeof jsf&&jsf.ajax.addOnEvent(function(a){"success"===a.status&&fixViewState(a.responseXML);handleDisableButton(a)});
function fixViewState(a){if(a=getViewState(a))for(var c=0;c<document.forms.length;c++){var f=document.forms[c];"post"===f.method.toLowerCase()?hasViewState(f)||createViewState(f,a):removeViewState(f)}}function getViewState(a){a=a.getElementsByTagName("update");for(var c=0;c<a.length;c++){var f=a[c];if(f.getAttribute("id").match(/^([\w]+:)?javax\.faces\.ViewState(:[0-9]+)?$/))return f.firstChild.nodeValue}return null}
function hasViewState(a){for(var c=0;c<a.elements.length;c++)if("javax.faces.ViewState"==a.elements[c].name)return!0;return!1}function createViewState(a,c){var f;try{f=document.createElement("\x3cinput name\x3d'javax.faces.ViewState'\x3e")}catch(v){f=document.createElement("input"),f.setAttribute("name","javax.faces.ViewState")}f.setAttribute("type","hidden");f.setAttribute("value",c);f.setAttribute("autocomplete","off");a.appendChild(f)}
function removeViewState(a){for(var c=0;c<a.elements.length;c++){var f=a.elements[c];"javax.faces.ViewState"==f.name&&f.parentNode.removeChild(f)}}$(function(){$(".imj_overlayMenu").each(function(a,c){a=$(this).find(".imj_menuHeader").width();c=$(this).find(".imj_menuBody");a>c.width()&&c.width(a)})});
function menuRightOffset(){$(".imj_overlayMenu.imj_menuRight").each(function(a,c){a=$(this).find(".imj_menuHeader").width();c=$(this).find(".imj_menuBody").width();a-=c;$(this).find(".imj_menuBody").css("margin-left",a+"px")})}jQuery(document).ready(function(){highlighter();menuRightOffset()});var selectedSearch,numberOfContext=$(".imj_bodyContextSearch li").length;function submitSimpleSearch(){""!=$(".imj_simpleSearchInput").val()&&goToSearch(selectedSearch);return!1}
$(".imj_bodyContextSearch li").click(function(){goToSearch($(this).index()+1)});function goToSearch(a){var c="?";a=$(".imj_bodyContextSearch li:nth-child("+a+")").data("url");a.includes("?")&&(c="\x26");window.open(a+c+"q\x3d"+encodeURIComponent($(".imj_simpleSearchInput").val()),"_self")}
$(".imj_simpleSearchInput").focusin(function(){$(".imj_menuSimpleSearch").show()}).keyup(function(a){40==a.which?(incrementSelectedSearch(),highlightSearch()):38==a.which?(decrementSelectedSearch(),highlightSearch()):""!=$(this).val()&&$(".imj_menuSimpleSearch").show()});$(document).ready(function(){selectedSearch=1;var a=window.location.pathname;$("ul.imj_bodyContextSearch li").each(function(c){if(-1!==$(this).data("url").indexOf(a))return selectedSearch=c+1,!1});highlightSearch()});
function changePlaceholder(){var a=$("ul.imj_bodyContextSearch li:nth-child("+selectedSearch+")").data("placeholder");$("#quickSearchString").attr("placeholder",a)}$(".imj_simpleSearch").focusout(function(){$(".imj_menuSimpleSearch").delay(200).hide(0)});$("ul.imj_bodyContextSearch li").mouseover(function(){});$("ul.imj_bodyContextSearch li").mouseout(function(){highlightSearch()});
function highlightSearch(){$("ul.imj_bodyContextSearch li").removeClass("hovered");$("ul.imj_bodyContextSearch li:nth-child("+selectedSearch+")").addClass("hovered");changePlaceholder()}function incrementSelectedSearch(){selectedSearch<numberOfContext&&(selectedSearch+=1)}function decrementSelectedSearch(){1<selectedSearch&&--selectedSearch}$(document).ready(function(){resizeMenu()});function resizeMenu(){var a=$(".imj_menuHeader").outerWidth(!0);$(".imj_menuSimpleSearch").css({width:a})}
function lockPage(){$(".loaderWrapper").show()}function unlockPage(){$(".loaderWrapper").hide()}function startLoader(){$(".loaderWrapper").show();$(".loader").show()}function stopLoader(){$(".loaderWrapper").hide();$(".loader").hide()}jsf.ajax.addOnEvent(function(a){switch(a.status){case "complete":stopLoader()}});$(function(){$(".scrollbarTopWrapper").scroll(function(){$(".edit_selected_table_wrapper").scrollLeft($(".scrollbarTopWrapper").scrollLeft())});$(".edit_selected_table_wrapper").scroll(function(){$(".scrollbarTopWrapper").scrollLeft($(".edit_selected_table_wrapper").scrollLeft())})});
setTimeout(function(){$(".imj_messageSuccess").fadeOut(1E3)},2E3);function showAndFocus(a,c){$("."+c).toggle();document.getElementById(a).focus()}$(".selectMetadata-content").click(function(a){a.stopPropagation()});$(document).click(function(){$(".selectMetadata-content").hide()});function hideOnFocusOut(a){}$(".selectMetadata-content").focusout(function(a){console.log("focusout");$(this).delay(200).hide(0)});
qrcode&&(qrcode.stringToBytes=qrcode.createStringToBytes("\x3d",7070));
var qrcode=function(){function a(c,f){if("undefined"==typeof c.length)throw Error(c.length+"/"+f);var n=function(){for(var a=0;a<c.length&&0==c[a];)a+=1;for(var b=Array(c.length-a+f),e=0;e<c.length-a;e+=1)b[e]=c[e+a];return b}(),b={getAt:function(a){return n[a]},getLength:function(){return n.length},multiply:function(h){for(var g=Array(b.getLength()+h.getLength()-1),e=0;e<b.getLength();e+=1)for(var d=0;d<h.getLength();d+=1)g[e+d]^=u.gexp(u.glog(b.getAt(e))+u.glog(h.getAt(d)));return a(g,0)},mod:function(h){if(0>
b.getLength()-h.getLength())return b;for(var g=u.glog(b.getAt(0))-u.glog(h.getAt(0)),e=Array(b.getLength()),d=0;d<b.getLength();d+=1)e[d]=b.getAt(d);for(d=0;d<h.getLength();d+=1)e[d]^=u.gexp(u.glog(h.getAt(d))+g);return a(e,0).mod(h)}};return b}var c=function(c,k){var n=f[k],b=null,h=0,g=null,e=[],d={},r=function(d,f){for(var l=h=4*c+17,m=Array(l),k=0;k<l;k+=1){m[k]=Array(l);for(var r=0;r<l;r+=1)m[k][r]=null}b=m;y(0,0);y(h-7,0);y(0,h-7);l=v.getPatternPosition(c);for(m=0;m<l.length;m+=1)for(k=0;k<
l.length;k+=1){var r=l[m],t=l[k];if(null==b[r][t])for(var p=-2;2>=p;p+=1)for(var q=-2;2>=q;q+=1)b[r+p][t+q]=-2==p||2==p||-2==q||2==q||0==p&&0==q?!0:!1}for(l=8;l<h-8;l+=1)null==b[l][6]&&(b[l][6]=0==l%2);for(l=8;l<h-8;l+=1)null==b[6][l]&&(b[6][l]=0==l%2);l=v.getBCHTypeInfo(n<<3|f);for(m=0;15>m;m+=1)k=!d&&1==(l>>m&1),6>m?b[m][8]=k:8>m?b[m+1][8]=k:b[h-15+m][8]=k;for(m=0;15>m;m+=1)k=!d&&1==(l>>m&1),8>m?b[8][h-m-1]=k:9>m?b[8][15-m-1+1]=k:b[8][15-m-1]=k;b[h-8][8]=!d;if(7<=c){l=v.getBCHTypeNumber(c);for(m=
0;18>m;m+=1)k=!d&&1==(l>>m&1),b[Math.floor(m/3)][m%3+h-8-3]=k;for(m=0;18>m;m+=1)k=!d&&1==(l>>m&1),b[m%3+h-8-3][Math.floor(m/3)]=k}if(null==g){d=A.getRSBlocks(c,n);l=B();for(m=0;m<e.length;m+=1)k=e[m],l.put(k.getMode(),4),l.put(k.getLength(),v.getLengthInBits(k.getMode(),c)),k.write(l);for(m=k=0;m<d.length;m+=1)k+=d[m].dataCount;if(l.getLengthInBits()>8*k)throw Error("code length overflow. ("+l.getLengthInBits()+"\x3e"+8*k+")");for(l.getLengthInBits()+4<=8*k&&l.put(0,4);0!=l.getLengthInBits()%8;)l.putBit(!1);
for(;!(l.getLengthInBits()>=8*k);){l.put(236,8);if(l.getLengthInBits()>=8*k)break;l.put(17,8)}for(var w=0,k=m=0,r=Array(d.length),t=Array(d.length),p=0;p<d.length;p+=1){var u=d[p].dataCount,x=d[p].totalCount-u,m=Math.max(m,u),k=Math.max(k,x);r[p]=Array(u);for(q=0;q<r[p].length;q+=1)r[p][q]=255&l.getBuffer()[q+w];w+=u;q=v.getErrorCorrectPolynomial(x);u=a(r[p],q.getLength()-1).mod(q);t[p]=Array(q.getLength()-1);for(q=0;q<t[p].length;q+=1)x=q+u.getLength()-t[p].length,t[p][q]=0<=x?u.getAt(x):0}for(q=
l=0;q<d.length;q+=1)l+=d[q].totalCount;l=Array(l);for(q=w=0;q<m;q+=1)for(p=0;p<d.length;p+=1)q<r[p].length&&(l[w]=r[p][q],w+=1);for(q=0;q<k;q+=1)for(p=0;p<d.length;p+=1)q<t[p].length&&(l[w]=t[p][q],w+=1);g=l}d=g;l=-1;m=h-1;k=7;r=0;f=v.getMaskFunction(f);for(t=h-1;0<t;t-=2)for(6==t&&--t;;){for(p=0;2>p;p+=1)null==b[m][t-p]&&(q=!1,r<d.length&&(q=1==(d[r]>>>k&1)),f(m,t-p)&&(q=!q),b[m][t-p]=q,--k,-1==k&&(r+=1,k=7));m+=l;if(0>m||h<=m){m-=l;l=-l;break}}},y=function(a,g){for(var d=-1;7>=d;d+=1)if(!(-1>=a+
d||h<=a+d))for(var e=-1;7>=e;e+=1)-1>=g+e||h<=g+e||(b[a+d][g+e]=0<=d&&6>=d&&(0==e||6==e)||0<=e&&6>=e&&(0==d||6==d)||2<=d&&4>=d&&2<=e&&4>=e?!0:!1)};d.addData=function(a){a=C(a);e.push(a);g=null};d.isDark=function(a,d){if(0>a||h<=a||0>d||h<=d)throw Error(a+","+d);return b[a][d]};d.getModuleCount=function(){return h};d.make=function(){for(var a=0,h=0,b=0;8>b;b+=1){r(!0,b);var e=v.getLostPoint(d);if(0==b||a>e)a=e,h=b}r(!1,h)};d.createTableTag=function(a,b){a=a||2;b='\x3ctable style\x3d" border-width: 0px; border-style: none; border-collapse: collapse;'+
(" padding: 0px; margin: "+("undefined"==typeof b?4*a:b)+"px;")+'"\x3e\x3ctbody\x3e';for(var h=0;h<d.getModuleCount();h+=1){b+="\x3ctr\x3e";for(var e=0;e<d.getModuleCount();e+=1)b+='\x3ctd style\x3d"',b+=" border-width: 0px; border-style: none;",b+=" border-collapse: collapse;",b+=" padding: 0px; margin: 0px;",b+=" width: "+a+"px;",b+=" height: "+a+"px;",b+=" background-color: ",b+=d.isDark(h,e)?"#000000":"#ffffff",b+=";",b+='"/\x3e';b+="\x3c/tr\x3e"}return b+"\x3c/tbody\x3e\x3c/table\x3e"};d.createImgTag=
function(a,b){a=a||2;b="undefined"==typeof b?4*a:b;var h=d.getModuleCount()*a+2*b,e=b,g=h-b;return D(h,h,function(b,h){return e<=b&&b<g&&e<=h&&h<g?d.isDark(Math.floor((h-e)/a),Math.floor((b-e)/a))?0:1:1})};return d};c.stringToBytes=function(a){for(var k=[],c=0;c<a.length;c+=1){var b=a.charCodeAt(c);k.push(b&255)}return k};c.createStringToBytes=function(a,c){var k=function(){for(var b=E(a),h=function(){var a=b.read();if(-1==a)throw Error();return a},g=0,e={};;){var d=b.read();if(-1==d)break;var k=
h(),n=h(),r=h(),d=String.fromCharCode(d<<8|k);e[d]=n<<8|r;g+=1}if(g!=c)throw Error(g+" !\x3d "+c);return e}();return function(a){for(var b=[],g=0;g<a.length;g+=1){var e=a.charCodeAt(g);128>e?b.push(e):(e=k[a.charAt(g)],"number"==typeof e?(e&255)==e?b.push(e):(b.push(e>>>8),b.push(e&255)):b.push(63))}return b}};var f={L:1,M:0,Q:3,H:2},v=function(){var c=[[],[6,18],[6,22],[6,26],[6,30],[6,34],[6,22,38],[6,24,42],[6,26,46],[6,28,50],[6,30,54],[6,32,58],[6,34,62],[6,26,46,66],[6,26,48,70],[6,26,50,74],
[6,30,54,78],[6,30,56,82],[6,30,58,86],[6,34,62,90],[6,28,50,72,94],[6,26,50,74,98],[6,30,54,78,102],[6,28,54,80,106],[6,32,58,84,110],[6,30,58,86,114],[6,34,62,90,118],[6,26,50,74,98,122],[6,30,54,78,102,126],[6,26,52,78,104,130],[6,30,56,82,108,134],[6,34,60,86,112,138],[6,30,58,86,114,142],[6,34,62,90,118,146],[6,30,54,78,102,126,150],[6,24,50,76,102,128,154],[6,28,54,80,106,132,158],[6,32,58,84,110,136,162],[6,26,54,82,110,138,166],[6,30,58,86,114,142,170]],k={},n=function(a){for(var b=0;0!=a;)b+=
1,a>>>=1;return b};k.getBCHTypeInfo=function(a){for(var b=a<<10;0<=n(b)-n(1335);)b^=1335<<n(b)-n(1335);return(a<<10|b)^21522};k.getBCHTypeNumber=function(a){for(var b=a<<12;0<=n(b)-n(7973);)b^=7973<<n(b)-n(7973);return a<<12|b};k.getPatternPosition=function(a){return c[a-1]};k.getMaskFunction=function(a){switch(a){case 0:return function(a,b){return 0==(a+b)%2};case 1:return function(a,b){return 0==a%2};case 2:return function(a,b){return 0==b%3};case 3:return function(a,b){return 0==(a+b)%3};case 4:return function(a,
b){return 0==(Math.floor(a/2)+Math.floor(b/3))%2};case 5:return function(a,b){return 0==a*b%2+a*b%3};case 6:return function(a,b){return 0==(a*b%2+a*b%3)%2};case 7:return function(a,b){return 0==(a*b%3+(a+b)%2)%2};default:throw Error("bad maskPattern:"+a);}};k.getErrorCorrectPolynomial=function(b){for(var h=a([1],0),g=0;g<b;g+=1)h=h.multiply(a([1,u.gexp(g)],0));return h};k.getLengthInBits=function(a,h){if(1<=h&&10>h)switch(a){case 1:return 10;case 2:return 9;case 4:return 8;case 8:return 8;default:throw Error("mode:"+
a);}else if(27>h)switch(a){case 1:return 12;case 2:return 11;case 4:return 16;case 8:return 10;default:throw Error("mode:"+a);}else if(41>h)switch(a){case 1:return 14;case 2:return 13;case 4:return 16;case 8:return 12;default:throw Error("mode:"+a);}else throw Error("type:"+h);};k.getLostPoint=function(a){for(var b=a.getModuleCount(),g=0,e=0;e<b;e+=1)for(var d=0;d<b;d+=1){for(var c=0,k=a.isDark(e,d),n=-1;1>=n;n+=1)if(!(0>e+n||b<=e+n))for(var r=-1;1>=r;r+=1)0>d+r||b<=d+r||0==n&&0==r||k!=a.isDark(e+
n,d+r)||(c+=1);5<c&&(g+=3+c-5)}for(e=0;e<b-1;e+=1)for(d=0;d<b-1;d+=1)if(c=0,a.isDark(e,d)&&(c+=1),a.isDark(e+1,d)&&(c+=1),a.isDark(e,d+1)&&(c+=1),a.isDark(e+1,d+1)&&(c+=1),0==c||4==c)g+=3;for(e=0;e<b;e+=1)for(d=0;d<b-6;d+=1)a.isDark(e,d)&&!a.isDark(e,d+1)&&a.isDark(e,d+2)&&a.isDark(e,d+3)&&a.isDark(e,d+4)&&!a.isDark(e,d+5)&&a.isDark(e,d+6)&&(g+=40);for(d=0;d<b;d+=1)for(e=0;e<b-6;e+=1)a.isDark(e,d)&&!a.isDark(e+1,d)&&a.isDark(e+2,d)&&a.isDark(e+3,d)&&a.isDark(e+4,d)&&!a.isDark(e+5,d)&&a.isDark(e+6,
d)&&(g+=40);for(d=c=0;d<b;d+=1)for(e=0;e<b;e+=1)a.isDark(e,d)&&(c+=1);return g+Math.abs(100*c/b/b-50)/5*10};return k}(),u=function(){for(var a=Array(256),c=Array(256),n=0;8>n;n+=1)a[n]=1<<n;for(n=8;256>n;n+=1)a[n]=a[n-4]^a[n-5]^a[n-6]^a[n-8];for(n=0;255>n;n+=1)c[a[n]]=n;return{glog:function(a){if(1>a)throw Error("glog("+a+")");return c[a]},gexp:function(b){for(;0>b;)b+=255;for(;256<=b;)b-=255;return a[b]}}}(),A=function(){var a=[[1,26,19],[1,26,16],[1,26,13],[1,26,9],[1,44,34],[1,44,28],[1,44,22],
[1,44,16],[1,70,55],[1,70,44],[2,35,17],[2,35,13],[1,100,80],[2,50,32],[2,50,24],[4,25,9],[1,134,108],[2,67,43],[2,33,15,2,34,16],[2,33,11,2,34,12],[2,86,68],[4,43,27],[4,43,19],[4,43,15],[2,98,78],[4,49,31],[2,32,14,4,33,15],[4,39,13,1,40,14],[2,121,97],[2,60,38,2,61,39],[4,40,18,2,41,19],[4,40,14,2,41,15],[2,146,116],[3,58,36,2,59,37],[4,36,16,4,37,17],[4,36,12,4,37,13],[2,86,68,2,87,69],[4,69,43,1,70,44],[6,43,19,2,44,20],[6,43,15,2,44,16],[4,101,81],[1,80,50,4,81,51],[4,50,22,4,51,23],[3,36,12,
8,37,13],[2,116,92,2,117,93],[6,58,36,2,59,37],[4,46,20,6,47,21],[7,42,14,4,43,15],[4,133,107],[8,59,37,1,60,38],[8,44,20,4,45,21],[12,33,11,4,34,12],[3,145,115,1,146,116],[4,64,40,5,65,41],[11,36,16,5,37,17],[11,36,12,5,37,13],[5,109,87,1,110,88],[5,65,41,5,66,42],[5,54,24,7,55,25],[11,36,12,7,37,13],[5,122,98,1,123,99],[7,73,45,3,74,46],[15,43,19,2,44,20],[3,45,15,13,46,16],[1,135,107,5,136,108],[10,74,46,1,75,47],[1,50,22,15,51,23],[2,42,14,17,43,15],[5,150,120,1,151,121],[9,69,43,4,70,44],[17,
50,22,1,51,23],[2,42,14,19,43,15],[3,141,113,4,142,114],[3,70,44,11,71,45],[17,47,21,4,48,22],[9,39,13,16,40,14],[3,135,107,5,136,108],[3,67,41,13,68,42],[15,54,24,5,55,25],[15,43,15,10,44,16],[4,144,116,4,145,117],[17,68,42],[17,50,22,6,51,23],[19,46,16,6,47,17],[2,139,111,7,140,112],[17,74,46],[7,54,24,16,55,25],[34,37,13],[4,151,121,5,152,122],[4,75,47,14,76,48],[11,54,24,14,55,25],[16,45,15,14,46,16],[6,147,117,4,148,118],[6,73,45,14,74,46],[11,54,24,16,55,25],[30,46,16,2,47,17],[8,132,106,4,
133,107],[8,75,47,13,76,48],[7,54,24,22,55,25],[22,45,15,13,46,16],[10,142,114,2,143,115],[19,74,46,4,75,47],[28,50,22,6,51,23],[33,46,16,4,47,17],[8,152,122,4,153,123],[22,73,45,3,74,46],[8,53,23,26,54,24],[12,45,15,28,46,16],[3,147,117,10,148,118],[3,73,45,23,74,46],[4,54,24,31,55,25],[11,45,15,31,46,16],[7,146,116,7,147,117],[21,73,45,7,74,46],[1,53,23,37,54,24],[19,45,15,26,46,16],[5,145,115,10,146,116],[19,75,47,10,76,48],[15,54,24,25,55,25],[23,45,15,25,46,16],[13,145,115,3,146,116],[2,74,46,
29,75,47],[42,54,24,1,55,25],[23,45,15,28,46,16],[17,145,115],[10,74,46,23,75,47],[10,54,24,35,55,25],[19,45,15,35,46,16],[17,145,115,1,146,116],[14,74,46,21,75,47],[29,54,24,19,55,25],[11,45,15,46,46,16],[13,145,115,6,146,116],[14,74,46,23,75,47],[44,54,24,7,55,25],[59,46,16,1,47,17],[12,151,121,7,152,122],[12,75,47,26,76,48],[39,54,24,14,55,25],[22,45,15,41,46,16],[6,151,121,14,152,122],[6,75,47,34,76,48],[46,54,24,10,55,25],[2,45,15,64,46,16],[17,152,122,4,153,123],[29,74,46,14,75,47],[49,54,24,
10,55,25],[24,45,15,46,46,16],[4,152,122,18,153,123],[13,74,46,32,75,47],[48,54,24,14,55,25],[42,45,15,32,46,16],[20,147,117,4,148,118],[40,75,47,7,76,48],[43,54,24,22,55,25],[10,45,15,67,46,16],[19,148,118,6,149,119],[18,75,47,31,76,48],[34,54,24,34,55,25],[20,45,15,61,46,16]],c=function(a,b){var e={};e.totalCount=a;e.dataCount=b;return e},n={},b=function(b,c){switch(c){case f.L:return a[4*(b-1)+0];case f.M:return a[4*(b-1)+1];case f.Q:return a[4*(b-1)+2];case f.H:return a[4*(b-1)+3]}};n.getRSBlocks=
function(a,g){var e=b(a,g);if("undefined"==typeof e)throw Error("bad rs block @ typeNumber:"+a+"/errorCorrectLevel:"+g);a=e.length/3;g=[];for(var d=0;d<a;d+=1)for(var k=e[3*d+0],h=e[3*d+1],n=e[3*d+2],f=0;f<k;f+=1)g.push(c(h,n));return g};return n}(),B=function(){var a=[],c=0,n={getBuffer:function(){return a},getAt:function(b){return 1==(a[Math.floor(b/8)]>>>7-b%8&1)},put:function(a,c){for(var b=0;b<c;b+=1)n.putBit(1==(a>>>c-b-1&1))},getLengthInBits:function(){return c},putBit:function(b){var k=Math.floor(c/
8);a.length<=k&&a.push(0);b&&(a[k]|=128>>>c%8);c+=1}};return n},C=function(a){var k=c.stringToBytes(a);return{getMode:function(){return 4},getLength:function(a){return k.length},write:function(a){for(var b=0;b<k.length;b+=1)a.put(k[b],8)}}},z=function(){var a=[],c={writeByte:function(c){a.push(c&255)},writeShort:function(a){c.writeByte(a);c.writeByte(a>>>8)},writeBytes:function(a,b,k){b=b||0;k=k||a.length;for(var g=0;g<k;g+=1)c.writeByte(a[g+b])},writeString:function(a){for(var b=0;b<a.length;b+=
1)c.writeByte(a.charCodeAt(b))},toByteArray:function(){return a},toString:function(){var c;c="[";for(var b=0;b<a.length;b+=1)0<b&&(c+=","),c+=a[b];return c+"]"}};return c},F=function(){var a=0,c=0,f=0,b="",h={},g=function(a){if(!(0>a)){if(26>a)return 65+a;if(52>a)return 97+(a-26);if(62>a)return 48+(a-52);if(62==a)return 43;if(63==a)return 47}throw Error("n:"+a);};h.writeByte=function(e){a=a<<8|e&255;c+=8;for(f+=1;6<=c;)b+=String.fromCharCode(g(a>>>c-6&63)),c-=6};h.flush=function(){0<c&&(b+=String.fromCharCode(g(a<<
6-c&63)),c=a=0);if(0!=f%3)for(var e=3-f%3,d=0;d<e;d+=1)b+="\x3d"};h.toString=function(){return b};return h},E=function(a){var c=0,f=0,b=0,h=function(a){if(65<=a&&90>=a)return a-65;if(97<=a&&122>=a)return a-97+26;if(48<=a&&57>=a)return a-48+52;if(43==a)return 62;if(47==a)return 63;throw Error("c:"+a);};return{read:function(){for(;8>b;){if(c>=a.length){if(0==b)return-1;throw Error("unexpected end of file./"+b);}var g=a.charAt(c);c+=1;if("\x3d"==g)return b=0,-1;g.match(/^\s$/)||(f=f<<6|h(g.charCodeAt(0)),
b+=6)}g=f>>>b-8&255;b-=8;return g}}},G=function(a,c){var k=Array(a*c),b=function(a){var b=0,c=0;return{write:function(d,e){if(0!=d>>>e)throw Error("length over");for(;8<=b+e;)a.writeByte(255&(d<<b|c)),e-=8-b,d>>>=8-b,b=c=0;c|=d<<b;b+=e},flush:function(){0<b&&a.writeByte(c)}}},h=function(){var a={},b=0,c={add:function(d){if(c.contains(d))throw Error("dup key:"+d);a[d]=b;b+=1},size:function(){return b},indexOf:function(b){return a[b]},contains:function(b){return"undefined"!=typeof a[b]}};return c};
return{setPixel:function(b,c,d){k[c*a+b]=d},write:function(g){g.writeString("GIF87a");g.writeShort(a);g.writeShort(c);g.writeByte(128);g.writeByte(0);g.writeByte(0);g.writeByte(0);g.writeByte(0);g.writeByte(0);g.writeByte(255);g.writeByte(255);g.writeByte(255);g.writeString(",");g.writeShort(0);g.writeShort(0);g.writeShort(a);g.writeShort(c);g.writeByte(0);var e;e=3;for(var d=h(),f=0;4>f;f+=1)d.add(String.fromCharCode(f));d.add(String.fromCharCode(4));d.add(String.fromCharCode(5));var f=z(),n=b(f);
n.write(4,e);for(var p=0,r=String.fromCharCode(k[p]),p=p+1;p<k.length;){var l=String.fromCharCode(k[p]),p=p+1;d.contains(r+l)?r+=l:(n.write(d.indexOf(r),e),4095>d.size()&&(d.size()==1<<e&&(e+=1),d.add(r+l)),r=l)}n.write(d.indexOf(r),e);n.write(5,e);n.flush();e=f.toByteArray();g.writeByte(2);for(d=0;255<e.length-d;)g.writeByte(255),g.writeBytes(e,d,255),d+=255;g.writeByte(e.length-d);g.writeBytes(e,d,e.length-d);g.writeByte(0);g.writeString(";")}}},D=function(a,c,f,b){for(var h=G(a,c),g=0;g<c;g+=1)for(var e=
0;e<a;e+=1)h.setPixel(e,g,f(e,g));f=z();h.write(f);h=F();f=f.toByteArray();for(g=0;g<f.length;g+=1)h.writeByte(f[g]);h.flush();h='\x3cimg src\x3d"data:image/gif;base64,'+h+'" width\x3d"';h+=a;h+='"';h+=' height\x3d"';h+=c;h+='"';b&&(h+=' alt\x3d"',h+=b,h+='"');return h+="/\x3e"};return c}();(function(a){"function"===typeof define&&define.amd?define([],a):"object"===typeof exports&&(module.exports=a())})(function(){return qrcode});