...
focus:function(){return!1},select:function(a,c){a=this.id.split(":");for(var g="",v=0;v<a.length-1;v++)g=g+a[v]+":";console.log(g);setInputValue(this.id,c.item.value);setInputValue(g+"identifier",c.item.id);setInputValue(g+"given",c.item.givenname);setInputValue(g+"family",c.item.family);setInputValue(g+"identifier",c.item.id);setInputValue(g+"organization:0:name",c.item.organization);setInputValue(g+"latitude",c.item.latitude);setInputValue(g+"longitude",c.item.longitude);return!1}}).focus(function(){0==
...
function sortOptionsByText(){var a,c;if(a=jQuery('select[data-sort\x3d"sort"]'))c=jQuery(a.find("option")),c=jQuery.makeArray(c).sort(function(a,c){return a.innerHTML>c.innerHTML?1:-1}),a.html(c)}function loadContent(a,c,g){$.ajax({type:"GET",url:a,cache:!1,success:function(a){c&&$(c).html(a);g&&setTimeout(g,15,c,a)}})}function validateInputNumber(a){a.value=formatNumber(a.value);isNumber(a.value)||(a.value="")}
...
function highlighter(){var a=$(".highlight_area");a.off("mouseover");a.off("mouseout");a.mouseover(function(){var a=parseId(jQuery(this).attr("class")),g=parseId(jQuery(this).attr("class"),"parent_");highlight(a);checkForChilds(a);g&&highlight(g,"id_")}).mouseout(function(){reset_highlight()})}function submitPanel(a,c){a=document.getElementById(a);null!=a&&(a.innerHTML='\x3ch2\x3e\x3cspan class\x3d"free_area0_p8 xTiny_marginLExcl"\x3e'+c+"\x3c/span\x3e\x3c/h2\x3e")}
...
function loadSWC(a,c){var g;g=$(a);swcObject={domSource:a,dataURL:g.data("swc-source")||void 0,serviceURL:g.data("swc-service")||void 0,elementID:c,displayID:"#"===g.data("target-id")[0]?g.data("target-id").substring(1):g.data("target-id"),width:g.data("target-width"),height:g.data("target-height"),placeholderID:"#"===g.data("placeholder-id")[0]?g.data("placeholder-id").substring(1):g.data("placeholder-id"),failedMsgID:"#"===g.data("failed-msg-id")[0]?g.data("failed-msg-id").substring(1):g.data("failed-msg-id")};
...
function fixViewState(a){if(a=getViewState(a))for(var c=0;c<document.forms.length;c++){var g=document.forms[c];"post"===g.method.toLowerCase()?hasViewState(g)||createViewState(g,a):removeViewState(g)}}function getViewState(a){a=a.getElementsByTagName("update");for(var c=0;c<a.length;c++){var g=a[c];if(g.getAttribute("id").match(/^([\w]+:)?javax\.faces\.ViewState(:[0-9]+)?$/))return g.firstChild.nodeValue}return null}
function hasViewState(a){for(var c=0;c<a.elements.length;c++)if("javax.faces.ViewState"==a.elements[c].name)return!0;return!1}function createViewState(a,c){var g;try{g=document.createElement("\x3cinput name\x3d'javax.faces.ViewState'\x3e")}catch(v){g=document.createElement("input"),g.setAttribute("name","javax.faces.ViewState")}g.setAttribute("type","hidden");g.setAttribute("value",c);g.setAttribute("autocomplete","off");a.appendChild(g)}
function removeViewState(a){for(var c=0;c<a.elements.length;c++){var g=a.elements[c];"javax.faces.ViewState"==g.name&&g.parentNode.removeChild(g)}}$(function(){$(".imj_overlayMenu").each(function(a,c){a=$(this).find(".imj_menuHeader").width();c=$(this).find(".imj_menuBody");a>c.width()&&c.width(a)})});
...
var qrcode=function(){function a(c,g){if("undefined"==typeof c.length)throw Error(c.length+"/"+g);var n=function(){for(var a=0;a<c.length&&0==c[a];)a+=1;for(var b=Array(c.length-a+g),e=0;e<c.length-a;e+=1)b[e]=c[e+a];return b}(),b={getAt:function(a){return n[a]},getLength:function(){return n.length},multiply:function(f){for(var h=Array(b.getLength()+f.getLength()-1),e=0;e<b.getLength();e+=1)for(var d=0;d<f.getLength();d+=1)h[e+d]^=u.gexp(u.glog(b.getAt(e))+u.glog(f.getAt(d)));return a(h,0)},mod:function(f){if(0>
b.getLength()-f.getLength())return b;for(var h=u.glog(b.getAt(0))-u.glog(f.getAt(0)),e=Array(b.getLength()),d=0;d<b.getLength();d+=1)e[d]=b.getAt(d);for(d=0;d<f.getLength();d+=1)e[d]^=u.gexp(u.glog(f.getAt(d))+h);return a(e,0).mod(f)}};return b}var c=function(c,k){var n=g[k],b=null,f=0,h=null,e=[],d={},r=function(d,g){for(var l=f=4*c+17,m=Array(l),k=0;k<l;k+=1){m[k]=Array(l);for(var r=0;r<l;r+=1)m[k][r]=null}b=m;y(0,0);y(f-7,0);y(0,f-7);l=v.getPatternPosition(c);for(m=0;m<l.length;m+=1)for(k=0;k<
l.length;k+=1){var r=l[m],t=l[k];if(null==b[r][t])for(var p=-2;2>=p;p+=1)for(var q=-2;2>=q;q+=1)b[r+p][t+q]=-2==p||2==p||-2==q||2==q||0==p&&0==q?!0:!1}for(l=8;l<f-8;l+=1)null==b[l][6]&&(b[l][6]=0==l%2);for(l=8;l<f-8;l+=1)null==b[6][l]&&(b[6][l]=0==l%2);l=v.getBCHTypeInfo(n<<3|g);for(m=0;15>m;m+=1)k=!d&&1==(l>>m&1),6>m?b[m][8]=k:8>m?b[m+1][8]=k:b[f-15+m][8]=k;for(m=0;15>m;m+=1)k=!d&&1==(l>>m&1),8>m?b[8][f-m-1]=k:9>m?b[8][15-m-1+1]=k:b[8][15-m-1]=k;b[f-8][8]=!d;if(7<=c){l=v.getBCHTypeNumber(c);for(m=
0;18>m;m+=1)k=!d&&1==(l>>m&1),b[Math.floor(m/3)][m%3+f-8-3]=k;for(m=0;18>m;m+=1)k=!d&&1==(l>>m&1),b[m%3+f-8-3][Math.floor(m/3)]=k}if(null==h){d=A.getRSBlocks(c,n);l=B();for(m=0;m<e.length;m+=1)k=e[m],l.put(k.getMode(),4),l.put(k.getLength(),v.getLengthInBits(k.getMode(),c)),k.write(l);for(m=k=0;m<d.length;m+=1)k+=d[m].dataCount;if(l.getLengthInBits()>8*k)throw Error("code length overflow. ("+l.getLengthInBits()+"\x3e"+8*k+")");for(l.getLengthInBits()+4<=8*k&&l.put(0,4);0!=l.getLengthInBits()%8;)l.putBit(!1);
...
l=0;q<d.length;q+=1)l+=d[q].totalCount;l=Array(l);for(q=w=0;q<m;q+=1)for(p=0;p<d.length;p+=1)q<r[p].length&&(l[w]=r[p][q],w+=1);for(q=0;q<k;q+=1)for(p=0;p<d.length;p+=1)q<t[p].length&&(l[w]=t[p][q],w+=1);h=l}d=h;l=-1;m=f-1;k=7;r=0;g=v.getMaskFunction(g);for(t=f-1;0<t;t-=2)for(6==t&&--t;;){for(p=0;2>p;p+=1)null==b[m][t-p]&&(q=!1,r<d.length&&(q=1==(d[r]>>>k&1)),g(m,t-p)&&(q=!q),b[m][t-p]=q,--k,-1==k&&(r+=1,k=7));m+=l;if(0>m||f<=m){m-=l;l=-l;break}}},y=function(a,h){for(var d=-1;7>=d;d+=1)if(!(-1>=a+
d||f<=a+d))for(var e=-1;7>=e;e+=1)-1>=h+e||f<=h+e||(b[a+d][h+e]=0<=d&&6>=d&&(0==e||6==e)||0<=e&&6>=e&&(0==d||6==d)||2<=d&&4>=d&&2<=e&&4>=e?!0:!1)};d.addData=function(a){a=C(a);e.push(a);h=null};d.isDark=function(a,d){if(0>a||f<=a||0>d||f<=d)throw Error(a+","+d);return b[a][d]};d.getModuleCount=function(){return f};d.make=function(){for(var a=0,f=0,b=0;8>b;b+=1){r(!0,b);var e=v.getLostPoint(d);if(0==b||a>e)a=e,f=b}r(!1,f)};d.createTableTag=function(a,b){a=a||2;b='\x3ctable style\x3d" border-width: 0px; border-style: none; border-collapse: collapse;'+
(" padding: 0px; margin: "+("undefined"==typeof b?4*a:b)+"px;")+'"\x3e\x3ctbody\x3e';for(var f=0;f<d.getModuleCount();f+=1){b+="\x3ctr\x3e";for(var e=0;e<d.getModuleCount();e+=1)b+='\x3ctd style\x3d"',b+=" border-width: 0px; border-style: none;",b+=" border-collapse: collapse;",b+=" padding: 0px; margin: 0px;",b+=" width: "+a+"px;",b+=" height: "+a+"px;",b+=" background-color: ",b+=d.isDark(f,e)?"#000000":"#ffffff",b+=";",b+='"/\x3e';b+="\x3c/tr\x3e"}return b+"\x3c/tbody\x3e\x3c/table\x3e"};d.createImgTag=
function(a,b){a=a||2;b="undefined"==typeof b?4*a:b;var f=d.getModuleCount()*a+2*b,e=b,h=f-b;return D(f,f,function(b,f){return e<=b&&b<h&&e<=f&&f<h?d.isDark(Math.floor((f-e)/a),Math.floor((b-e)/a))?0:1:1})};return d};c.stringToBytes=function(a){for(var k=[],c=0;c<a.length;c+=1){var b=a.charCodeAt(c);k.push(b&255)}return k};c.createStringToBytes=function(a,k){var c=function(){for(var b=E(a),f=function(){var a=b.read();if(-1==a)throw Error();return a},h=0,e={};;){var d=b.read();if(-1==d)break;var c=
f(),n=f(),r=f(),d=String.fromCharCode(d<<8|c);e[d]=n<<8|r;h+=1}if(h!=k)throw Error(h+" !\x3d "+k);return e}();return function(a){for(var b=[],h=0;h<a.length;h+=1){var e=a.charCodeAt(h);128>e?b.push(e):(e=c[a.charAt(h)],"number"==typeof e?(e&255)==e?b.push(e):(b.push(e>>>8),b.push(e&255)):b.push(63))}return b}};var g={L:1,M:0,Q:3,H:2},v=function(){var c=[[],[6,18],[6,22],[6,26],[6,30],[6,34],[6,22,38],[6,24,42],[6,26,46],[6,28,50],[6,30,54],[6,32,58],[6,34,62],[6,26,46,66],[6,26,48,70],[6,26,50,74],
...
b){return 0==(Math.floor(a/2)+Math.floor(b/3))%2};case 5:return function(a,b){return 0==a*b%2+a*b%3};case 6:return function(a,b){return 0==(a*b%2+a*b%3)%2};case 7:return function(a,b){return 0==(a*b%3+(a+b)%2)%2};default:throw Error("bad maskPattern:"+a);}};k.getErrorCorrectPolynomial=function(b){for(var f=a([1],0),h=0;h<b;h+=1)f=f.multiply(a([1,u.gexp(h)],0));return f};k.getLengthInBits=function(a,f){if(1<=f&&10>f)switch(a){case 1:return 10;case 2:return 9;case 4:return 8;case 8:return 8;default:throw Error("mode:"+
a);}else if(27>f)switch(a){case 1:return 12;case 2:return 11;case 4:return 16;case 8:return 10;default:throw Error("mode:"+a);}else if(41>f)switch(a){case 1:return 14;case 2:return 13;case 4:return 16;case 8:return 12;default:throw Error("mode:"+a);}else throw Error("type:"+f);};k.getLostPoint=function(a){for(var b=a.getModuleCount(),h=0,e=0;e<b;e+=1)for(var d=0;d<b;d+=1){for(var c=0,k=a.isDark(e,d),n=-1;1>=n;n+=1)if(!(0>e+n||b<=e+n))for(var r=-1;1>=r;r+=1)0>d+r||b<=d+r||0==n&&0==r||k!=a.isDark(e+
n,d+r)||(c+=1);5<c&&(h+=3+c-5)}for(e=0;e<b-1;e+=1)for(d=0;d<b-1;d+=1)if(c=0,a.isDark(e,d)&&(c+=1),a.isDark(e+1,d)&&(c+=1),a.isDark(e,d+1)&&(c+=1),a.isDark(e+1,d+1)&&(c+=1),0==c||4==c)h+=3;for(e=0;e<b;e+=1)for(d=0;d<b-6;d+=1)a.isDark(e,d)&&!a.isDark(e,d+1)&&a.isDark(e,d+2)&&a.isDark(e,d+3)&&a.isDark(e,d+4)&&!a.isDark(e,d+5)&&a.isDark(e,d+6)&&(h+=40);for(d=0;d<b;d+=1)for(e=0;e<b-6;e+=1)a.isDark(e,d)&&!a.isDark(e+1,d)&&a.isDark(e+2,d)&&a.isDark(e+3,d)&&a.isDark(e+4,d)&&!a.isDark(e+5,d)&&a.isDark(e+6,
d)&&(h+=40);for(d=c=0;d<b;d+=1)for(e=0;e<b;e+=1)a.isDark(e,d)&&(c+=1);return h+Math.abs(100*c/b/b-50)/5*10};return k}(),u=function(){for(var a=Array(256),c=Array(256),n=0;8>n;n+=1)a[n]=1<<n;for(n=8;256>n;n+=1)a[n]=a[n-4]^a[n-5]^a[n-6]^a[n-8];for(n=0;255>n;n+=1)c[a[n]]=n;return{glog:function(a){if(1>a)throw Error("glog("+a+")");return c[a]},gexp:function(b){for(;0>b;)b+=255;for(;256<=b;)b-=255;return a[b]}}}(),A=function(){var a=[[1,26,19],[1,26,16],[1,26,13],[1,26,9],[1,44,34],[1,44,28],[1,44,22],
...
10,55,25],[24,45,15,46,46,16],[4,152,122,18,153,123],[13,74,46,32,75,47],[48,54,24,14,55,25],[42,45,15,32,46,16],[20,147,117,4,148,118],[40,75,47,7,76,48],[43,54,24,22,55,25],[10,45,15,67,46,16],[19,148,118,6,149,119],[18,75,47,31,76,48],[34,54,24,34,55,25],[20,45,15,61,46,16]],c=function(a,b){var e={};e.totalCount=a;e.dataCount=b;return e},n={},b=function(b,c){switch(c){case g.L:return a[4*(b-1)+0];case g.M:return a[4*(b-1)+1];case g.Q:return a[4*(b-1)+2];case g.H:return a[4*(b-1)+3]}};n.getRSBlocks=
function(a,h){var e=b(a,h);if("undefined"==typeof e)throw Error("bad rs block @ typeNumber:"+a+"/errorCorrectLevel:"+h);a=e.length/3;h=[];for(var d=0;d<a;d+=1)for(var k=e[3*d+0],f=e[3*d+1],n=e[3*d+2],g=0;g<k;g+=1)h.push(c(f,n));return h};return n}(),B=function(){var a=[],c=0,n={getBuffer:function(){return a},getAt:function(b){return 1==(a[Math.floor(b/8)]>>>7-b%8&1)},put:function(a,c){for(var b=0;b<c;b+=1)n.putBit(1==(a>>>c-b-1&1))},getLengthInBits:function(){return c},putBit:function(b){var f=Math.floor(c/
8);a.length<=f&&a.push(0);b&&(a[f]|=128>>>c%8);c+=1}};return n},C=function(a){var k=c.stringToBytes(a);return{getMode:function(){return 4},getLength:function(a){return k.length},write:function(a){for(var b=0;b<k.length;b+=1)a.put(k[b],8)}}},z=function(){var a=[],c={writeByte:function(c){a.push(c&255)},writeShort:function(a){c.writeByte(a);c.writeByte(a>>>8)},writeBytes:function(a,b,f){b=b||0;f=f||a.length;for(var h=0;h<f;h+=1)c.writeByte(a[h+b])},writeString:function(a){for(var b=0;b<a.length;b+=
1)c.writeByte(a.charCodeAt(b))},toByteArray:function(){return a},toString:function(){var c;c="[";for(var b=0;b<a.length;b+=1)0<b&&(c+=","),c+=a[b];return c+"]"}};return c},F=function(){var a=0,c=0,g=0,b="",f={},h=function(a){if(!(0>a)){if(26>a)return 65+a;if(52>a)return 97+(a-26);if(62>a)return 48+(a-52);if(62==a)return 43;if(63==a)return 47}throw Error("n:"+a);};f.writeByte=function(e){a=a<<8|e&255;c+=8;for(g+=1;6<=c;)b+=String.fromCharCode(h(a>>>c-6&63)),c-=6};f.flush=function(){0<c&&(b+=String.fromCharCode(h(a<<
6-c&63)),c=a=0);if(0!=g%3)for(var e=3-g%3,d=0;d<e;d+=1)b+="\x3d"};f.toString=function(){return b};return f},E=function(a){var c=0,g=0,b=0,f=function(a){if(65<=a&&90>=a)return a-65;if(97<=a&&122>=a)return a-97+26;if(48<=a&&57>=a)return a-48+52;if(43==a)return 62;if(47==a)return 63;throw Error("c:"+a);};return{read:function(){for(;8>b;){if(c>=a.length){if(0==b)return-1;throw Error("unexpected end of file./"+b);}var h=a.charAt(c);c+=1;if("\x3d"==h)return b=0,-1;h.match(/^\s$/)||(g=g<<6|f(h.charCodeAt(0)),
b+=6)}h=g>>>b-8&255;b-=8;return h}}},G=function(a,c){var g=Array(a*c),b=function(a){var b=0,c=0;return{write:function(d,e){if(0!=d>>>e)throw Error("length over");for(;8<=b+e;)a.writeByte(255&(d<<b|c)),e-=8-b,d>>>=8-b,b=c=0;c|=d<<b;b+=e},flush:function(){0<b&&a.writeByte(c)}}},f=function(){var a={},b=0,c={add:function(d){if(c.contains(d))throw Error("dup key:"+d);a[d]=b;b+=1},size:function(){return b},indexOf:function(b){return a[b]},contains:function(b){return"undefined"!=typeof a[b]}};return c};
return{setPixel:function(b,c,d){g[c*a+b]=d},write:function(h){h.writeString("GIF87a");h.writeShort(a);h.writeShort(c);h.writeByte(128);h.writeByte(0);h.writeByte(0);h.writeByte(0);h.writeByte(0);h.writeByte(0);h.writeByte(255);h.writeByte(255);h.writeByte(255);h.writeString(",");h.writeShort(0);h.writeShort(0);h.writeShort(a);h.writeShort(c);h.writeByte(0);var e;e=3;for(var d=f(),k=0;4>k;k+=1)d.add(String.fromCharCode(k));d.add(String.fromCharCode(4));d.add(String.fromCharCode(5));var k=z(),n=b(k);
n.write(4,e);for(var p=0,r=String.fromCharCode(g[p]),p=p+1;p<g.length;){var l=String.fromCharCode(g[p]),p=p+1;d.contains(r+l)?r+=l:(n.write(d.indexOf(r),e),4095>d.size()&&(d.size()==1<<e&&(e+=1),d.add(r+l)),r=l)}n.write(d.indexOf(r),e);n.write(5,e);n.flush();e=k.toByteArray();h.writeByte(2);for(d=0;255<e.length-d;)h.writeByte(255),h.writeBytes(e,d,255),d+=255;h.writeByte(e.length-d);h.writeBytes(e,d,e.length-d);h.writeByte(0);h.writeString(";")}}},D=function(a,c,g,b){for(var f=G(a,c),h=0;h<c;h+=1)for(var e=
0;e<a;e+=1)f.setPixel(e,h,g(e,h));g=z();f.write(g);f=F();g=g.toByteArray();for(h=0;h<g.length;h+=1)f.writeByte(g[h]);f.flush();f='\x3cimg src\x3d"data:image/gif;base64,'+f+'" width\x3d"';f+=a;f+='"';f+=' height\x3d"';f+=c;f+='"';b&&(f+=' alt\x3d"',f+=b,f+='"');return f+="/\x3e"};return c}();(function(a){"function"===typeof define&&define.amd?define([],a):"object"===typeof exports&&(module.exports=a())})(function(){return qrcode});