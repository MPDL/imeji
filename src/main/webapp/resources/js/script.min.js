var datasourceUrl,result,servlet,offset=2,id;function getDatasourceUrl(a,g){datasourceUrl=a;this.servlet=g;offset=2}function getDatasourceUrlWithFixedDelay(a,g,c){getDatasourceUrl(a,g);offset=c}function split(a){return a.split(/,\s*/)}function extractLast(a){return split(a).pop()}function setInputValue(a,g){null!=document.getElementById(a)&&(null!=g?document.getElementById(a).value=g:""==document.getElementById(a).value)}
$(function(){$(".autocomplete_js").bind("keydown",function(a){null!=datasourceUrl&&""!=datasourceUrl&&a.keyCode===$.ui.keyCode.TAB&&$(this).data("autocomplete").menu.active&&a.getPreventDefault()}).autocomplete({source:function(a,g){$.getJSON(servlet,{searchkeyword:a.term,datasource:datasourceUrl},function(a){result=a;g(result)})},minLength:0,messages:{noResults:"",results:function(){return""}},search:function(){if(null==datasourceUrl||""==datasourceUrl||extractLast(this.value).length<offset)return!1},
focus:function(){return!1},select:function(a,g){a=this.id.split(":");for(var c="",v=0;v<a.length-1;v++)c=c+a[v]+":";setInputValue(this.id,g.item.value);setInputValue(c+"identifier",g.item.id);setInputValue(c+"given",g.item.givenname);setInputValue(c+"family",g.item.family);setInputValue(c+"identifier",g.item.id);setInputValue(c+"organization:0:name",g.item.organization);setInputValue(c+"latitude",g.item.latitude);setInputValue(c+"longitude",g.item.longitude);return!1}}).focus(function(){0==offset&&
null!=datasourceUrl&&""!=datasourceUrl&&(this.value=" ",$(this).autocomplete("search"),this.value="",event.getPreventDefault())})});function endsWith(a,g){return-1!==a.indexOf(g,a.length-g.length)}function formatNumber(a){return a.replace(",",".").replace(" ","")}function isNumber(a){a=formatNumber(a);return!isNaN(parseFloat(a))&&isFinite(a)}
function sortOptionsByText(){var a,g;if(a=jQuery('select[data-sort\x3d"sort"]'))g=jQuery(a.find("option")),g=jQuery.makeArray(g).sort(function(a,g){return a.innerHTML>g.innerHTML?1:-1}),a.html(g)}function loadContent(a,g,c){$.ajax({type:"GET",url:a,cache:!1,success:function(a){g&&$(g).html(a);c&&setTimeout(c,15,g,a)}})}function validateInputNumber(a){a.value=formatNumber(a.value);isNumber(a.value)||(a.value="")}
function openDialog(a){a=$(document.getElementById(a));a.css("left",Math.max(0,Math.round(($(window).width()-$(a).outerWidth())/2)+$(window).scrollLeft())+"px");a.show();$(".imj_modalDialogBackground").show()}function closeDialog(a){a=$(document.getElementById(a));$(".imj_modalDialogBackground").hide();a.hide()}$(window).resize(function(a){a=$(".imj_modalDialogBox:visible");0<a.length&&a.css("left",Math.max(0,Math.round(($(window).width()-$(a).outerWidth())/2)+$(window).scrollLeft())+"px")});
var swcObject={};
function initSWC(a){swcObject.data=$(a).text();swcObject.json=swc_parser(swcObject.data);a=document.createElement("canvas");window.WebGLRenderingContext&&(a.getContext("webgl")||a.getContext("experimental-webgl"))?(a=$("*[id*\x3d"+swcObject.placeholderID+"]"),a.get(0).style.display="none",a=new SharkViewer({swc:swcObject.json,dom_element:swcObject.displayID,WIDTH:swcObject.width,HEIGHT:swcObject.height,center_node:-1,show_stats:!1,screenshot:!1}),a.init(),a.animate()):document.getElementById(swcObject.failedMsgID).style.display="block"}
function loadSWC(a,g){var c;c=$(a);swcObject={domSource:a,dataURL:c.data("swc-source")||void 0,serviceURL:c.data("swc-service")||void 0,elementID:g,displayID:"#"===c.data("target-id")[0]?c.data("target-id").substring(1):c.data("target-id"),width:c.data("target-width"),height:c.data("target-height"),placeholderID:"#"===c.data("placeholder-id")[0]?c.data("placeholder-id").substring(1):c.data("placeholder-id"),failedMsgID:"#"===c.data("failed-msg-id")[0]?c.data("failed-msg-id").substring(1):c.data("failed-msg-id")};
initSWC("#"+swcObject.elementID)}function handleDisableButton(a){if("submit"===a.source.type)switch(a.status){case "begin":a.source.disabled=!0;break;case "complete":a.source.disabled=!1}}"undefined"!==typeof jsf&&jsf.ajax.addOnEvent(function(a){"success"===a.status&&fixViewState(a.responseXML);handleDisableButton(a)});
function fixViewState(a){if(a=getViewState(a))for(var g=0;g<document.forms.length;g++){var c=document.forms[g];"post"===c.method.toLowerCase()?hasViewState(c)||createViewState(c,a):removeViewState(c)}}function getViewState(a){a=a.getElementsByTagName("update");for(var g=0;g<a.length;g++){var c=a[g];if(c.getAttribute("id").match(/^([\w]+:)?javax\.faces\.ViewState(:[0-9]+)?$/))return c.firstChild.nodeValue}return null}
function hasViewState(a){for(var g=0;g<a.elements.length;g++)if("javax.faces.ViewState"==a.elements[g].name)return!0;return!1}function createViewState(a,g){var c;try{c=document.createElement("\x3cinput name\x3d'javax.faces.ViewState'\x3e")}catch(v){c=document.createElement("input"),c.setAttribute("name","javax.faces.ViewState")}c.setAttribute("type","hidden");c.setAttribute("value",g);c.setAttribute("autocomplete","off");a.appendChild(c)}
function removeViewState(a){for(var g=0;g<a.elements.length;g++){var c=a.elements[g];"javax.faces.ViewState"==c.name&&c.parentNode.removeChild(c)}}var selectedSearch,numberOfContext=$(".imj_bodyContextSearch li").length;function submitSimpleSearch(){""!=$("#simpleSearchInputText").val()&&goToSearch(selectedSearch);return!1}$(".imj_bodyContextSearch li").click(function(){goToSearch($(this).index()+1)});
function goToSearch(a){var g="?";a=$(".imj_bodyContextSearch li:nth-child("+a+")").data("url");0<=a.indexOf("?")&&(g="\x26");window.open(a+g+"q\x3d"+encodeURIComponent($("#simpleSearchInputText").val()),"_self")}$("#simpleSearchInput").focusin(function(){$(".imj_menuSimpleSearch").show()}).keyup(function(a){40==a.which?(incrementSelectedSearch(),highlightSearch()):38==a.which?(decrementSelectedSearch(),highlightSearch()):""!=$(this).val()&&$(".imj_menuSimpleSearch").show()});
$(document).ready(function(){selectedSearch=1;var a=window.location.pathname;$("ul.imj_bodyContextSearch li").each(function(g){if(-1!==$(this).data("url").indexOf(a))return selectedSearch=g+1,!1});highlightSearch()});function changePlaceholder(){var a=$("ul.imj_bodyContextSearch li:nth-child("+selectedSearch+")").data("placeholder");$("#simpleSearchInputText").attr("placeholder",a)}$(".imj_simpleSearch").focusout(function(){$(".imj_menuSimpleSearch").delay(200).hide(0)});$("ul.imj_bodyContextSearch li").mouseover(function(){});
$("ul.imj_bodyContextSearch li").mouseout(function(){highlightSearch()});function highlightSearch(){$("ul.imj_bodyContextSearch li").removeClass("hovered");$("ul.imj_bodyContextSearch li:nth-child("+selectedSearch+")").addClass("hovered");changePlaceholder()}function incrementSelectedSearch(){selectedSearch<numberOfContext&&(selectedSearch+=1)}function decrementSelectedSearch(){1<selectedSearch&&--selectedSearch}function lockPage(){$(".loaderWrapper").show()}
function unlockPage(){$(".loaderWrapper").hide()}function startLoader(){$(".loaderWrapper").show();$(".loader").show()}function stopLoader(){$(".loaderWrapper").hide();$(".loader").hide()}jsf.ajax.addOnEvent(function(a){switch(a.status){case "complete":stopLoader()}});$(function(){$(".scrollbarTopWrapper").scroll(function(){$(".edit_selected_table_wrapper").scrollLeft($(".scrollbarTopWrapper").scrollLeft())});$(".edit_selected_table_wrapper").scroll(function(){$(".scrollbarTopWrapper").scrollLeft($(".edit_selected_table_wrapper").scrollLeft())})});
setTimeout(function(){$(".imj_messageSuccess").slideUp(200)},4E3);function closeSuccessMessage(){setTimeout(function(){$(".imj_messageSuccess").slideUp(200)},4E3)}$(document).ready(function(){var a=0,g=0;$(".imj_tileItem").each(function(){var c=$(this).position().top;if(0<g&&c>g)return a=$(".imj_tileItem").index(this),!1;g=c});var c=new Date;c.setTime(+c+31536E6);0<a&&(document.cookie="ELEMENTS_PER_LINE_COOKIE\x3d"+a+"; expires\x3d"+c.toGMTString()+"; path\x3d/")});
function getElementsPerLineCookie(){return document.cookie.replace(/(?:(?:^|.*;\s*)ELEMENTS_PER_LINE_COOKIE\s*\=\s*([^;]*).*$)|^.*$/,"$1")}$(function(){$(".datepicker").datepicker({changeMonth:!0,changeYear:!0,dateFormat:"yy-mm-dd",firstDay:1})});
qrcode&&(qrcode.stringToBytes=qrcode.createStringToBytes("\x3d",7070));
var qrcode=function(){function a(r,g){if("undefined"==typeof r.length)throw Error(r.length+"/"+g);var k=function(){for(var f=0;f<r.length&&0==r[f];)f+=1;for(var b=Array(r.length-f+g),a=0;a<r.length-f;a+=1)b[a]=r[a+f];return b}(),b={getAt:function(f){return k[f]},getLength:function(){return k.length},multiply:function(f){for(var h=Array(b.getLength()+f.getLength()-1),e=0;e<b.getLength();e+=1)for(var d=0;d<f.getLength();d+=1)h[e+d]^=u.gexp(u.glog(b.getAt(e))+u.glog(f.getAt(d)));return a(h,0)},mod:function(f){if(0>
b.getLength()-f.getLength())return b;for(var h=u.glog(b.getAt(0))-u.glog(f.getAt(0)),e=Array(b.getLength()),d=0;d<b.getLength();d+=1)e[d]=b.getAt(d);for(d=0;d<f.getLength();d+=1)e[d]^=u.gexp(u.glog(f.getAt(d))+h);return a(e,0).mod(f)}};return b}var g=function(g,q){var k=c[q],b=null,f=0,h=null,e=[],d={},r=function(d,r){for(var l=f=4*g+17,m=Array(l),c=0;c<l;c+=1){m[c]=Array(l);for(var q=0;q<l;q+=1)m[c][q]=null}b=m;y(0,0);y(f-7,0);y(0,f-7);l=v.getPatternPosition(g);for(m=0;m<l.length;m+=1)for(c=0;c<
l.length;c+=1){var q=l[m],t=l[c];if(null==b[q][t])for(var n=-2;2>=n;n+=1)for(var p=-2;2>=p;p+=1)b[q+n][t+p]=-2==n||2==n||-2==p||2==p||0==n&&0==p?!0:!1}for(l=8;l<f-8;l+=1)null==b[l][6]&&(b[l][6]=0==l%2);for(l=8;l<f-8;l+=1)null==b[6][l]&&(b[6][l]=0==l%2);l=v.getBCHTypeInfo(k<<3|r);for(m=0;15>m;m+=1)c=!d&&1==(l>>m&1),6>m?b[m][8]=c:8>m?b[m+1][8]=c:b[f-15+m][8]=c;for(m=0;15>m;m+=1)c=!d&&1==(l>>m&1),8>m?b[8][f-m-1]=c:9>m?b[8][15-m-1+1]=c:b[8][15-m-1]=c;b[f-8][8]=!d;if(7<=g){l=v.getBCHTypeNumber(g);for(m=
0;18>m;m+=1)c=!d&&1==(l>>m&1),b[Math.floor(m/3)][m%3+f-8-3]=c;for(m=0;18>m;m+=1)c=!d&&1==(l>>m&1),b[m%3+f-8-3][Math.floor(m/3)]=c}if(null==h){d=A.getRSBlocks(g,k);l=B();for(m=0;m<e.length;m+=1)c=e[m],l.put(c.getMode(),4),l.put(c.getLength(),v.getLengthInBits(c.getMode(),g)),c.write(l);for(m=c=0;m<d.length;m+=1)c+=d[m].dataCount;if(l.getLengthInBits()>8*c)throw Error("code length overflow. ("+l.getLengthInBits()+"\x3e"+8*c+")");for(l.getLengthInBits()+4<=8*c&&l.put(0,4);0!=l.getLengthInBits()%8;)l.putBit(!1);
for(;!(l.getLengthInBits()>=8*c);){l.put(236,8);if(l.getLengthInBits()>=8*c)break;l.put(17,8)}for(var w=0,c=m=0,q=Array(d.length),t=Array(d.length),n=0;n<d.length;n+=1){var u=d[n].dataCount,x=d[n].totalCount-u,m=Math.max(m,u),c=Math.max(c,x);q[n]=Array(u);for(p=0;p<q[n].length;p+=1)q[n][p]=255&l.getBuffer()[p+w];w+=u;p=v.getErrorCorrectPolynomial(x);u=a(q[n],p.getLength()-1).mod(p);t[n]=Array(p.getLength()-1);for(p=0;p<t[n].length;p+=1)x=p+u.getLength()-t[n].length,t[n][p]=0<=x?u.getAt(x):0}for(p=
l=0;p<d.length;p+=1)l+=d[p].totalCount;l=Array(l);for(p=w=0;p<m;p+=1)for(n=0;n<d.length;n+=1)p<q[n].length&&(l[w]=q[n][p],w+=1);for(p=0;p<c;p+=1)for(n=0;n<d.length;n+=1)p<t[n].length&&(l[w]=t[n][p],w+=1);h=l}d=h;l=-1;m=f-1;c=7;q=0;r=v.getMaskFunction(r);for(t=f-1;0<t;t-=2)for(6==t&&--t;;){for(n=0;2>n;n+=1)null==b[m][t-n]&&(p=!1,q<d.length&&(p=1==(d[q]>>>c&1)),r(m,t-n)&&(p=!p),b[m][t-n]=p,--c,-1==c&&(q+=1,c=7));m+=l;if(0>m||f<=m){m-=l;l=-l;break}}},y=function(a,h){for(var d=-1;7>=d;d+=1)if(!(-1>=a+
d||f<=a+d))for(var e=-1;7>=e;e+=1)-1>=h+e||f<=h+e||(b[a+d][h+e]=0<=d&&6>=d&&(0==e||6==e)||0<=e&&6>=e&&(0==d||6==d)||2<=d&&4>=d&&2<=e&&4>=e?!0:!1)};d.addData=function(f){f=C(f);e.push(f);h=null};d.isDark=function(d,a){if(0>d||f<=d||0>a||f<=a)throw Error(d+","+a);return b[d][a]};d.getModuleCount=function(){return f};d.make=function(){for(var f=0,a=0,b=0;8>b;b+=1){r(!0,b);var e=v.getLostPoint(d);if(0==b||f>e)f=e,a=b}r(!1,a)};d.createTableTag=function(f,b){f=f||2;b='\x3ctable style\x3d" border-width: 0px; border-style: none; border-collapse: collapse;'+
(" padding: 0px; margin: "+("undefined"==typeof b?4*f:b)+"px;")+'"\x3e\x3ctbody\x3e';for(var a=0;a<d.getModuleCount();a+=1){b+="\x3ctr\x3e";for(var e=0;e<d.getModuleCount();e+=1)b+='\x3ctd style\x3d"',b+=" border-width: 0px; border-style: none;",b+=" border-collapse: collapse;",b+=" padding: 0px; margin: 0px;",b+=" width: "+f+"px;",b+=" height: "+f+"px;",b+=" background-color: ",b+=d.isDark(a,e)?"#000000":"#ffffff",b+=";",b+='"/\x3e';b+="\x3c/tr\x3e"}return b+"\x3c/tbody\x3e\x3c/table\x3e"};d.createImgTag=
function(f,b){f=f||2;b="undefined"==typeof b?4*f:b;var a=d.getModuleCount()*f+2*b,e=b,h=a-b;return D(a,a,function(b,a){return e<=b&&b<h&&e<=a&&a<h?d.isDark(Math.floor((a-e)/f),Math.floor((b-e)/f))?0:1:1})};return d};g.stringToBytes=function(a){for(var c=[],k=0;k<a.length;k+=1){var b=a.charCodeAt(k);c.push(b&255)}return c};g.createStringToBytes=function(a,c){var k=function(){for(var b=E(a),f=function(){var f=b.read();if(-1==f)throw Error();return f},h=0,e={};;){var d=b.read();if(-1==d)break;var k=
f(),g=f(),q=f(),d=String.fromCharCode(d<<8|k);e[d]=g<<8|q;h+=1}if(h!=c)throw Error(h+" !\x3d "+c);return e}();return function(b){for(var f=[],a=0;a<b.length;a+=1){var e=b.charCodeAt(a);128>e?f.push(e):(e=k[b.charAt(a)],"number"==typeof e?(e&255)==e?f.push(e):(f.push(e>>>8),f.push(e&255)):f.push(63))}return f}};var c={L:1,M:0,Q:3,H:2},v=function(){var c=[[],[6,18],[6,22],[6,26],[6,30],[6,34],[6,22,38],[6,24,42],[6,26,46],[6,28,50],[6,30,54],[6,32,58],[6,34,62],[6,26,46,66],[6,26,48,70],[6,26,50,74],
[6,30,54,78],[6,30,56,82],[6,30,58,86],[6,34,62,90],[6,28,50,72,94],[6,26,50,74,98],[6,30,54,78,102],[6,28,54,80,106],[6,32,58,84,110],[6,30,58,86,114],[6,34,62,90,118],[6,26,50,74,98,122],[6,30,54,78,102,126],[6,26,52,78,104,130],[6,30,56,82,108,134],[6,34,60,86,112,138],[6,30,58,86,114,142],[6,34,62,90,118,146],[6,30,54,78,102,126,150],[6,24,50,76,102,128,154],[6,28,54,80,106,132,158],[6,32,58,84,110,136,162],[6,26,54,82,110,138,166],[6,30,58,86,114,142,170]],g={},k=function(b){for(var f=0;0!=b;)f+=
1,b>>>=1;return f};g.getBCHTypeInfo=function(b){for(var f=b<<10;0<=k(f)-k(1335);)f^=1335<<k(f)-k(1335);return(b<<10|f)^21522};g.getBCHTypeNumber=function(b){for(var f=b<<12;0<=k(f)-k(7973);)f^=7973<<k(f)-k(7973);return b<<12|f};g.getPatternPosition=function(b){return c[b-1]};g.getMaskFunction=function(b){switch(b){case 0:return function(b,a){return 0==(b+a)%2};case 1:return function(b,a){return 0==b%2};case 2:return function(b,a){return 0==a%3};case 3:return function(b,a){return 0==(b+a)%3};case 4:return function(b,
a){return 0==(Math.floor(b/2)+Math.floor(a/3))%2};case 5:return function(b,a){return 0==b*a%2+b*a%3};case 6:return function(b,a){return 0==(b*a%2+b*a%3)%2};case 7:return function(b,a){return 0==(b*a%3+(b+a)%2)%2};default:throw Error("bad maskPattern:"+b);}};g.getErrorCorrectPolynomial=function(b){for(var f=a([1],0),h=0;h<b;h+=1)f=f.multiply(a([1,u.gexp(h)],0));return f};g.getLengthInBits=function(b,a){if(1<=a&&10>a)switch(b){case 1:return 10;case 2:return 9;case 4:return 8;case 8:return 8;default:throw Error("mode:"+
b);}else if(27>a)switch(b){case 1:return 12;case 2:return 11;case 4:return 16;case 8:return 10;default:throw Error("mode:"+b);}else if(41>a)switch(b){case 1:return 14;case 2:return 13;case 4:return 16;case 8:return 12;default:throw Error("mode:"+b);}else throw Error("type:"+a);};g.getLostPoint=function(b){for(var a=b.getModuleCount(),h=0,e=0;e<a;e+=1)for(var d=0;d<a;d+=1){for(var c=0,g=b.isDark(e,d),k=-1;1>=k;k+=1)if(!(0>e+k||a<=e+k))for(var r=-1;1>=r;r+=1)0>d+r||a<=d+r||0==k&&0==r||g!=b.isDark(e+
k,d+r)||(c+=1);5<c&&(h+=3+c-5)}for(e=0;e<a-1;e+=1)for(d=0;d<a-1;d+=1)if(c=0,b.isDark(e,d)&&(c+=1),b.isDark(e+1,d)&&(c+=1),b.isDark(e,d+1)&&(c+=1),b.isDark(e+1,d+1)&&(c+=1),0==c||4==c)h+=3;for(e=0;e<a;e+=1)for(d=0;d<a-6;d+=1)b.isDark(e,d)&&!b.isDark(e,d+1)&&b.isDark(e,d+2)&&b.isDark(e,d+3)&&b.isDark(e,d+4)&&!b.isDark(e,d+5)&&b.isDark(e,d+6)&&(h+=40);for(d=0;d<a;d+=1)for(e=0;e<a-6;e+=1)b.isDark(e,d)&&!b.isDark(e+1,d)&&b.isDark(e+2,d)&&b.isDark(e+3,d)&&b.isDark(e+4,d)&&!b.isDark(e+5,d)&&b.isDark(e+6,
d)&&(h+=40);for(d=c=0;d<a;d+=1)for(e=0;e<a;e+=1)b.isDark(e,d)&&(c+=1);return h+Math.abs(100*c/a/a-50)/5*10};return g}(),u=function(){for(var a=Array(256),c=Array(256),k=0;8>k;k+=1)a[k]=1<<k;for(k=8;256>k;k+=1)a[k]=a[k-4]^a[k-5]^a[k-6]^a[k-8];for(k=0;255>k;k+=1)c[a[k]]=k;return{glog:function(a){if(1>a)throw Error("glog("+a+")");return c[a]},gexp:function(b){for(;0>b;)b+=255;for(;256<=b;)b-=255;return a[b]}}}(),A=function(){var a=[[1,26,19],[1,26,16],[1,26,13],[1,26,9],[1,44,34],[1,44,28],[1,44,22],
[1,44,16],[1,70,55],[1,70,44],[2,35,17],[2,35,13],[1,100,80],[2,50,32],[2,50,24],[4,25,9],[1,134,108],[2,67,43],[2,33,15,2,34,16],[2,33,11,2,34,12],[2,86,68],[4,43,27],[4,43,19],[4,43,15],[2,98,78],[4,49,31],[2,32,14,4,33,15],[4,39,13,1,40,14],[2,121,97],[2,60,38,2,61,39],[4,40,18,2,41,19],[4,40,14,2,41,15],[2,146,116],[3,58,36,2,59,37],[4,36,16,4,37,17],[4,36,12,4,37,13],[2,86,68,2,87,69],[4,69,43,1,70,44],[6,43,19,2,44,20],[6,43,15,2,44,16],[4,101,81],[1,80,50,4,81,51],[4,50,22,4,51,23],[3,36,12,
8,37,13],[2,116,92,2,117,93],[6,58,36,2,59,37],[4,46,20,6,47,21],[7,42,14,4,43,15],[4,133,107],[8,59,37,1,60,38],[8,44,20,4,45,21],[12,33,11,4,34,12],[3,145,115,1,146,116],[4,64,40,5,65,41],[11,36,16,5,37,17],[11,36,12,5,37,13],[5,109,87,1,110,88],[5,65,41,5,66,42],[5,54,24,7,55,25],[11,36,12,7,37,13],[5,122,98,1,123,99],[7,73,45,3,74,46],[15,43,19,2,44,20],[3,45,15,13,46,16],[1,135,107,5,136,108],[10,74,46,1,75,47],[1,50,22,15,51,23],[2,42,14,17,43,15],[5,150,120,1,151,121],[9,69,43,4,70,44],[17,
50,22,1,51,23],[2,42,14,19,43,15],[3,141,113,4,142,114],[3,70,44,11,71,45],[17,47,21,4,48,22],[9,39,13,16,40,14],[3,135,107,5,136,108],[3,67,41,13,68,42],[15,54,24,5,55,25],[15,43,15,10,44,16],[4,144,116,4,145,117],[17,68,42],[17,50,22,6,51,23],[19,46,16,6,47,17],[2,139,111,7,140,112],[17,74,46],[7,54,24,16,55,25],[34,37,13],[4,151,121,5,152,122],[4,75,47,14,76,48],[11,54,24,14,55,25],[16,45,15,14,46,16],[6,147,117,4,148,118],[6,73,45,14,74,46],[11,54,24,16,55,25],[30,46,16,2,47,17],[8,132,106,4,
133,107],[8,75,47,13,76,48],[7,54,24,22,55,25],[22,45,15,13,46,16],[10,142,114,2,143,115],[19,74,46,4,75,47],[28,50,22,6,51,23],[33,46,16,4,47,17],[8,152,122,4,153,123],[22,73,45,3,74,46],[8,53,23,26,54,24],[12,45,15,28,46,16],[3,147,117,10,148,118],[3,73,45,23,74,46],[4,54,24,31,55,25],[11,45,15,31,46,16],[7,146,116,7,147,117],[21,73,45,7,74,46],[1,53,23,37,54,24],[19,45,15,26,46,16],[5,145,115,10,146,116],[19,75,47,10,76,48],[15,54,24,25,55,25],[23,45,15,25,46,16],[13,145,115,3,146,116],[2,74,46,
29,75,47],[42,54,24,1,55,25],[23,45,15,28,46,16],[17,145,115],[10,74,46,23,75,47],[10,54,24,35,55,25],[19,45,15,35,46,16],[17,145,115,1,146,116],[14,74,46,21,75,47],[29,54,24,19,55,25],[11,45,15,46,46,16],[13,145,115,6,146,116],[14,74,46,23,75,47],[44,54,24,7,55,25],[59,46,16,1,47,17],[12,151,121,7,152,122],[12,75,47,26,76,48],[39,54,24,14,55,25],[22,45,15,41,46,16],[6,151,121,14,152,122],[6,75,47,34,76,48],[46,54,24,10,55,25],[2,45,15,64,46,16],[17,152,122,4,153,123],[29,74,46,14,75,47],[49,54,24,
10,55,25],[24,45,15,46,46,16],[4,152,122,18,153,123],[13,74,46,32,75,47],[48,54,24,14,55,25],[42,45,15,32,46,16],[20,147,117,4,148,118],[40,75,47,7,76,48],[43,54,24,22,55,25],[10,45,15,67,46,16],[19,148,118,6,149,119],[18,75,47,31,76,48],[34,54,24,34,55,25],[20,45,15,61,46,16]],g=function(a,b){var e={};e.totalCount=a;e.dataCount=b;return e},k={},b=function(b,h){switch(h){case c.L:return a[4*(b-1)+0];case c.M:return a[4*(b-1)+1];case c.Q:return a[4*(b-1)+2];case c.H:return a[4*(b-1)+3]}};k.getRSBlocks=
function(a,c){var e=b(a,c);if("undefined"==typeof e)throw Error("bad rs block @ typeNumber:"+a+"/errorCorrectLevel:"+c);a=e.length/3;c=[];for(var d=0;d<a;d+=1)for(var f=e[3*d+0],h=e[3*d+1],k=e[3*d+2],q=0;q<f;q+=1)c.push(g(h,k));return c};return k}(),B=function(){var a=[],c=0,k={getBuffer:function(){return a},getAt:function(b){return 1==(a[Math.floor(b/8)]>>>7-b%8&1)},put:function(a,c){for(var b=0;b<c;b+=1)k.putBit(1==(a>>>c-b-1&1))},getLengthInBits:function(){return c},putBit:function(b){var f=Math.floor(c/
8);a.length<=f&&a.push(0);b&&(a[f]|=128>>>c%8);c+=1}};return k},C=function(a){var c=g.stringToBytes(a);return{getMode:function(){return 4},getLength:function(a){return c.length},write:function(a){for(var b=0;b<c.length;b+=1)a.put(c[b],8)}}},z=function(){var a=[],c={writeByte:function(c){a.push(c&255)},writeShort:function(a){c.writeByte(a);c.writeByte(a>>>8)},writeBytes:function(a,b,f){b=b||0;f=f||a.length;for(var h=0;h<f;h+=1)c.writeByte(a[h+b])},writeString:function(a){for(var b=0;b<a.length;b+=
1)c.writeByte(a.charCodeAt(b))},toByteArray:function(){return a},toString:function(){var c;c="[";for(var b=0;b<a.length;b+=1)0<b&&(c+=","),c+=a[b];return c+"]"}};return c},F=function(){var a=0,c=0,g=0,b="",f={},h=function(a){if(!(0>a)){if(26>a)return 65+a;if(52>a)return 97+(a-26);if(62>a)return 48+(a-52);if(62==a)return 43;if(63==a)return 47}throw Error("n:"+a);};f.writeByte=function(e){a=a<<8|e&255;c+=8;for(g+=1;6<=c;)b+=String.fromCharCode(h(a>>>c-6&63)),c-=6};f.flush=function(){0<c&&(b+=String.fromCharCode(h(a<<
6-c&63)),c=a=0);if(0!=g%3)for(var e=3-g%3,d=0;d<e;d+=1)b+="\x3d"};f.toString=function(){return b};return f},E=function(a){var c=0,g=0,b=0,f=function(a){if(65<=a&&90>=a)return a-65;if(97<=a&&122>=a)return a-97+26;if(48<=a&&57>=a)return a-48+52;if(43==a)return 62;if(47==a)return 63;throw Error("c:"+a);};return{read:function(){for(;8>b;){if(c>=a.length){if(0==b)return-1;throw Error("unexpected end of file./"+b);}var h=a.charAt(c);c+=1;if("\x3d"==h)return b=0,-1;h.match(/^\s$/)||(g=g<<6|f(h.charCodeAt(0)),
b+=6)}h=g>>>b-8&255;b-=8;return h}}},G=function(a,c){var g=Array(a*c),b=function(a){var b=0,c=0;return{write:function(d,e){if(0!=d>>>e)throw Error("length over");for(;8<=b+e;)a.writeByte(255&(d<<b|c)),e-=8-b,d>>>=8-b,b=c=0;c|=d<<b;b+=e},flush:function(){0<b&&a.writeByte(c)}}},f=function(){var a={},b=0,c={add:function(d){if(c.contains(d))throw Error("dup key:"+d);a[d]=b;b+=1},size:function(){return b},indexOf:function(b){return a[b]},contains:function(b){return"undefined"!=typeof a[b]}};return c};
return{setPixel:function(b,c,d){g[c*a+b]=d},write:function(h){h.writeString("GIF87a");h.writeShort(a);h.writeShort(c);h.writeByte(128);h.writeByte(0);h.writeByte(0);h.writeByte(0);h.writeByte(0);h.writeByte(0);h.writeByte(255);h.writeByte(255);h.writeByte(255);h.writeString(",");h.writeShort(0);h.writeShort(0);h.writeShort(a);h.writeShort(c);h.writeByte(0);var e;e=3;for(var d=f(),k=0;4>k;k+=1)d.add(String.fromCharCode(k));d.add(String.fromCharCode(4));d.add(String.fromCharCode(5));var k=z(),q=b(k);
q.write(4,e);for(var n=0,r=String.fromCharCode(g[n]),n=n+1;n<g.length;){var l=String.fromCharCode(g[n]),n=n+1;d.contains(r+l)?r+=l:(q.write(d.indexOf(r),e),4095>d.size()&&(d.size()==1<<e&&(e+=1),d.add(r+l)),r=l)}q.write(d.indexOf(r),e);q.write(5,e);q.flush();e=k.toByteArray();h.writeByte(2);for(d=0;255<e.length-d;)h.writeByte(255),h.writeBytes(e,d,255),d+=255;h.writeByte(e.length-d);h.writeBytes(e,d,e.length-d);h.writeByte(0);h.writeString(";")}}},D=function(a,c,g,b){for(var f=G(a,c),h=0;h<c;h+=1)for(var e=
0;e<a;e+=1)f.setPixel(e,h,g(e,h));g=z();f.write(g);f=F();g=g.toByteArray();for(h=0;h<g.length;h+=1)f.writeByte(g[h]);f.flush();f='\x3cimg src\x3d"data:image/gif;base64,'+f+'" width\x3d"';f+=a;f+='"';f+=' height\x3d"';f+=c;f+='"';b&&(f+=' alt\x3d"',f+=b,f+='"');return f+="/\x3e"};return g}();(function(a){"function"===typeof define&&define.amd?define([],a):"object"===typeof exports&&(module.exports=a())})(function(){return qrcode});