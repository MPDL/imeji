 ...
 * been changed since it was last read by that same process. This can happen when another process
 * (user session) updates the object after it was read from database by our process and before it is
 * written back. Context: Multiple processes (users) access shared data resources (i.e. collection,
 * item).
 * 
...
public class ReloadBeforeSaveException extends ImejiExceptionWithUserMessage {
  ...
  /**
   * User readable name of the object that needs to be reloaded for GUI error message
   */
