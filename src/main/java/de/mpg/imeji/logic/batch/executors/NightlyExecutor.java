package de.mpg.imeji.logic.batch.executors;
...
public class NightlyExecutor {
  ...
  public void start() {
    ...
    LOGGER.info(
        "Nightly Executor started. First job will start in " + getDelay() + " hours (around: " + NEXT_JOB_DATE.getTime().toString() + ")");
  ...
  private static Calendar scheduleNextDate() {
    ...
    date.set(date.get(Calendar.YEAR), date.get(Calendar.MONTH), date.get(Calendar.DATE), JOB_HOUR, JOB_MINUTE);
    // Add one day
