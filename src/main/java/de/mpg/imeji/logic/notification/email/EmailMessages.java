...
public class EmailMessages {
	...
	/**
	 * Denotes whether a text for sending an email is a subject text or a message
	 * text
	 * 
	 * @author breddin
	 *
	...
	/**
	 * Read a message text from an .xml file in the file system (for example
	 * emailMessages_en.xml)
	 * 
	 * @param identifier
	 *            the identifier of the text (i.e. )
	 * @param textType
	 *            is the text a email subject or an email message
	 * @param locale
	 *            the user's language
	 * @return
	 */
	...
	/**
	 * Reads a text (GUI message) from a .properties file in the .war distribution
	...
	public static String getNewPasswordMessage(String password, String email, String username, Locale locale) {
		...
		try {
			...
			return getEmailOnAccountAction_Body(password, email, username, "email_new_password", TextType.MESSAGE,
					locale).replace("XXX_INSTANCE_NAME_XXX", instanceName);
	...
	/**
	 * Create the subject of the email being send, for either new account or new
	 * password
	 *
	 * @param newAccount
	 * @return
	...
	/**
	 * Email content when a collection has been shared with the addressee by the
	 * sender
	 *
	 * @param sender
	 * @param dest
	 ...
	 * @return
	 */
	public static String getEmailOnZipDownload_Body(User to, User actor, String itemsDownloaded, String url,
			Locale locale) {
		return getContent("email_zip_images_downloaded", TextType.MESSAGE, locale)
				...
				.replace("XXX_ACTOR_NAME_XXX",
						(actor != null ? actor.getPerson().getCompleteName() : "non_logged_in_user"))
				.replace("XXX_ACTOR_EMAIL_XXX", (actor != null ? actor.getEmail() : ""))
				.replace("XXX_TIME_XXX", new Date().toString()).replace("XXX_ITEMS_DOWNLOADED_XXX", itemsDownloaded)
	 ...
	 * @return
	 */
	public static String getUnshareMessage(String sender, String dest, String title, String collectionLink,
			Locale locale) {
	...
	public static String getResetRequestEmailSubject(Locale locale) {
		return getContent("email_password_reset", TextType.SUBJECT, locale).replaceAll("XXX_INSTANCE_NAME_XXX",
				Imeji.CONFIG.getInstanceName());
	}
	...
	public static String getResetRequestEmailBody(String url, User user, String expirationDate, Locale locale) {
		return getContent("email_password_reset", TextType.MESSAGE, locale)
				...
				.replaceAll("XXX_PWD_RESET_LINK_XXX", url).replaceAll("XXX_EXPIRATION_DATE_XXX", expirationDate);
	}
	...
	public static String getResetConfirmEmailSubject(Locale locale) {
		return getContent("email_new_password", TextType.SUBJECT, locale).replaceAll("XXX_INSTANCE_NAME_XXX",
				Imeji.CONFIG.getInstanceName());
	}
	...
	public static String getSubscriptionEmailSubject(Locale locale) {
		return getContent("email_subscribtion", TextType.SUBJECT, locale).replace("XXX_INSTANCE_NAME_XXX",
				Imeji.CONFIG.getInstanceName());
	}
