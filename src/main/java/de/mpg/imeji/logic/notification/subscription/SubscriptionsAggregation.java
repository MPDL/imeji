...
public class SubscriptionsAggregation implements Aggregation {
  ...
  private void init() throws ImejiException {
    ...
    messagesByCollection = new MessageService().readAll().stream().collect(
        Collectors.groupingBy(m -> ObjectHelper.getId(URI.create(hierarchyService.getLastParent(getCollectionUri(m.getObjectId()))))));
  }
  ...
  private String createEmailBody(User user, Map<String, List<Message>> messages) {
    String collectionSummaries =
        messages.keySet().stream().map(id -> getCollectionSummary(id, messages, user)).collect(Collectors.joining());
  ...
  private String getCollectionSummary(String collectionId, Map<String, List<Message>> messages, User user) {
    ...
    if (c != null) {
      String newFilesText =
          messages.get(collectionId).stream().filter(m -> m.getType() == MessageType.UPLOAD_FILE || m.getType() == MessageType.MOVE_ITEM)
              .map(m -> getItemText(c, m)).collect(Collectors.joining("\n"));
      String newCollectionText = messages.get(collectionId).stream().filter(m -> m.getType() == MessageType.MOVE_COLLECTION)
          .map(m -> getItemText(c, m)).collect(Collectors.joining("\n"));
      String changedFilesText = messages.get(collectionId).stream().filter(m -> m.getType() == MessageType.CHANGE_FILE)
          .map(m -> getItemText(c, m)).collect(Collectors.joining("\n"));
      ...
      if (!StringHelper.isNullOrEmptyTrim(newFilesText) || !StringHelper.isNullOrEmptyTrim(newCollectionText)
        ...
        text += "\n- " + getCollectionText(c) + " -" + "\n\n";
        text += !StringHelper.isNullOrEmptyTrim(newFilesText) ? " --- New files ---\n" + newFilesText + "\n\n" : "";
        text += !StringHelper.isNullOrEmptyTrim(newCollectionText) ? " --- New collections ---\n" + newCollectionText + "\n\n" : "";
        text += !StringHelper.isNullOrEmptyTrim(changedFilesText) ? " --- Changed files ---\n" + changedFilesText + "\n\n" : "";
      }
    }
    return text;
  }
  ...
  private Map<String, List<Message>> getMessagesForUser(User user) {
    return messagesByCollection.entrySet().stream().filter(e -> hasSubscribedfor(user, e.getKey()) && isActiveForUser(user, e.getKey()))
  ...
  private Entry<String, List<Message>> filterMessages(Entry<String, List<Message>> entry, User user) {
    return new AbstractMap.SimpleEntry<String, List<Message>>(entry.getKey(),
        entry.getValue().stream().filter(m -> !isMovedInsameCollection(m) && exists(m, user)).collect(Collectors.toList()));
  }
  ...
  private boolean hasSubscribedfor(User user, String collectionId) {
    ...
    if (userSubscription != null) {
      return userSubscription.stream().filter(s -> s.getObjectId().equals(getCollectionId(collectionId))).findAny().isPresent();
    }
    return false;
  }
  ...
  private boolean exists(Message message, User user) {
    try {
      if (message instanceof ItemMessage) {
        new ItemService().retrieveLazy(ObjectHelper.getURI(Item.class, ((ItemMessage) message).getItemId()), user);
      }
      return true;
    } catch (Exception e) {
      return false;
  ...
  private String getItemText(CollectionImeji c, Message m) {
    switch (m.getType()) {
      case MOVE_COLLECTION:
        ...
        return getPath(cm) + cm.getName() + " (" + Imeji.PROPERTIES.getApplicationURL() + "collection/" + cm.getObjectId() + ")";
      default:
        ItemMessage im = (ItemMessage) m;
        return getPath(im) + im.getFilename() + " (" + Imeji.PROPERTIES.getApplicationURL() + "item/" + im.getItemId() + ")";
    }
  }
  ...
  private String getPath(String collectionId) {
    List<CollectionUriNameWrapper> parents =
        hierarchyService.findAllParentsWithNames(ObjectHelper.getURI(CollectionImeji.class, collectionId).toString(), true);
    parents.remove(0);
    if (parents.isEmpty()) {
      return "";
    }
