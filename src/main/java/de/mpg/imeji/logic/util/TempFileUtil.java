...
public class TempFileUtil {
	...
	private static File initTempDirectory() {
		try {
			File f = new File(PropertyReader.getProperty("imeji.storage.path") + StringHelper.fileSeparator + ".."
					+ StringHelper.fileSeparator + "tmp");
			if (!f.exists()) {
				f.mkdirs();
			}
			return f;
	...
	/**
	 * Create a temp {@link File}. This method should be used to ensure that all
