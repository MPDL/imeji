...
public class HierarchyService implements Serializable {
  ...
  public List<String> findAllSubcollections(String collectionUri) {
    return hierarchy.getTree().get(collectionUri) != null
        ? hierarchy.getTree().get(collectionUri).stream().flatMap(s -> Stream.concat(Stream.of(s), findAllSubcollections(s).stream()))
  ...
  public List<CollectionUriNameWrapper> findAllParentsWithNames(String uri, boolean includeSelft) {
    List<CollectionUriNameWrapper> l =
        findAllParents(uri).stream().map(id -> new CollectionUriNameWrapper(id)).collect(Collectors.toList());
    if (includeSelft) {
  ...
  private String getParentUri(Object o) {
    ...
    if (o instanceof CollectionImeji) {
      return ((CollectionImeji) o).getCollection() != null ? ((CollectionImeji) o).getCollection().toString() : null;
    }
    return null;
  }
