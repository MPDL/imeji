...
/**
 * Access to files that provide content for email messages and email subjects in
 * different languages
 * 
 * @author breddin
 *
...
public class EmailContentLocaleResource {
	...
	/**
	 * Internal representation of content of the associated file Data structure
	 * contains a list of identifiers and mapped email content
	 */
	private EmailContentListXML emailContent;

	/**
	 * Create a resource class to access a XML file in the file system that contains
	 * user configures content for e-mails (subject and message) in the given
	 * language
	 * 
	 * @param locale
	 *            language
	...
	public EmailContentLocaleResource(Locale locale) {
		...
		this.locale = locale;
		this.emailContentLanguageFile = new File(
				getResourceFilePath() + "emailMessages" + "_" + locale.getLanguage() + ".xml");
	}

	/**
	 * Read the path in file system where the associated XML file is stored
	 * 
	 * @return file path or empty string of file path could not be retrieved from
	 *         imeji properties
	 */
	 ...
	 * associated file in the file system. Cache this information in a map-like data
	 * structure
	 */
	...
	/**
	 * Write email content to associated file only if the content has been
	 * changed/edited in GUI before
	 */
