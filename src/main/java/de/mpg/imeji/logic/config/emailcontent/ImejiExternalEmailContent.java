...
/**
 * Class takes care of copying content of e-mails (messages and subjects) from internal 
 * .war resources files (messages_[en,de,es,ja].properties) to external xml files.
 * These are stored in the server's file system.
 * After the texts have been copied and stored outside the project
 * the Imeji administrator can edit them in the Admin > Config > EMail Messages view
 * Copying/restoring of email content is done via Admin > Tools > Copy email content
 *   
 * @author breddin
 *
 */
public class ImejiExternalEmailContent {
	 ...
	 * Some subjects and bodys that exist in messages_[en,de,es,ja].properties are not used in Imeji
	 * These are maked with "(not used)" and will not be copied from  messages_[en,de,es,ja].properties
	 * 
	 * Mail content:
	 * 
	 * (currently used and copied)
	 * email_registration_request_subject, email_registration_request_body
	 * email_account_activation_body, email_account_activation_subject
	 * email_password_reset_body, email_password_reset_subject
	 * email_new_password, email_new_password_subject
	 * email_invitation_body, email_invitation_subject
	 * email_shared_collection, email_shared_collection_subject
	 * email_subscribtion_body, email_subscribtion_subject
	 * 
	 * (currently not in use and not copied)
	 * email_item_downloaded_body, email_item_downloaded_subject
	 * email_new_user, email_new_user_subject
	 * email_zip_images_downloaded_body, email_zip_images_downloaded_subject
	 * email_unshared_object
	 * email_shared_item
	*/

	/*
	 *  identifier of mails that will be copied from internal messages_[en,de,es,ja].properties
	 */
	private static String[] emailContentIdentifiersBody = {
			"email_account_activation_body", 
			"email_invitation_body", 
			"email_new_password", 
			"email_password_reset_body", 
			"email_registration_request_body", 
			"email_shared_collection",  
			"email_subscribtion_body"  
			};
	
	private static String[] emailContentIdentifiersSubject = {
			"email_account_activation_subject", 
			"email_invitation_subject", 
			"email_new_password_subject", 
			"email_password_reset_subject", 
			"email_registration_request_subject", 
			"email_shared_collection_subject",
			"email_subscribtion_subject"
	        };
	
	// identifier of mail content in newly created xml files
	// remark: in messages_[en,de,es,ja].properties content identifiers are marked with "subject" and "body" 
	// in xml there is a representation of a complete email (subject and body)
	// as an identifier for the complete email the word-stem is used and postfixes "subject" and "body" are omitted
	// example:
	// "email_account_activation_body" (identifier for message of email_account_activation)
	// "email_account_activation_subject" (identifier for subject of email_account_activation)
	// -> xml identifier for whole email: "email_account_activation"
	
	private static String[] emailContentIdentifiersXML = {
			"email_account_activation", 
			"email_invitation", 
			"email_new_password", 
			"email_password_reset", 
			"email_registration_request", 
			"email_shared_collection",  
			"email_subscribtion"				
	};
