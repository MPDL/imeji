...
/**
 * Class takes care of copying content of e-mails (messages and subjects) from internal .war
 * resources files (messages_[en,de,es,ja].properties) to external xml files. These are stored in
 * the server's file system. After the texts have been copied and stored outside the project the
 * Imeji administrator can edit them in the Admin > Config > EMail Messages view Copying/restoring
 * of email content is done via Admin > Tools > Copy email content
 * 
 * @author breddin
 *
 */
public class ImejiExternalEmailContent {
   ...
   */
  private static String[] emailContentIdentifiersBody = {"email_account_activation_body", "email_invitation_body", "email_new_password",
      "email_password_reset_body", "email_registration_request_body", "email_shared_collection", "email_subscribtion_body"};

  private static String[] emailContentIdentifiersSubject =
      {"email_account_activation_subject", "email_invitation_subject", "email_new_password_subject", "email_password_reset_subject",
  ...
  private static String[] emailContentIdentifiersXML = {"email_account_activation", "email_invitation", "email_new_password",
      "email_password_reset", "email_registration_request", "email_shared_collection", "email_subscribtion"};

  /**
   * Function should only be called from GUI Admin > Tools > Edit email content button Function
   * takes the current system languages and copies all email content from internal files
   * (.properties) to xml files in the file system
   */
   ...
   * 
   * In case no XML file with email content in this language exists yet (and can be read from file
   * system) a new XML file is created by copying email content from internal
   * messages_[en,de,es,ja].properties file
   * 
   * @param languageCode
   */
  ...
  /**
   * Reads email content (subject and message) in the given language from an internal
   * messages_[en,es,de,ja].properties file Converts read email content to XML structure Writes XML
   * structured email content to an XML file in the file system
   * 
  ...
  private static void copyEmailContentToExternalXMLFile(String languageCode) {
    ...
    try {
      ...
      LOGGER.info("Couldn't find a ResourceBundle for language code " + languageCode + ". No texts, labels, messages for language "
          + languageCode + " are available for Imeji");
    }
    ...
    if (languageBundle != null) {
      ...
      try {
        ...
        LOGGER.info(
            "Could not write to " + xmlWriter.getResourceFilePath() + ".  Standard e-mail content from distribution will be used instead.");
      }
    }
  }
