...
public class StatisticsService {
	...
	public List<String> getAllInstitute() {
		...
		return s.searchString(JenaCustomQueries.selectAllInstitutes(), null, null, Search.SEARCH_FROM_START_INDEX,
				Search.GET_ALL_RESULTS).getResults();
	}

	/**
	 * Return the size of the storage used by one institute (i.e. the size of all
	 * files in collections belonging the institut)
	 *
	 * @param instituteName
	 * @return
	 */
	...
	public long getAllFileSize() {
		final Search s = new JenaSearch(SearchObjectTypes.ALL, null);
		final List<String> result = s.searchString(JenaCustomQueries.selectFileSizeForAll(), null, null,
				Search.SEARCH_FROM_START_INDEX, Search.GET_ALL_RESULTS).getResults();
		if (result.size() == 1 && result.get(0) != null) {
			final String size = result.get(0).replace("^^http://www.w3.org/2001/XMLSchema#integer", "");
			return Long.parseLong(size);
