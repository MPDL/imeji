...
public class InternalStorageManager implements Serializable {
	...
	/**
	 * The URL used to access the storage (this is a dummy url, used by the internal
	 * storage to parse file location)
	 */
	...
	/**
	 * Remove a {@link InternalStorageItem} from the internal storage (i.e. removes
	 * all resolutuion of the {@link Item})
	 *
	 * @param item
	 */
	...
	/**
	 * Extract the filename out of a path (then use StringHelper.fileSeparator as
	 * separator), or of an url(then use StringHelper.urlSeparator as separator)
	 *
	 * @param pathOrUrl
	 * @param separator
	 * @return
	...
	private String generateId(int version) {
		...
		return uuid.substring(0, 2) + StringHelper.urlSeparator + uuid.substring(2, 4) + StringHelper.urlSeparator
				+ uuid.substring(4, 6) + StringHelper.urlSeparator + uuid.substring(6) + StringHelper.urlSeparator
				+ version;
	}

	/**
	 * Create the URL of the file from its filename, its id, and its resolution.
	 * Important: the filename is encoded, to avoid problems by reading this url
	 *
	 * @param id
	 * @param filename
	 * @param resolution
	...
	public String generateUrl(String id, String filename, FileResolution resolution) {
		...
		return storageUrl + id + StringHelper.urlSeparator + resolution.name().toLowerCase() + StringHelper.urlSeparator
				+ filename;
	}
	...
	/**
	 * Delete existing full web thumbnail images/previews of a file and create new
	 * ones Needed after changes in GUI or changes in file icons
	 * 
	 * @param item
	 *            InternalStorageItem of the file
	 * @param file
	 *            file for which new full web thumbnail images/previews will be
	 *            generated
	 */
