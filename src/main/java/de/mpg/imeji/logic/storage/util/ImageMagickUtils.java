...
public class ImageMagickUtils {
	...
	public static File resizeJpg(File file, String extension, FileResolution resolution) {
		try {
			...
			LOGGER.error("Error resizing jpg with Imagemagick for file " + file.getAbsolutePath() + " with extension "
					+ extension, e);
		}
		return null;
	}
	...
	/**
	 * Convert a File from a file format (extensionIn) to another File format
	 * (extensionOut)
	 * 
	 * @param tmp
	 * @param extensionIn
	...
	/**
	 * Search for the first non blank image generated by imagemagick, based on
	 * commandline: convert image.jpg -shave 1%x1% -resize 40% -fuzz 10% -trim
	 * +repage info: | grep ' 1x1 '
	 *
	 * @param path
	 * @return
	 * @throws IOException
