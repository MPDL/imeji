...
/**
 * {@link ImageGenerator} that returns a stored jpg icon file Different icons files exist for
 * different extensions (i.e. txt, ppt, etc)
 * 
 * @author jandura
 *
 */
...
public class TextFileIconGenerator extends ImageGenerator {
  ...
  private static final String[] TEXT_MIME_TYPES =
      {"text/plain", "text/comma-separated-values", "text/tab-separated-values", "application/msword", "application/mspowerpoint",
          "application/msexcel", "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
  ...
  public File generatePreview(File file, String fileExtension) throws ImejiException {
    ...
    try {
      ...
      if (textIconResource != null) {
        File tempIconFile =
            TempFileUtil.createTempFile("NiceRawFile", FilenameUtils.getExtension(new File(textIconResource.toURI()).getName()));
