...
public final class ImageGeneratorManager {
	 ...
	 * top-down and the first class that "fits" the file type creates the image
	 * file.
	 */
	...
	public ImageGeneratorManager() {
		...
		generators.add(new SimpleAudioImageGenerator()); // creates a preview for audio files (file with mime type
															// "audio")
		generators.add(new MagickImageGenerator()); // creates a preview for image and video files using ImageMagick
													// project software library (see
													// http://www.imagemagick.org/script/index.php)
		generators.add(new SimpleImageGenerator()); // creates a preview for image files (file with mime type "image")
		generators.add(new TextFileIconGenerator()); // creates a preview for all files types for which a fixed icon
														// exists (icon files are stored in
		...
		generators.add(new RawFileImageGenerator()); // creates a default preview image for file types that haven't been
														// handled above
	...
	/**
	 * Generate a full resolution image. Convert the original to jpeg if possible,
	...
	/**
	 * Generate an image (only jpg and gif supported here) into a smaller image
	 * according to the {@link FileResolution}
	 *
	 * @param bytes
	 * @param extension
	 * @param resolution
	...
	/**
	 * Generate image/icon/preview in jpeg format for a file with a given
	 * resolution.
	 *
	 * @param file
	 *            File for which an preview/icon/thumbnail shall be created
	 * @param extension
	 *            file extension
	 * @param resolution
	 *            resolution of the preview/icon/thumbnail
	 * @return
	 */
	 ...
	 *
	 * @param file
	 *            the file for which a preview is created
	 * @param extension
	 *            the file extension
