...
/**
 * Generates a preview image of a file.
 * Provides the opportunity to write customized text on a given background file
...
public abstract class AbstractWritableImageGenerator extends ImageGenerator{
	...
	protected File generateAndWriteOnImage(File file, String textForImage) throws IOException, URISyntaxException {
	  ...
	  BufferedImage icon;
      icon = ImageIO.read(new FileImageInputStream(new File(AbstractWritableImageGenerator.class
          .getClassLoader().getResource(this.writeOnThisIcon.getIconPath()).toURI())));
	...
	private BufferedImage writeTextOnImage(BufferedImage backgroundImage, String textForImage) {
	    ...
	    g2d.drawString(textForImage, this.writeOnThisIcon.getXPosition() - fontMetrics.stringWidth(textForImage), this.writeOnThisIcon.getYPosition());
	    g2d.dispose();
	    return img;
    }
