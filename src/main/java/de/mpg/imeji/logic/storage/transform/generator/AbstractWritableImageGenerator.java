...
/**
 * Generates a preview image of a file. Provides the opportunity to write
 * customized text on a given background file
 * 
...
public abstract class AbstractWritableImageGenerator extends ImageGenerator {
	...
	public AbstractWritableImageGenerator(String backgroundIconName, Point textPosition, Font font, Color textColor,
			int maxTextLength) {
	...
	/**
	 * Use this function to access the background image, write text (file extension)
	 * on it and save it
	 * 
	 * @param file
	 * @param textForImage
	...
	protected File generateAndWriteOnImage(File file, String textForImage) throws IOException, URISyntaxException {
		...
		BufferedImage icon;
		icon = ImageIO.read(new FileImageInputStream(new File(AbstractWritableImageGenerator.class.getClassLoader()
				.getResource(this.writeOnThisIcon.getIconPath()).toURI())));
