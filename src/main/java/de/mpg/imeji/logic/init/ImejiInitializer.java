...
public class ImejiInitializer {
	...
	public static void init(String path) throws IOException, URISyntaxException, ImejiException {
		...
		Imeji.dataset = Imeji.tdbPath != null ? TDBFactory.createDataset(Imeji.tdbPath) : TDBFactory.createDataset();
	...
	/**
	 * Initialize the system administrator {@link User}, accoring to credentials in
	 * imeji.properties
	 */
	private static void initadminUser() {
		try {
			...
			Imeji.adminUser.setPerson(ImejiFactory.newPerson("Admin", "imeji", "Max Planck Digital Library"));
			...
			Imeji.adminUser.setApiKey(APIKeyAuthentication.generateKey(Imeji.adminUser.getId(), Integer.MAX_VALUE));
			Imeji.adminUser.setGrants(AuthorizationPredefinedRoles.imejiAdministrator(Imeji.PROPERTIES.getBaseURI()));
			// create
	...
	public static void initDefaultStatements() {
		...
		l.add(ImejiFactory.newStatement().setIndex("Location").setType(StatementType.GEOLOCATION).build());
		...
		try {
			if (service.search(null, null, Imeji.adminUser, Search.SEARCH_FROM_START_INDEX, Search.GET_ALL_RESULTS)
