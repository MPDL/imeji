...
public class FolderExport extends ExportAbstract {
	...
	private void builZip(ZipOutputStream zip, List<CollectionElement> objects, String parentPath)
		...
		final List<String> collectionUris = objects.stream().filter(o -> o.getType() == ObjectType.COLLECTION)
				.map(o -> o.getUri()).collect(Collectors.toList());
		final List<String> itemUris = objects.stream().filter(o -> o.getType() == ObjectType.ITEM).map(o -> o.getUri())
	...
	private List<CollectionElement> retrieveObjects(CollectionImeji collection) throws ImejiException {
		final SearchResult itemUIDs = facade.search(null, collection, super.user, null, Search.GET_ALL_RESULTS,
