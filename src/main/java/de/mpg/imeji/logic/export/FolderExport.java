...
public class FolderExport extends ExportAbstract {
  ...
  public FolderExport(String collectionId, User user) throws ImejiException {
    super(user);
    this.collection = retrieveCollection(ObjectHelper.getURI(CollectionImeji.class, collectionId).toString());
  ...
  private void builZip(ZipOutputStream zip, List<CollectionElement> objects, String parentPath) throws ImejiException {
    final List<String> collectionUris =
        objects.stream().filter(o -> o.getType() == ObjectType.COLLECTION).map(o -> o.getUri()).collect(Collectors.toList());
    final List<String> itemUris =
        objects.stream().filter(o -> o.getType() == ObjectType.ITEM).map(o -> o.getUri()).collect(Collectors.toList());
   ...
   * @throws IOException
   */
  private void addFolders(ZipOutputStream zip, List<String> collectionUris, String parentPath) throws ImejiException {
   ...
   * @throws ImejiException
   */
  private void addFiles(ZipOutputStream zip, List<String> itemUris, String parentPath) throws ImejiException {
