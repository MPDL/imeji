...
public class AuthorizationPredefinedRoles {
	...
	public static List<String> defaultUser(String uri) {
		return new ArrayList<String>(Arrays.asList(new Grant(GrantType.EDIT, IMEJI_GLOBAL_URI).toGrantString(),
				new Grant(GrantType.ADMIN, uri).toGrantString()));
	}

	/**
	 * This user can not create a collection in imeji. He only has the {@link Grant}
	 * on his account
	 *
	 * @param uri
	 * @return
	...
	public static List<String> restrictedUser(String uri) {
		return new ArrayList<String>(Arrays.asList(new Grant(GrantType.READ, IMEJI_GLOBAL_URI).toGrantString(),
				new Grant(GrantType.ADMIN, uri).toGrantString()));
	}
