...
public class UserService {
	...
	/**
	 * Create a new user in the database with predefined roles (ADMIN, DEFAULT or
	 * RESTRICTED)
	 *
	 * @param u
	 * @param type
	...
	/**
	 * True if a user has been Modified, i.e the last modification of the user in
	 * the database is older than the last modification of the user in the session.
	 * (For instance when an object has been shared with the user)
	 *
	 * @param u
	 * @return
	 */
	...
	public long checkQuota(User user, File file, CollectionImeji col) throws ImejiException {
		...
		final User targetCollectionUser = user.getId().equals(col.getCreatedBy())
				? user
				: retrieve(col.getCreatedBy(), Imeji.adminUser);

		final Search search = SearchFactory.create();
		final List<String> results = search
				.searchString(JenaCustomQueries.selectUserFileSize(col.getCreatedBy().toString()), null, null,
	...
	public String getCompleteName(URI uri, Locale locale) {
		...
		final List<String> users = search
				.searchString(JenaCustomQueries.selectUserCompleteName(uri), null, Imeji.adminUser, 0, 1).getResults();
	...
	/**
	 * Search for all {@link Organization} in imeji, i.e. t The search looks within
	 * the {@link User} and the {@link Collection} what {@link Organization are
	 * already existing.
	 *
	 * @param name
	 * @return
	...
	private Collection<Organization> searchOrganizationByNameInUsers(String name) {
		final Search search = SearchFactory.create(SearchObjectTypes.USER, SEARCH_IMPLEMENTATIONS.JENA);
		return loadOrganizations(search.searchString(JenaCustomQueries.selectOrganizationByName(name), null, null,
				Search.SEARCH_FROM_START_INDEX, Search.GET_ALL_RESULTS).getResults(), Imeji.userModel);
	}
	...
	public static boolean adminUserExist() {
		boolean exist = false;
		final Search search = SearchFactory.create();
		final List<String> uris = search.searchString(JenaCustomQueries.selectUserSysAdmin(), null, null,
	...
	public List<User> searchUsersToBeNotified(User user, CollectionImeji c) {
		final Search search = SearchFactory.create();
		final List<String> uris = search.searchString(JenaCustomQueries.selectUsersToBeNotifiedByFileDownload(user, c),
