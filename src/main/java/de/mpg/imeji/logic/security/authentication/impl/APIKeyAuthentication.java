...
public class APIKeyAuthentication implements Authentication {
  ...
  public User doLogin() throws AuthenticationError {
    try {
      ...
      if (!user.isActive()) {
        throw new InactiveAuthenticationError("Not active user: please activate your account with the limk sent after your registration");
   ...
   */
  public static String generateKey(URI userId, int expirationTimeMinutesInTheFuture) throws JoseException {
   ...
   * @throws JoseException
   */
  private static String produceJsonWebToken(URI userId, int expirationTimeMinutesInTheFuture) throws JoseException {
    // Create the Claims, which will be the content of the JWT
    final JwtClaims claims = new JwtClaims();
    // claims.setIssuer("Issuer"); // who creates the token and signs it
    // claims.setAudience(userId.toString()); // to whom the token is intended to be
    // sent
  ...
  public static String consumeJsonWebToken(String token) throws JoseException {
    final JwtConsumer jwtConsumer = new JwtConsumerBuilder().setRequireExpirationTime().setAllowedClockSkewInSeconds(30).setRequireSubject()
