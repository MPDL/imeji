package de.mpg.imeji.logic.security.registration;
...
public class RegistrationService {
  ...
  private static final KeyValueStoreService KEY_VALUE_STORE_BC = new KeyValueStoreService(new HTreeMapStore("registrationStore"));
  ...
  public Registration retrieveByToken(String token) throws ImejiException {
    final List<Registration> registrations = KEY_VALUE_STORE_BC.getList(token + ":.*", Registration.class);
    if (registrations.size() == 1) {
      return registrations.get(0);
    }
    throw new NotFoundException("Count of registration with token " + token + ": " + registrations.size());
  }
  ...
  public Registration retrieveByEmail(String email) throws ImejiException {
    final List<Registration> registrations = KEY_VALUE_STORE_BC.getList(".*:" + email, Registration.class);
    if (registrations.size() == 1) {
      return registrations.get(0);
    }
    throw new NotFoundException("Count of registration with email " + email + ": " + registrations.size());
  }
  ...
  public Registration register(User user) throws ImejiException {
    ...
    final Registration registration = new Registration(IdentifierUtil.newUniversalUniqueId(), user, password);
  ...
  public User activate(Registration registration) throws ImejiException {
    ...
    final USER_TYPE type = isAuthorizedEmail(registration.getUser().getEmail()) ? USER_TYPE.DEFAULT : USER_TYPE.RESTRICTED;
    final User user = new UserService().create(registration.getUser(), type);
    delete(registration);
    return user;
  }
