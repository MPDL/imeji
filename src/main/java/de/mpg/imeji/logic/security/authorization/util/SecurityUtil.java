...
public class SecurityUtil {
  ...
  public static List<String> getListOfAllowedCollections(User user) {
    return authorization().toGrantList(authorization().getAllGrants(user)).stream().filter(g -> g.getGrantFor().contains("/collection/"))
  ...
  public static Grant getGrantForObject(Collection<String> grants, String id) {
    try {
      Optional<Grant> optional =
          grants.stream().map(s -> new Grant(s)).filter(s -> s.getGrantFor() != null && s.getGrantFor().equals(id)).findFirst();
