...
public class StatementUtil {
	...
	/**
	 * Transform a String of {@link Statement} names (ex: title,Description) into a
	 * {@link List} of {@link Statement} ids. <br/>
	...
	/**
	 * Return a list of statement as a String of all their names separated by a
	 * comma
	 * 
	 * @param statements
	 * @return
	...
	public static String toStatementNamesString(List<Statement> statements) {
		return statements.stream().map(s -> ObjectHelper.getId(s.getUri())).collect(Collectors.joining(","));
	}
	...
	public static Map<String, Statement> statementListToMap(List<Statement> l) {
		return l.stream().collect(Collectors.toMap(Statement::getIndex, Function.identity(), (s1, s2) -> s1));
	}
	...
	public static String formatIndex(String index) {
		return index != null ? new String(index.trim().replace(" ", "_").replaceAll("[^\\w]", "").toLowerCase()) : "";
	}
	...
	public static String getIndexUrlEncoded(String index) {
		try {
			return URLEncoder.encode(formatIndex(index), "UTF-8");
