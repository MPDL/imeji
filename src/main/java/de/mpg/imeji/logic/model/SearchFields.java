...
public enum SearchFields {
	id, collection, collectionid, creatorid, creator, collaborator, role, read, editor, created, modified, status, filename, col, family, given, organization, title, description, author, author_familyname(
			"author.familyname"), author_givenname("author.givenname"), author_organization(
					"author.organization"), md, index, all, license, checksum, filetype, filesize, fileextension, pid, info_label, info_text, info_url, email, technical, fulltext, completename, collection_title(
							"collection.title"), collection_description("collection.description"), collection_author(
	...
	public static SearchFields valueOfIndex(String index) {
		return Stream.of(SearchFields.values()).filter(f -> f.getIndex().equals(index.toLowerCase())).findFirst()
				.orElse(all);
	}
}
