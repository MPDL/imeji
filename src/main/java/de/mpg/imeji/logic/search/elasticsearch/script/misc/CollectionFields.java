...
public class CollectionFields {
	...
	public CollectionFields(CollectionImeji c) {
		...
		this.organizations = c.getPersons().stream().flatMap(p -> p.getOrganizations().stream()).map(o -> o.getName())
				.collect(Collectors.toList());
		this.titleWithId = this.titleWithIdOfCollection(c.getTitle(),
	...
	public CollectionFields(byte[] sourceAsBytes) throws IOException {
		...
		this.authors = sourceNode.findValues("author").stream().map(a -> a.get("completename").asText())
				.collect(Collectors.toList());
		this.organizations = sourceNode.findValues("author").stream().map(a -> a.get("organization").asText())
				.collect(Collectors.toList());
	...
	/**
	 * Constructs a compound String that contains both the title and the ID of a
	 * collection
	 * 
	...
	/**
	 * Deconstructs a compound String that contains both the title and the ID of a
	 * collection Returns the title
	 * 
	 * @return
	 */
