...
public class ElasticInitializer {
	...
	public static void start(String clusterName) throws IOException, URISyntaxException {
		TransportClient tc = TransportClient.builder().settings(
				Settings.builder().put("path.home", ElasticService.CLUSTER_DIR).put("cluster.name", clusterName))
				.build();
		tc.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("localhost"), 9300));
	...
	private static void start(String clusterName, Client client) throws IOException, URISyntaxException {
	...
	public static String createIndexWithAlias() {
		try {
			...
			ElasticService.getClient().admin().indices().prepareAliases().addAlias(indexName, ElasticService.DATA_ALIAS)
					.execute().actionGet();
			return indexName;
	...
	public synchronized static String getIndexNameFromAliasName(final String aliasName) {
		try {
			final ImmutableOpenMap<String, List<AliasMetaData>> map = ElasticService.getClient().admin().indices()
	...
	public static void setNewIndexAndRemoveOldIndex(String newIndex) {
		...
		if (oldIndex != null && !oldIndex.equals(newIndex)) {
			ElasticService.getClient().admin().indices().prepareAliases().addAlias(newIndex, ElasticService.DATA_ALIAS)
	...
	public static String createIndex() {
		try {
			...
			final String settingsName = ElasticService.ANALYSER == ElasticAnalysers.ducet_sort
					? ElasticService.SETTINGS_DUCET
					: ElasticService.SETTINGS_DEFAULT;
			final String settingsJson = new String(
					Files.readAllBytes(
							Paths.get(ElasticIndexer.class.getClassLoader().getResource(settingsName).toURI())),
					"UTF-8");
			ElasticService.getClient().admin().indices().prepareCreate(indexName).setSettings(settingsJson).execute()
					.actionGet();
			return indexName;
		} catch (final Exception e) {
