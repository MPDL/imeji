...
public class FacetService extends SearchServiceAbstract<Facet> {
	...
	private void checkUniqueIndex(Facet facet) throws AlreadyExistsException {
		if (exists(facet.getIndex()) && !cachedFacetsMapByIndex.get(facet.getIndex()).getUri().toString()
				.equals(facet.getUri().toString())) {
	...
	public List<Facet> retrieve(List<String> ids, User user) throws ImejiException {
		return controller.retrieveBatch(ids, Imeji.adminUser).stream()
				.sorted((f1, f2) -> Integer.compare(f1.getPosition(), f2.getPosition())).collect(Collectors.toList());
	}
