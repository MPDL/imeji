...
public class SearchQueryParser {
	...
	/**
	 * Parse a url search query into a {@link SearchQuery}. Decode the query with
	 * UTF-8
	 *
	 * @param query
	 * @return
	...
	public static void main(String[] args) throws UnprocessableError {
		String q = "rien du tout OR (title=this is an example AND ((description=\"Super Description\" OR created=2000)) OR (md.description=\"Other Description\" OR (md.created.number>=2000 AND md.created.number<50)))";
	...
	private static SearchElement parsePair(String s, boolean not) throws UnprocessableError {
		if (new StringParser(METADATA_PATTERN).find(s)) {
			...
			return new SearchFactory()
					.or(Arrays.asList(new SearchPair(SearchFields.all, SearchOperators.EQUALS, unescape(s), not),
	...
	private static SearchPair parseMetadata(String s, boolean not) {
		...
		if (parser.find(s)) {
			...
			return new SearchMetadata(indexes[0],
					indexes.length > 1 ? SearchMetadataFields.valueOfIndex(indexes[1]) : null, operator, value, not);
		}
		return new SearchPair();
	}

	/**
	 * Parse to a {@link TechnicalMetadata} following strings:
	...
	/**
	 * True is a {@link SearchQuery} is a simple search (i.e. triggered from the
	 * simple search form)
	 *
	 * @param searchQuery
	 * @return
	...
	private static String searchPairToStringQuery(SearchPair pair) {
		return (pair.isNot() ? "NOT " : "") + pair.getField().getIndex() + operator2URL(pair.getOperator())
	...
	/**
	 * Transform a {@link SearchOperators} to a {@link String} value used in url
	 * query
	 *
	 * @param op
	 * @return
	...
	/**
	 * True if the character at the passed index is a special character and is
	 * escaped
	 * 
	 * @param s
	 */
