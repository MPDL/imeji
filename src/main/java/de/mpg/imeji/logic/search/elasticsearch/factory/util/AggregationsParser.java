...
public class AggregationsParser {
	...
	/**
	 * Parse a the Aggregation part of a SearchResponse and return a list of
	 * {@link FacetResult}. Note: the aggregation must have been created by
	 * {@link ElasticAggregationFactory}
	 * 
	 * @param resp
	 * @return
	 */
				...
				for (Aggregation mdAgg : metadata.getAggregations()) {
					...
					if (mdAgg instanceof Filter) {
						...
						if (terms instanceof StringTerms) {
							...
							FacetResultValue result = new FacetResultValue(terms.getName(),
									((InternalStats) terms).getCount());
