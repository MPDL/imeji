...
public class ElasticQueryFactory {
	...
	public QueryBuilder build() {
		...
		final QueryBuilder securityQuery = new SecurityQueryFactory().user(user)
				.searchForCollection(searchForCollection).build();
		final QueryBuilder statusQuery = buildStatusQuery(query, user);
		final QueryBuilder filterQuery = buildSearchQuery(query != null ? query.getFilterElements() : new ArrayList<>(),
				user);
	...
	/**
	 * Check if at least on {@link SearchPair} is related to the status. If yes,
	 * return true
	 *
	 * @param elements
	 * @return
	...
	private QueryBuilder termQuery(SearchPair pair, User user) {
		...
		switch (index) {
			...
			case collection :
				return fieldQuery(
						ElasticFields.FOLDER, "\"" + ObjectHelper
			...
			case collection_author_organisation :
				return fieldQuery(ElasticFields.ORGANIZATION_OF_COLLECTION, pair.getValue(), pair.getOperator(),
						pair.isNot());
			case family :
			...
			case role :
				// same as grant_type
				final GrantType grant = pair.getValue().equals("upload")
						? GrantType.EDIT
	...
	private QueryBuilder metadataFilter(SearchMetadata md) {
		if (md.getMetadataField() == null) {
			return metadataQuery(fieldQuery(ElasticFields.METADATA_TEXT, md.getValue(), md.getOperator(), md.isNot()),
					md.getIndex());
		}
		switch (md.getMetadataField()) {
			...
			case number :
				return metadataQuery(
						numberQuery(ElasticFields.METADATA_NUMBER.field(), md.getValue(), md.getOperator(), md.isNot()),
						md.getIndex());
			case date :
				return metadataQuery(
						timeQuery(ElasticFields.METADATA_TIME.field(), md.getValue(), md.getOperator(), md.isNot()),
						md.getIndex());
			case url :
				return metadataQuery(
			...
			case familyname :
				return metadataQuery(
						fieldQuery(ElasticFields.METADATA_FAMILYNAME, md.getValue(), md.getOperator(), md.isNot()),
						md.getIndex());
			case givenname :
				return metadataQuery(
						fieldQuery(ElasticFields.METADATA_GIVENNAME, md.getValue(), md.getOperator(), md.isNot()),
						md.getIndex());
			case coordinates :
	...
	/**
	 * Create a {@link QueryBuilder} - used to sarch for metadata which are defined
	 * with a statement
	 *
	 * @param index
	 * @param value
	...
	private QueryBuilder geoQuery(String value) {
		...
		return QueryBuilders.geoDistanceQuery(ElasticFields.METADATA_LOCATION.field()).distance(distance).point(lat,
				lon);
	}
	...
	/**
	 * Search for items/collections of collaobrator, i.e. users with who the
	 * collections have been shared
	 * 
	 * @param email
	 * @return
	 */
	private QueryBuilder collaboratorQuery(String email) {
		try {
			...
			QueryBuilder creatorQuery = fieldQuery(ElasticFields.CREATOR, user.getId().toString(),
					SearchOperators.EQUALS, false);
			QueryBuilder roleQuery = new SecurityQueryFactory().user(user).searchForCollection(searchForCollection)
					.role(GrantType.READ).build();
	...
	private QueryBuilder licenseQuery(SearchPair pair) {
		...
		for (final String licenseName : pair.getValue().split(" OR ")) {
			if ("*".equals(licenseName)) {
				licenseQuery.mustNot(
						fieldQuery(ElasticFields.LICENSE, ImejiLicenses.NO_LICENSE, SearchOperators.EQUALS, false));
