...
public class JenaCustomQueries {
  ...
  public static final String selectPersonByName(String name) {
    return X_PATH_FUNCTIONS_DECLARATION
        + "  SELECT DISTINCT ?s WHERE {?s <http://purl.org/escidoc/metadata/terms/0.1/complete-name>  ?name . filter(regex(?name, '" + name
        + "','i'))}";
  }
  ...
  public static final String selectUserByEmail(String email) {
    return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE { ?s a  <http://imeji.org/terms/user>. "
        + " ?s <http://xmlns.com/foaf/0.1/email> \"" + email + "\"^^<http://www.w3.org/2001/XMLSchema#string> }";
  }
  ...
  public static final String selectSubscriptionByObjectId(String objectId) {
    return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/objectId> \"" + objectId
  ...
  public static final String selectSubscriptionByUserId(String userId) {
    return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/userId> \"" + userId
        + "\"^^<http://www.w3.org/2001/XMLSchema#string> }";
  }

  public static final String selectSubscriptionAll() {
    return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE {?s a <http://imeji.org/terms/subscription>}";
  }
  ...
  public static final String selectUserByRegistrationToken(String registrationToken) {
    return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE { " + " ?s <http://imeji.org/terms/registrationToken> \""
        + registrationToken + "\"^^<http://www.w3.org/2001/XMLSchema#string> . " + " ?s a <http://imeji.org/terms/user> }";
  }
  ...
  public static final String selectUserSysAdmin() {
    return X_PATH_FUNCTIONS_DECLARATION + " SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/grant>  \"ADMIN,"
  ...
  public static final String selectUserGroupByName(String name) {
    return X_PATH_FUNCTIONS_DECLARATION
        + "  SELECT DISTINCT ?s WHERE {?s a <http://imeji.org/terms/userGroup> . ?s <http://xmlns.com/foaf/0.1/name> \"" + name
        + "\"^^<http://www.w3.org/2001/XMLSchema#string>}";
  }
  ...
  public static final String selectUserGroupAll() {
    return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE {?s a <http://imeji.org/terms/userGroup>}";
  }
  ...
  public static final String selectUsersToBeNotifiedByFileDownload(User user, CollectionImeji c) {
    return X_PATH_FUNCTIONS_DECLARATION + "  " + "SELECT DISTINCT ?s WHERE {" + "filter(?c='" + ObjectHelper.getId(c.getId()) + "'"
        + (user != null ? " && ?s!=<" + user.getId().toString() + "> " : "") + ") . ?s <http://imeji.org/terms/observedCollections> ?c }";
  }
  ...
  public static final String selectCollectionIdOfItem(String itemUri) {
    return " SELECT DISTINCT ?s WHERE {<" + itemUri + "> <http://imeji.org/terms/collection> ?s} LIMIT 1 ";
  }
  ...
  public static final String selectItemIdOfFileUrl(String fileUrl) {
    ...
    return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE {"
        ...
        + path + "', 'i') || REGEX(str(?url2), '" + path + "', 'i') || REGEX(str(?url3), '" + path + "', 'i'))} LIMIT 1 ";
  }
  ...
  public static final String selectItemOfFile(String fileUrl) {
    return X_PATH_FUNCTIONS_DECLARATION + XSD_DECLARATION + "SELECT DISTINCT  (str(?id) AS ?s) WHERE {?content ?p \"" + fileUrl
  ...
  public static final String selectContentByStorageId(String storageId) {
    return "SELECT ?s WHERE {?s <http://imeji.org/terms/original> ?o . filter(regex(?o,'" + storageId + "'))} limit 1";
  }
  ...
  public static final String selectCollectionByLogoUrl(String logoUrl) {
    return X_PATH_FUNCTIONS_DECLARATION + XSD_DECLARATION + "SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/logoUrl> <" + logoUrl
        + ">} limit 1";
  }
  ...
  public static final String selectCollectionByLogoStorageId(String logoStorageId) {
    return "SELECT ?s WHERE {?s <http://imeji.org/terms/logoUrl> ?o . filter(regex(str(?o),'" + logoStorageId + "'))} limit 1";
  }
  ...
  public static final String selectGrantWithoutUser() {
    return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE { ?s <http://imeji.org/terms/grantType> ?type"
  ...
  public static final String updateRemoveGrantsFor(String uri) {
    return "WITH <http://imeji.org/user> DELETE {?user <http://imeji.org/terms/grant> ?s . ?s ?p ?o } " + "USING <http://imeji.org/user> "
        + " WHERE {?user <http://imeji.org/terms/grant> ?s . ?s <http://imeji.org/terms/grantFor> <" + uri + "> . ?s ?p ?o}";
  }
  ...
  public static final String updateCollectionParent(String id, String oldUri, String newUri) {
    ...
    q += "INSERT {<" + id + "> <http://imeji.org/terms/collection>  <" + newUri + ">} " + "USING <http://imeji.org/collection> WHERE {<"
        + id + "> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type>  <http://imeji.org/terms/collection>}";
    return q;
  }
  ...
  public static final String updateReleaseObject(String id, Calendar date) {
    String model = ObjectHelper.getObjectType(URI.create(id)) == ObjectType.ITEM ? "item" : "collection";
    String q = "WITH <http://imeji.org/" + model + "> ";
    q += " DELETE{<" + id + "> <http://imeji.org/terms/status> <http://imeji.org/terms/status#PENDING>} ";
    q += "INSERT{<" + id + "> <http://imeji.org/terms/status> <http://imeji.org/terms/status#RELEASED> . <" + id
  ...
  public static final String updateAddLicensetoItem(String itemId, License l) {
    ...
    return "WITH <http://imeji.org/item> " + "INSERT {<" + itemId + "> <http://imeji.org/terms/license> <" + licenseId + "> . <" + licenseId
        + ">  <http://imeji.org/terms/name> \"" + l.getName() + "\"^^<http://www.w3.org/2001/XMLSchema#string> "
        + (StringHelper.isNullOrEmptyTrim(l.getLabel()) ? ""
            : " . <" + licenseId + ">  <http://imeji.org/terms/label> \"" + l.getLabel() + "\"^^<http://www.w3.org/2001/XMLSchema#string> ")
        + (StringHelper.isNullOrEmptyTrim(l.getUrl()) ? ""
            : " . <" + licenseId + ">  <http://imeji.org/terms/url> \"" + l.getUrl() + "\"^^<http://www.w3.org/2001/XMLSchema#string> ")
        + " . <" + licenseId + ">  <http://imeji.org/terms/start>" + System.currentTimeMillis() + " . <" + licenseId
        + ">  <http://imeji.org/terms/start> -1} " + "USING <http://imeji.org/item> " + "WHERE{ <" + itemId
  ...
  public static final String updateCollection(String id, String oldUri, String newUri) {
    return "WITH <http://imeji.org/item> " + "DELETE {<" + id + "> <http://imeji.org/terms/collection>  <" + oldUri + ">} " + "INSERT {<"
        + id + "> <http://imeji.org/terms/collection>  <" + newUri + ">} " + "USING <http://imeji.org/item> " + " WHERE {<" + id
  ...
  public static final String selectVersion(String id) {
    return "SELECT (str(?version) AS ?s) WHERE { <" + id + "> <" + ImejiNamespaces.VERSION + "> ?version}";
  }
  ...
  public static final String insertFileSize(String itemId, String fileSize) {
    return "WITH <http://imeji.org/item> " + "INSERT {<" + itemId + "> <http://imeji.org/terms/fileSize> " + fileSize + "}"
   ...
   * @return
   */
  public static final String insertFileSizeAndDimension(String itemId, String fileSize, String width, String height) {
    return "WITH <http://imeji.org/item> " + "INSERT {<" + itemId + "> <http://imeji.org/terms/fileSize> " + fileSize + " . <" + itemId
        + "> <http://www.w3.org/2003/12/exif/ns#width> " + width + " . <" + itemId + "> <http://www.w3.org/2003/12/exif/ns#height> "
        + height + "}" + "USING <http://imeji.org/item> " + "WHERE{<" + itemId + "> ?p ?o}";
  }
  ...
  public static final String updateEmptyMetadata() {
    return "WITH <http://imeji.org/item> " + "DELETE {?mds <" + ImejiNamespaces.METADATA + "> ?s} " + "USING <http://imeji.org/item> "
        + "WHERE {?mds <" + ImejiNamespaces.METADATA + "> ?s . NOT EXISTS{?s ?p ?o}}";
  }
  ...
  public static final String selectContainerItemByFilename(URI containerURI, String filename) {
    ...
    return "SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/filename> ?el . FILTER(regex(?el, '^" + filename
        + "\\\\..+', 'i')) .?s <http://imeji.org/terms/collection> <" + containerURI.toString() + "> . ?s <" + ImejiNamespaces.STATUS
  ...
  public static final String selectItemByChecksum(URI containerURI, String checksum) {
    return "SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/checksum> \"" + checksum
        + "\"^^<http://www.w3.org/2001/XMLSchema#string>. " + "?s <http://imeji.org/terms/collection> <" + containerURI.toString() + "> . "
        + "?s <" + ImejiNamespaces.STATUS + "> ?status . " + " FILTER (?status!=<" + Status.WITHDRAWN.getUriString() + ">)} LIMIT 1";
  ...
  public static final String selectInstituteFileSize(String instituteName) {
    return "SELECT (SUM(?size) AS ?s) WHERE {?c <" + ImejiNamespaces.CREATOR
        + "> ?user . ?user <http://xmlns.com/foaf/0.1/email> ?email .filter(regex(?email, '@" + instituteName
  ...
  public static final String selectCollectionName(String uri) {
    return "SELECT (str(?name) as ?s) WHERE{<" + uri.toString() + "> <http://purl.org/dc/elements/1.1/title> ?name} LIMIT 1";
  }
