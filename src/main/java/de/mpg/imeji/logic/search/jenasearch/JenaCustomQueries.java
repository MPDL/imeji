...
public class JenaCustomQueries {
	...
	public static final String selectSubscriptionByObjectId(String objectId) {
		return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/objectId> \""
	...
	public static final String selectSubscriptionByUserId(String userId) {
		return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/userId> \""
				+ userId + "\"^^<http://www.w3.org/2001/XMLSchema#string> }";
	}
	...
	public static final String selectUserByRegistrationToken(String registrationToken) {
		return X_PATH_FUNCTIONS_DECLARATION + "  SELECT DISTINCT ?s WHERE { "
				...
				+ "\"^^<http://www.w3.org/2001/XMLSchema#string> . " + " ?s a <http://imeji.org/terms/user> }";
	}
	...
	/**
	 * Select Users to be notified by file download Note: Current <code>user</code>
	...
	public static final String selectContentByStorageId(String storageId) {
		return "SELECT ?s WHERE {?s <http://imeji.org/terms/original> ?o . filter(regex(?o,'" + storageId
				+ "'))} limit 1";
	}
	...
	public static final String selectCollectionByLogoUrl(String logoUrl) {
		return X_PATH_FUNCTIONS_DECLARATION + XSD_DECLARATION
				+ "SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/logoUrl> <" + logoUrl + ">} limit 1";
	}
	...
	public static final String selectCollectionByLogoStorageId(String logoStorageId) {
		return "SELECT ?s WHERE {?s <http://imeji.org/terms/logoUrl> ?o . filter(regex(str(?o),'" + logoStorageId
				+ "'))} limit 1";
	}

	/**
	 * Select all {@link Grant} which are not valid anymore. For instance, when a
	 * {@link User}, or an object ( {@link CollectionImeji}, {@link Album}) is
	 * deleted, some related {@link Grant} might stay in the database, even if they
	...
	public static final String updateRemoveGrantsFor(String uri) {
		return "WITH <http://imeji.org/user> DELETE {?user <http://imeji.org/terms/grant> ?s . ?s ?p ?o } "
				...
				+ " WHERE {?user <http://imeji.org/terms/grant> ?s . ?s <http://imeji.org/terms/grantFor> <" + uri
				+ "> . ?s ?p ?o}";
	}
	...
	public static final String updateAddLicensetoItem(String itemId, License l) {
		...
		return "WITH <http://imeji.org/item> " + "INSERT {<" + itemId + "> <http://imeji.org/terms/license> <"
				+ licenseId + "> . <" + licenseId + ">  <http://imeji.org/terms/name> \"" + l.getName()
				+ "\"^^<http://www.w3.org/2001/XMLSchema#string> "
				...
				+ licenseId + ">  <http://imeji.org/terms/start> -1} " + "USING <http://imeji.org/item> " + "WHERE{ <"
				+ itemId + "> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://imeji.org/terms/item>}";
	}
	...
	public static final String updateCollection(String id, String oldUri, String newUri) {
		return "WITH <http://imeji.org/item> " + "DELETE {<" + id + "> <http://imeji.org/terms/collection>  <" + oldUri
				+ ">} " + "INSERT {<" + id + "> <http://imeji.org/terms/collection>  <" + newUri + ">} "
				+ "USING <http://imeji.org/item> " + " WHERE {<" + id + "> <http://imeji.org/terms/collection>  <"
				+ oldUri + ">}";
	}
	...
	public static final String insertFileSize(String itemId, String fileSize) {
		return "WITH <http://imeji.org/item> " + "INSERT {<" + itemId + "> <http://imeji.org/terms/fileSize> "
				+ fileSize + "}" + "USING <http://imeji.org/item> " + "WHERE{<" + itemId + "> ?p ?o}";
	}
	...
	public static final String insertFileSizeAndDimension(String itemId, String fileSize, String width, String height) {
		return "WITH <http://imeji.org/item> " + "INSERT {<" + itemId + "> <http://imeji.org/terms/fileSize> "
				+ fileSize + " . <" + itemId + "> <http://www.w3.org/2003/12/exif/ns#width> " + width + " . <" + itemId
				+ "> <http://www.w3.org/2003/12/exif/ns#height> " + height + "}" + "USING <http://imeji.org/item> "
				+ "WHERE{<" + itemId + "> ?p ?o}";
	}
	...
	/**
	 * Update all {@link Item}. Remove all {@link Metadata} which doesn't have any
	 * content (no triples as subject)
	 *
	 * @return
	 */
	...
	public static final String selectContainerItemByFilename(URI containerURI, String filename) {
		...
		return "SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/filename> ?el . FILTER(regex(?el, '^" + filename
				+ "\\\\..+', 'i')) .?s <http://imeji.org/terms/collection> <" + containerURI.toString() + "> . ?s <"
				+ ImejiNamespaces.STATUS + "> ?status . FILTER (?status!=<" + Status.WITHDRAWN.getUriString()
				+ ">)} LIMIT 2";
	}
	...
	public static final String selectItemByChecksum(URI containerURI, String checksum) {
		return "SELECT DISTINCT ?s WHERE {?s <http://imeji.org/terms/checksum> \"" + checksum
				+ "\"^^<http://www.w3.org/2001/XMLSchema#string>. " + "?s <http://imeji.org/terms/collection> <"
				+ containerURI.toString() + "> . " + "?s <" + ImejiNamespaces.STATUS + "> ?status . "
				+ " FILTER (?status!=<" + Status.WITHDRAWN.getUriString() + ">)} LIMIT 1";

	}

	/**
	 * Search for all Institute of all {@link User} . An institute is defined by the
	 * emai of the {@link User}, for instance user@mpdl.mpg.de has institute
	 * mpdl.mpg.de
	 *
	 * @return
	 */
	...
	/**
	 * Search for all {@link Item} within a {@link CollectionImeji} belonging the
	 * the institute, and sum all fileSize
	 *
	 * @param instituteName
	 * @return
	 */
	...
	/**
	 * Chararters ( and ) can not be accepted in the sparql query and must therefore
	 * removed
	 *
	 * @param s
	 * @return
	...
	public static final String selectLastModifiedDate(URI id) {
		return "SELECT (str(?date) AS ?s) WHERE {<" + id.toString() + "> <" + ImejiNamespaces.LAST_MODIFICATION_DATE
				+ "> ?date}";
	}
