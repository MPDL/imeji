...
/**
 * Build a Boolean query with a list of all allowed collection and
 * subcollections for one user. <br/>
 * This query can be used to filter a search query for items or for collection.
 * <br/>
 * <br/>
 * There are some options to make the query more specific:
 * <li>Set a user
 * <li>Set the Folder (collection or subcollection) in which the query is
 * executed
...
public class SecurityQueryFactory {
	...
	/**
	 * Return the security query related to the status: Released object can be read
	 * by everybody
	 * 
	 * @return
	 */
	...
	private BoolQueryBuilder toQuery(List<String> collectionUris) {
		...
		collectionUris.stream().forEach(uri -> q.should(QueryBuilders
				.termQuery(searchForCollections ? ElasticFields.ID.field() : ElasticFields.FOLDER.field(), uri)));
		return q;
	}

	/**
	 * Return the list of all collections allowed for the user and, if defined, for
	 * for the grant
	 * 
	 * @return
	 */
	...
	private List<String> addChildren(List<String> collectionsUris) {
		return collectionsUris.stream().flatMap(uri -> new HierarchyService().addAllSubcollections(uri).stream())
	...
	private static List<String> filterByRoleAndGetCollectionUri(List<Grant> grants, GrantType role) {
		return grants.stream()
				.filter(g -> ObjectHelper.getObjectType(URI.create(g.getGrantFor())) == ObjectType.COLLECTION
