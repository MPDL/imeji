...
public class SearchMetadata extends SearchPair {
   ...
   * @param not
   */
  public SearchMetadata(String index, SearchMetadataFields f, SearchOperators operator, String value, boolean not) {
  ...
  public boolean isSame(SearchElement element) {
    if (SEARCH_ELEMENTS.METADATA == element.getType()) {
      ...
      return smd.getIndex().equals(index) && smd.getMetadataField().equals(getMetadataField()) && smd.getValue().equals(getValue())
          && smd.getOperator().equals(getOperator());
    }
    return false;
  }
