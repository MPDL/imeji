...
public abstract class DbRepository<ModelType> {
  ...
  public List<ModelType> retrieveAll() throws ImejiException {
    return inSession(em -> {
      ...
      return em.createQuery("select u from " + className + " u", classType).getResultList();
    });
  }
  ...
  public List<ModelType> retrieveByID(List<String> ids) throws ImejiException {
    return inSession(em -> {
      String className = classType.getSimpleName();
      return em.createQuery("select u from " + className + " u WHERE u.dbId IN :ids", classType).setParameter("ids", ids).getResultList();
    });
  }
  ...
  public List<String> retrieveAllIds() throws ImejiException {
    return inSession(em -> {
      String className = classType.getSimpleName();
      return em.createQuery("select u.dbId from " + className + " u", String.class).getResultList();
    });
  }
  ...
  public long countAll() throws ImejiException {
    return inSession(em -> {
      String className = classType.getSimpleName();
      Object res = em.createQuery("select count(*) from " + className + " u").getSingleResult();
