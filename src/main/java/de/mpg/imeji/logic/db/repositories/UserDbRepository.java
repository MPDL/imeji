...
public class UserDbRepository extends DbRepository<User> {
  ...
  public List<User> retrieveAllAdmins() throws ImejiException {
    return inSession(em -> {
      ...
      return em.createNativeQuery("select * from users where jsonb_exists(grants, :adminGrant);", User.class)
          .setParameter("adminGrant", adminRule).getResultList();
    });
  }
  ...
  public List<User> retrieveAllUsersForGroup(String userGroupId) throws ImejiException {
    return inSession(em -> {
      ...
      return em.createQuery("select u from User u where u.dbId IN :uIds", User.class)
          .setParameter("uIds", ug.getUsers().stream().map(uri -> uri.toString()).toList()).getResultList();
    });
  }
  ...
  public List<String> retrieveAllDomains() throws ImejiException {
    return inSession(em -> {
      //String adminRule = "ADMIN," + Imeji.PROPERTIES.getBaseURI();
      return em.createNativeQuery("SELECT DISTINCT substring(email FROM '@(.*)$') AS domain FROM users;", String.class).getResultList();
    });
  }
  ...
  public long getFileSize(String userId) throws ImejiException {
    return inSession(em -> {
      Object result = em
          .createNativeQuery(
              "select sum(filesize) from item where createdBy=:creatorId AND status='http://imeji.org/terms/status#WITHDRAWN';")
          .setParameter("creatorId", userId).getSingleResult();
      if (result == null) {
        return 0L;
      } else {
  ...
  public long getFileSizeForDomain(String mailDomain) throws ImejiException {
    return inSession(em -> {
      Object result = em.createNativeQuery(
          "SELECT sum(filesize) FROM content INNER JOIN item ON content.itemid = item.id INNER JOIN users on item.createdby = users.id WHERE users.email ilike :mailDomain")
          .setParameter("mailDomain", "%@" + mailDomain).getSingleResult();
      if (result == null) {
        return 0L;
      } else {
        return ((Number) result).longValue();
  ...
  public long getFileSizeForAll() throws ImejiException {
    return inSession(em -> {
      Object result = em.createNativeQuery("SELECT sum(filesize) FROM item").getSingleResult();
      if (result == null) {
        return 0L;
      } else {
        return ((Number) result).longValue();
