...
/**
 * Business Controller to persisted messages. This can be used to store
 * Notifications, Invitations, etc. Messages are stored in a Key/Value store
 *
 * @author bastiens
 *
 */
public class KeyValueStoreService {
	...
	public Object get(String key) throws NotFoundException {
		try {
			...
			throw new NotFoundException("Key " + key + " not found in  " + store.getName() + ": " + e.getMessage());
		}
	}

	/**
	 * Return all elements with a Key matching the Key pattern (according to REGEX
	 * Rules)
	 *
	 * @param keyPattern
	 * @return
	...
	public <T> List<T> getList(String keyPattern, Class<T> clazz) throws ImejiException {
		...
		for (final byte[] b : store.getList(keyPattern)) {
			try {
				...
				LOGGER.error("Error deserializing value " + new String(b) + " from store " + store.getName(), e);
			} catch (IOException e) {
