...
/**
 * Facade to manage to move operation on {@link Item} and
...
public class MoveFacade implements Serializable {
	 ...
	 * <li>it doens't already exists in the target collection (i.e. no items with
	 * the same checksum) <br/>
	 * <br/>
	 * <strong> Important:</strong> If the target collection is released, all items
	 ...
	 * <li>if the collection is released, the user has no admin rights on the
	 * objects
	...
	private void checkIfUserCanMoveObjectsToCollection(List<Object> objects, CollectionImeji collection, User user)
		...
		if (!authorization.update(user, collection)
			...
			throw new NotAllowedError(user.getEmail() + " is not allowed to moved the items to the collection "
	...
	/**
	 * Filter out items with a file which already exists in the target
	 * collection<br/>
	 * 
	 * @param items
	 * @param collection
	...
	private List<Item> filterAlreadyExists(List<Item> items, CollectionImeji collection) throws ImejiException {
		...
		final List<ContentVO> contents = retrieveContentBatchLazy(items).stream()
				.filter(content -> !itemService.checksumExistsInCollection(collection.getId(), content.getChecksum()))
				.collect(Collectors.toList());
		return items.stream().filter(item -> contents.stream()
	...
	private List<ContentVO> retrieveContentBatchLazy(List<Item> items) throws ImejiException {
		...
		List<String> contentIds = items.stream().map(item -> contentService.findContentId(item.getId().toString()))
				.collect(Collectors.toList());
