...
/**
 * Facade to search and retrieve {@link Item} and {@link Collection} in one
 * request
 * 
 * @author saquet
 *
...
public class SearchAndRetrieveFacade implements Serializable {
	...
	private final Search search = new ElasticSearch(SearchObjectTypes.ITEM, SearchObjectTypes.COLLECTION);
	private final ItemService itemService = new ItemService();
	private final CollectionService collectionService = new CollectionService();
	private final static Logger LOGGER = LogManager.getLogger(SearchAndRetrieveFacade.class);

	/**
	 * Search for all objects within the collection. Items and Sub-Collections are
	 * mixed
	 * 
	 * @param q
	 * @param collection
	 ...
	 * @return
	 */
	public SearchResult search(SearchQuery q, CollectionImeji collection, User user, SortCriterion sortCri, int size,
			int offset) {
	...
	public SearchResult searchWithFacetsAndMultiLevelSorting(SearchQuery q, CollectionImeji collection, User user,
		...
		return search.searchWithFacetsAndMultiLevelSorting(q, sortCriteria, user, collection.getId().toString(), offset,
				size);
	}

	/**
	 * Retrieve all objects of the uris list, which can be a mixed of items and
	 * collections
	 * 
	 * @param uris
	 * @param user
	 ...
	 */
	public List<CollectionElement> retrieveItemsAndCollections(List<String> uris, User user) throws ImejiException {
		// Split into 2 lists: one list of collection, one of items
		final Map<Boolean, List<String>> resultMap = uris.stream()
				.collect(Collectors.partitioningBy(s -> isCollectionUri(s)));
		// Retrieve objects of both list
		final List<CollectionImeji> subCollections = collectionService.retrieve(resultMap.get(true), user);
	...
	/**
	 * Retrieve all objects of the uris list, which can be a mixed of items and
	 * collections. Return the result as a list of item by transforming
	 * subcollections to pseudo items
	 * 
	 * @param uris
	 * @param user
	 * @return
	 * @throws ImejiException
	 */
	public List<Item> retrieveItemsAndCollectionsAsItems(List<String> uris, User user) throws ImejiException {
		// Split into 2 lists: one list of collection, one of items
		final Map<Boolean, List<String>> resultMap = uris.stream()
				.collect(Collectors.partitioningBy(s -> isCollectionUri(s)));
		// Retrieve objects of both list
		final Future<List<CollectionImeji>> subCollections = collectionService.retrieveAsync(resultMap.get(true), user);
	...
	/**
	 * Merge the subcollection with the items and order them according to the order
	 * of uris
	 * 
	 * @param uris
	 ...
	 * @return
	 */
	private List<CollectionElement> mergeAndOrder(List<String> uris, List<CollectionImeji> subCollections,
			List<Item> items) {
	...
	/**
	 * Merge the subcollections with the items and order them according to the order
	 * of uris. return the result as a list of item by transforming subcollections
	 * to pseudo items
	 * 
	 * @param uris
	 * @param subCollections
	 ...
	 * @return
	 */
	private List<Item> mergeAndOrderAsItem(List<String> uris, List<CollectionImeji> subCollections, List<Item> items) {
	...
	private Item toItem(CollectionImeji c) {
		return ImejiFactory.newItem().setUri(c.getId().toString()).setCollection(ObjectHelper.getId(c.getCollection()))
				.setFilename(c.getTitle()).build();
	}
