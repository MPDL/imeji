...
public class SearchAndRetrieveFacade implements Serializable {
   ...
   * @return
   */
  public SearchResult search(SearchQuery q, CollectionImeji collection, User user, SortCriterion sortCri, int size, int offset) {
  ...
  public List<CollectionElement> retrieveItemsAndCollections(List<String> uris, User user) throws ImejiException {
    ...
    final Map<Boolean, List<String>> resultMap = uris.stream().collect(Collectors.partitioningBy(s -> isCollectionUri(s)));
   ...
   * @return
   */
  private List<CollectionElement> mergeAndOrder(List<String> uris, List<CollectionImeji> subCollections, List<Item> items) {
