...
public class StatementChangedListener extends Listener {
	...
	private void updateItemIndex(String oldIndex, String newIndex, User user) throws ImejiException {
		...
		SearchQuery q = new SearchFactory().addElement(
				new SearchPair(SearchFields.index, StatementUtil.formatIndex(oldIndex)), LOGICAL_RELATIONS.AND).build();
		List<Item> items = itemService.searchAndRetrieve(q, null, user, Search.GET_ALL_RESULTS,
				Search.SEARCH_FROM_START_INDEX);
		items.stream().flatMap(item -> item.getMetadata().stream())
				.filter(md -> StatementUtil.indexEquals(oldIndex, md.getIndex())).forEach(md -> md.setIndex(newIndex));
