...
public class StatementService extends SearchServiceAbstract<Statement> {
	...
	/**
	 * Filter the list with only the existing statement and retrieve it as a list of
	 * {@link Statement}
	 *
	 * @param ids
	 * @param user
	...
	public void delete(Statement s, User user) throws ImejiException {
		if (isUsed(s)) {
			throw new NotAllowedError(
					"Statement " + s.getIndex() + " is used by at least one item and therefore can't be deleted");
		}
	...
	public boolean isUsed(Statement s) throws ImejiException {
		SearchQuery q = new SearchFactory()
				.addElement(new SearchPair(SearchFields.index, s.getIndexFormatted()), LOGICAL_RELATIONS.AND).build();
