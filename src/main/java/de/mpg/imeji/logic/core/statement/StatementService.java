...
public class StatementService extends SearchServiceAbstract<Statement> {
   ...
   * @throws ImejiException
   */
  public List<Statement> createBatchIfNotExists(List<Statement> l, User user) throws ImejiException {
   ...
   * @throws ImejiException
   */
  public List<Statement> retrieveBatchOnlyExistingStatemment(List<String> uris, User user) throws ImejiException {
    List<Statement> statements =
        filterNotExistingStatement(uris.stream().map(uri -> ImejiFactory.newStatement().setUri(URI.create(uri)).build()).collect(toList()));
    final List<Statement> l = controller.retrieveBatch(statements.stream().map(s -> s.getUri().toString()).collect(toList()), user);
    l.sort((s1, s2) -> s1.getIndex().compareToIgnoreCase(s2.getIndex()));
    return l;
  }
  ...
  public Statement update(Statement before, Statement after, User user) throws ImejiException {
    try {
      ...
      messageService.add(new StatementMessage(MessageType.STATEMENT_CHANGED, after, before.getIndex()));
    }
  ...
  private void changeDefaultStatement(Statement before, Statement after) {
    ...
    defaultStatements = defaultStatements.stream().map(s -> s = StatementUtil.indexEquals(s, before.getIndex()) ? after.getIndex() : s)
  ...
  public void delete(Statement s, User user) throws ImejiException {
    if (isUsed(s)) {
      throw new NotAllowedError("Statement " + s.getIndex() + " is used by at least one item and therefore can't be deleted");
    }
  ...
  public boolean isUsed(Statement s) throws ImejiException {
    SearchQuery q =
        new SearchFactory().addElement(new SearchPair(SearchFields.index, s.getIndexFormatted()), LOGICAL_RELATIONS.AND).build();
