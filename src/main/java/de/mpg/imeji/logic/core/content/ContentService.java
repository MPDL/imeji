...
public class ContentService extends SearchServiceAbstract<ContentVO> implements Serializable {
	...
	public List<ContentVO> createBatch(List<ItemWithStagedFile> itemWithFileList, User user) throws ImejiException {
		...
		itemWithFileList.stream()
				.forEach(item -> messageService.add(new ItemMessage(MessageType.UPLOAD_FILE, item.getItem())));
		return contents;
	}

	/**
	 * Create a ContentVO for an external file. File will not be downloaded and thus
	 * not analyzed
	 * 
	 * @param item
	...
	public ContentVO create(Item item, String externalFileUrl, User user) throws ImejiException {
		ContentVO contentVO = ImejiFactory.newContent().setItemId(item.getId().toString()).setOriginal(externalFileUrl)
				.build();
		messageService.add(new ItemMessage(MessageType.UPLOAD_FILE, item));
	...
	public ContentVO update(Item item, String externalFileUrl, User user) throws ImejiException {
		ContentVO contentVO = ImejiFactory.newContent().setId(item.getId().toString()).setOriginal(externalFileUrl)
				.build();
	...
	/**
	 * Extract the content of the file (Fulltext and technical metadata) and add it
	 * to the contentVO. This method is asynchronous
	 *
	 * @param contentVO
	 * @return
	 * @throws ImejiException
	...
	/**
	 * Extract the content of the File of the contentVO. Return true if th
	 * extraction happened
	 *
	 * @param contentVO
	 * @return
		...
		public Integer call() throws Exception {
			if (!StringHelper.isNullOrEmptyTrim(contentVO.getId())) {
				try {
					...
					LOGGER.warn("Error extracting fulltext/metadata from file of file " + contentVO.getItemId()
