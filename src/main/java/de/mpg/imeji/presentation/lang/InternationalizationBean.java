...
public class InternationalizationBean implements Serializable {
  ...
  private void readLocaleFromCookie() {
    this.locale = Locale.forLanguageTag(CookieUtils.readNonNull(lANGUAGE_COOKIE, getRequestedLocale().getLanguage()));
  }
  ...
  private List<SelectItem> getsupportedLanguages(boolean supported) {
    ...
    for (final SelectItem iso : isolanguages) {
      if (supported && isSupported(iso.getValue().toString()) || (!supported && !isSupported(iso.getValue().toString()))) {
        l.add(iso);
      }
    }
    return l;
