...
public class InternationalizationBean implements Serializable {
	...
	/**
	 * Menu with first, the supported languages out of the properties, second all
	 * the iso languages
	 *
	...
	/**
	 * If the user already set a lang cookie, return its value, else check the lang
	 * in the request (browser dependant)
	 *
	 * @return
	 */
	private void readLocaleFromCookie() {
		this.locale = Locale
				.forLanguageTag(CookieUtils.readNonNull(lANGUAGE_COOKIE, getRequestedLocale().getLanguage()));
	}

	/**
	 * Get the Locale according the user request and to the supported languages in
	 * the Configuration. If no valid local could be found, return English
	 *
	 * @return
	 */
	...
	/**
	 * True if a language (defined in iso639_1) is supported in imeji (according to
	 * the properties)
	 *
	 * @param langString
	 * @return
	...
	/**
	 * Return: <br/>
	 * - the supported languages (i.e languages with a translation for labels and
	 * messages) if the parameter is set to true <br/>
	...
	/**
	 * Method called when the user changed the language. The new language is setted
	 * via the listener. The method reload the current page
	 *
	 * @return
	 * @throws IOException
	 */
