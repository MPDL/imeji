...
public class LicenseEditor implements Serializable {
	...
	public LicenseEditor(Locale locale, Item item) {
		...
		if (!showInput) {
			this.licenseName = active == null ? Imeji.RESOURCE_BUNDLE.getLabel(NO_LICENSE, locale) : active.getName();
		} else {
	...
	public void licenseInputListener() {
		// just an empty listener which is triggered by the component to be sure that
		// changed value are
		// caught
	}
	...
	/**
	 * @param customLicenseName
	 *            the customLicenseName to set
	 */
	...
	/**
	 * @param customLicenseUrl
	 *            the customLicenseUrl to set
	 */
