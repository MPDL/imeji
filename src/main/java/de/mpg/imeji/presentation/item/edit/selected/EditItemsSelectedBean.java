...
public class EditItemsSelectedBean extends EditMetadataAbstract {
	...
	private void initHeaders(List<Item> items) {
		...
		headers = headerMap.values().stream()
				.sorted((c1, c2) -> c1.getStatement().getIndex().compareToIgnoreCase(c2.getStatement().getIndex()))
				.collect(toList());
	...
	public List<Statement> getAllStatements() {
		return rows.stream().flatMap(row -> row.getCells().stream()).filter(cell -> cell.getInputs() != null)
				.collect(Collectors.toMap(CellComponent::getIndex, cell -> cell.getStatement(), (a, b) -> a)).values()
	...
	public List<SelectItem> getFilteredStatementMenu() {
		return getStatementMenu().stream()
				.filter(s -> !headers.stream()
						.filter(h -> StatementUtil.indexEquals(h.getStatement().getIndex(), s.getLabel())).findAny()
						.isPresent())
				.collect(toList());
	}
	...
	public String getBackUrl() {
		return !StringHelper.isNullOrEmptyTrim(super.getBackUrl())
				? super.getBackUrl()
	...
	private List<Item> retrieveItems() throws ImejiException {
		return (List<Item>) itemService.retrieveBatch(selectedItemsIds, Search.GET_ALL_RESULTS,
	...
	private void initPageList() {
		...
		int end = start + paginationLength < getTotalNumberOfPages()
				? start + paginationLength
