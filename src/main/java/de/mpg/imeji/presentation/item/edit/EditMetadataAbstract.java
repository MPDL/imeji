...
public abstract class EditMetadataAbstract extends SuperBean {
	...
	public EditMetadataAbstract() {
		...
		try {
			statementMap = StatementUtil.statementListToMap(statementService.searchAndRetrieve(null, null,
	...
	private List<Statement> retrieveInstanceDefaultStatements() throws ImejiException {
		return statementService.retrieveBatch(StatementUtil.toStatementUriList(Imeji.CONFIG.getStatements()),
				Imeji.adminUser);
	}
	...
	/**
	 * Convert the List of {@link ItemMetadataInputComponent} to a list of
	 * {@link Item}
	 *
	 * @return
	 */
	...
	/**
	 * Return all Statement which are used by at least one metadata in at least one
	 * item
	 * 
	 * @return
	 */
