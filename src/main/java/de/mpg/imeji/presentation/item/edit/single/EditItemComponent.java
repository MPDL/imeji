...
public class EditItemComponent extends EditMetadataAbstract {
	...
	public EditItemComponent(Item item, User user, Locale locale) throws ImejiException {
		...
		Map<String, EditItemEntry> entriesMap = getDefaultStatements().values().stream().collect(toMap(
				Statement::getIndex,
				s -> new EditItemEntry(new MetadataFactory().setStatementId(s.getIndex()).build(), statementMap)));
	...
	public List<Statement> getAllStatements() {
		return entries.stream().filter(row -> row.getInput() != null && !row.getInput().isEmpty())
				.map(EditItemEntry::getInput).map(MetadataInputComponent::getStatement).filter(s -> s != null)
	 ...
	 * 
	 * @param sourceIndex
	 *            index in list of the source element that got dragged
	 * @param targetIndex
	 *            index in list of the element on which the source element got
	 *            dropped source element gets added behind the element on which it
	 *            was dropped
	 */
