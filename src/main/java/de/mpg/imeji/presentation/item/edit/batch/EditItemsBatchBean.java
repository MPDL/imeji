...
public class EditItemsBatchBean extends EditMetadataAbstract {
  ...
  private void initCollection() {
    try {
      collection = new CollectionService().retrieveLazy(ObjectHelper.getURI(CollectionImeji.class, collectionId), getSessionUser());
  ...
  public void fill() {
    try {
      retrieveItems();
      items.stream().filter(item -> item.getMetadata().stream().noneMatch(md -> md.getIndex().equals(statementSelector.getIndex())))
  ...
  public void overwrite() {
    try {
      retrieveItems();
      items.stream()
          .peek(item -> item.setMetadata(
              item.getMetadata().stream().filter(md -> !md.getIndex().equals(statementSelector.getIndex())).collect(Collectors.toList())))
  ...
  private void retrieveItems() throws ImejiException, IOException {
    ...
    if (collectionId != null) {
      items = itemService.searchAndRetrieve(ObjectHelper.getURI(CollectionImeji.class, collectionId), q, null, getSessionUser(),
