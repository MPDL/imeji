...
public class MoveItemsBean extends SuperBean {
	...
	public void searchCollectionsForMove(String collectionSrcId) throws ImejiException {
		...
		collectionsForMove = new CollectionService()
				.searchAndRetrieve(factory.build(), new SortCriterion(SearchFields.modified, SortOrder.DESCENDING),
	...
	/**
	 * Move an object (Item of Collection) to the target collection. If Id is null,
	 * move currently selected Items
	 * 
	 * @param collectionId
	 * @param id
	 * @throws IOException
	...
	/**
	 * ove an object (Item of Collection) to a new Subcollection. If Id is null,
	 * move currently selected Items
	 * 
	 * @param id
	 * @throws IOException
	...
	/**
	 * Move an object (Item of Collection) to a new Subcollection. If Id is null,
	 * move currently selected Items
	 * 
	 * @param id
	 * @param name
	 ...
	 * @throws ImejiException
	 */
	public void moveToNewSubCollection(String id, String name, CollectionImeji c) throws IOException, ImejiException {
		CollectionImeji subcollection = new CollectionService().create(ImejiFactory.newCollection().setTitle(name)
				.setPerson(getSessionUser().getPerson()).setCollection(c.getId().toString()).build(), getSessionUser());
	...
	private void moveItems(CollectionImeji col, List<String> ids) {
		try {
			List<Item> items = (List<Item>) itemService.retrieveBatchLazy(ids, Search.GET_ALL_RESULTS,
					Search.SEARCH_FROM_START_INDEX, getSessionUser());
			List<Item> moved = itemService.moveItems(items, col, getSessionUser(), licenseEditor.getLicense());
			if (moved.size() > 0) {
	...
	private void moveCollection(String collectionId, CollectionImeji target) {
		CollectionImeji collection = collectionsForMove.stream().filter(c -> c.getId().toString().equals(collectionId))
				.findAny().get();
		try {
	...
	public boolean isChildOf(Node node, String collectionId) {
		return node.hasParent() && hierarchyService.isChildOf(node.getCollection().getId().toString(), collectionId);
	}
	...
	public boolean isCollection(String objectId) {
		return objectId != null && ObjectHelper.getObjectType(URI.create(objectId)) == ObjectType.COLLECTION;
	}
	...
	/**
	 * @param newSubcollectionName
	 *            the newSubcollectionName to set
	 */
	...
	/**
	 * @param toNewCollection
	 *            the toNewCollection to set
	 */
