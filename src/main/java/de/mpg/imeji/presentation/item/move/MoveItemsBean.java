...
public class MoveItemsBean extends SuperBean {
  ...
  public void initMove(String collectionSrcId, boolean allObjectsToMoveHaveALicense) throws ImejiException {
    ...
    collectionsForMove =
        new CollectionService().searchAndRetrieve(factory.build(), new SortCriterion(SearchFields.modified, SortOrder.DESCENDING),
            getSessionUser(), Search.GET_ALL_RESULTS, Search.SEARCH_FROM_START_INDEX).stream().collect(Collectors.toList());
  ...
  public void moveToNewSubCollection(String id, String name, CollectionImeji c) throws IOException, ImejiException {
    CollectionImeji subcollection = new CollectionService().create(
        ImejiFactory.newCollection().setTitle(name).setPerson(getSessionUser().getPerson()).setCollection(c.getId().toString()).build(),
        getSessionUser());
  ...
  private void moveItems(CollectionImeji col, List<String> ids) {
    try {
      List<Item> items =
          (List<Item>) itemService.retrieveBatchLazy(ids, Search.GET_ALL_RESULTS, Search.SEARCH_FROM_START_INDEX, getSessionUser());
  ...
  public void moveItem(String id, CollectionImeji c) throws IOException {
    ...
    redirect(getNavigation().getCollectionUrl() + ObjectHelper.getId(c.getId()) + "/item/" + ObjectHelper.getId(URI.create(id)));
  }
  ...
  private void moveCollection(String collectionId, CollectionImeji target) {
    CollectionImeji collection = collectionsForMove.stream().filter(c -> c.getId().toString().equals(collectionId)).findAny().get();
    try {
