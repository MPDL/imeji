...
public class ItemsEditLicenseBean extends SuperBean {
	...
	public void save() throws ImejiException, IOException {
		List<Item> items = !StringHelper.isNullOrEmptyTrim(collectionId)
				? retrieveAllCollectionsItem(collectionId)
				: retrieveSelectedItems();
		items = addLicense(items);
		save(items);
		BeanHelper.addMessage(getLicenseName() + " " + Imeji.RESOURCE_BUNDLE.getLabel("licenses_added_to", getLocale())
	...
	private String getLicenseName() {
		...
		return StringHelper.isNullOrEmptyTrim(licenseEditor.getLicense().getLabel())
				? licenseEditor.getLicense().getUrl()
	...
	private List<Item> retrieveSelectedItems() throws ImejiException {
		...
		return (List<Item>) itemService.retrieveBatch(selectedItems, Search.GET_ALL_RESULTS,
	...
	private List<Item> retrieveAllCollectionsItem(String collectionId) throws ImejiException {
		final ItemService itemService = new ItemService();
		final List<String> uris = itemService.search(ObjectHelper.getURI(CollectionImeji.class, collectionId), null,
				null, getSessionUser(), Search.GET_ALL_RESULTS, Search.SEARCH_FROM_START_INDEX).getResults();
		return (List<Item>) itemService.retrieveBatch(uris, Search.GET_ALL_RESULTS, Search.SEARCH_FROM_START_INDEX,
				getSessionUser());
	}
	...
	/**
	 * @param overwriteLicenses
	 *            the overwriteLicenses to set
	 */
	...
	/**
	 * @param releasedCollection
	 *            the releasedCollection to set
	 */
