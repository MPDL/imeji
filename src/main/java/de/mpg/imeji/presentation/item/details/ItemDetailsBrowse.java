...
public class ItemDetailsBrowse implements Serializable {
	...
	/**
	 * Object for the browsing over the detail items. The Browsing is based on a
	 * ItemsBean and the current {@link Item}.
	 *
	 * @param imagesBean
	 * @param item
	 * @param type
	...
	public ItemDetailsBrowse(Item item, String type, String containerUri, User user) {
		try {
			this.query = URLEncoder.encode(UrlHelper.hasParameter("q") ? UrlHelper.getParameterValue("q") : "",
					"UTF-8");
			this.facetQuery = URLEncoder.encode(UrlHelper.hasParameter("fq") ? UrlHelper.getParameterValue("fq") : "",
					"UTF-8");
			this.filterQuery = URLEncoder
					.encode(UrlHelper.hasParameter("filter") ? UrlHelper.getParameterValue("filter") : "", "UTF-8");
		...
		this.currentPosition = UrlHelper.hasParameter("pos")
				? Integer.parseInt(UrlHelper.getParameterValue("pos"))
				: -1;
	...
	/**
	 * Initialize sort criteria for items align this with
	 * ItemsBean.getSortCriteriaForItems()
	 *
	 * @return
	 */
	private List<SortCriterion> initSortCriteria() {
		...
		final String orderFieldName = CookieUtils.readNonNull(ItemsBean.ITEM_SORT_ORDER_COOKIE,
	...
	/**
	 * Initialize the {@link ItemDetailsBrowse} for the current {@link Item}
	 * according to:
	 *
	 * @param type
	 *            - if the detail page is initialized within a collection, an album,
	 *            or a browse page (item)
	 * @param path
	 *            - the id (not the uri) of the current container ({@link Album} or
	...
	public void init(List<String> items, String type, String containerUri) throws UnsupportedEncodingException {
		...
		if (nextItem != null) {
			next = baseUrl + ObjectHelper.getId(URI.create(nextItem)) + "?q=" + query + "&fq=" + facetQuery + "&filter="
					+ filterQuery + "&pos=" + (this.currentPosition + 1);
		}
		if (previousItem != null) {
			previous = baseUrl + ObjectHelper.getId(URI.create(previousItem)) + "?q=" + query + "&fq=" + facetQuery
					+ "&filter=" + filterQuery + "&pos=" + (this.currentPosition - 1);
		}
	}
