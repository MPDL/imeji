...
public class ItemsBean extends SuperPaginatorBean<ThumbnailBean> {
	...
	/**
	 * Initialization which are specific for this bean. Can be overriden by other
	 * beans
	 */
	...
	public void initSortMenu() {
		try {
			...
			setSelectedSortOrder(SortOrder
					.valueOf(CookieUtils.readNonNull(ITEM_SORT_ORDER_COOKIE, SortOrder.DESCENDING.name())).name());
			...
			getSortMenu().add(
					new SelectItem(SearchFields.filename, Imeji.RESOURCE_BUNDLE.getLabel("filename", getLocale())));
			getSortMenu().add(
					new SelectItem(SearchFields.filesize, Imeji.RESOURCE_BUNDLE.getLabel("file_size", getLocale())));
	...
	public void initElementsPerPageMenu() {
		setElementsPerPage(Integer.parseInt(CookieUtils.readNonNull(SuperPaginatorBean.numberOfItemsPerPageCookieName,
				Integer.toString(DEFAULT_ELEMENTS_PER_PAGE))));
		setElementsPerPageSelectItems(
				Stream.of("12,24,48,96".split(",")).map(s -> new SelectItem(s)).collect(Collectors.toList()));
	}
	...
	public Collection<Item> loadItems(List<String> uris) throws ImejiException {
		...
		return itemService.retrieveBatch(uris, Search.GET_ALL_RESULTS, Search.SEARCH_FROM_START_INDEX,
				getSessionUser());
	}

	/**
	 * Clean the list of select {@link Item} in the session if the selected images
	 * context is not "pretty:browse"
	 */
	...
	/**
	 * Get sort criteria for Items page Two-level sorting: (First level) Sort all
	 * items by chosen sort criterion (date modified, filename, file size, file
	 * type) (Second level) sort all items that fall into the same category by
	...
	public List<String> searchAllItems() {
		return new ItemService()
				.search(searchQuery, null, getSessionUser(), Search.GET_ALL_RESULTS, Search.SEARCH_FROM_START_INDEX)
				.getResults();
	}
	...
	private void withdraw(List<String> uris) throws ImejiException {
		...
		final Collection<Item> items = new ItemService().retrieveBatch(uris, Search.GET_ALL_RESULTS,
				Search.SEARCH_FROM_START_INDEX, getSessionUser());
	...
	private void delete(List<String> uris) {
		try {
			...
			final Collection<Item> items = controller.retrieveBatch(uris, Search.GET_ALL_RESULTS,
					Search.SEARCH_FROM_START_INDEX, getSessionUser());
	...
	private Collection<Item> findSelectedDeletableItems() {
		...
		try {
			Collection<Item> selectedItems = itemService.retrieveBatch(selectedItemsUris, Search.GET_ALL_RESULTS,
					Search.SEARCH_FROM_START_INDEX, getSessionUser());
			Collection<Item> deletableItems = selectedItems.stream()
					...
					.filter(item -> (item.getStatus() == Status.PENDING)).collect(Collectors.toList());
	...
	public void deleteSelectedDeletableItems() {
		...
		List<String> deletableItemsUris = deletableItems.stream().map(item -> item.getUri())
