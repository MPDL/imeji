...
public class BeanHelper {
	...
	private static synchronized Object addRequestBean(final Class<?> cls, String name) {
		final Object result = FacesContext.getCurrentInstance().getExternalContext().getRequestMap().get(name);
		if (result != null) {
			return result;
		}
		try {
	...
	private static synchronized Object addSessionBean(final Class<?> cls, String name) {
		final Object result = FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get(name);
		if (result != null) {
			return result;
		}
		try {
	...
	private static synchronized Object addApplicationBean(final Class<?> cls, String name) {
		final Object result = FacesContext.getCurrentInstance().getExternalContext().getApplicationMap().get(name);
		if (result != null) {
			return result;
		}
		try {
	...
	/**
	 * Remove a Bean from the application map. Can be used to force a bean to be
	 * reinitialized
	 *
	 * @param cls
	 */
	...
	/**
	 * @param component
	 *            associated <code>UIComponent</code>
	 * @param summary
	 *            summary text
	 */
