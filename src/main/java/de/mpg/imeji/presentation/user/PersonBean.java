...
public class PersonBean extends SuperBean implements Serializable {
	...
	public String changeOrga(Object bean, int positionUser, int positionOrga) {
		...
		if (bean instanceof UserCreationBean) {
			final List<Organization> l = (List<Organization>) ((UserCreationBean) bean).getUser().getPerson()
					.getOrganizations();
			...
			final List<Organization> l = (List<Organization>) ((UserBean) bean).getUser().getPerson()
					.getOrganizations();
			l.set(positionOrga, orga);
		} else if (bean instanceof RegistrationBean) {
			final List<Organization> l = (List<Organization>) ((RegistrationBean) bean).getUser().getPerson()
					.getOrganizations();
			l.set(positionOrga, orga);
		}
		return ":";
