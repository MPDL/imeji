...
public class UsersBean extends SuperBean {
	...
	/**
	 * If the parameter group in the url is not null, try to retrieve this group.
	 * This happens when the admin want to add a {@link User} to a {@link UserGroup}
	 */
	...
	public String deleteUser() {
		final String email = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap()
				.get("email");
		final UserService controller = new UserService();
		try {
	...
	/**
	 * Add a {@link User} to a {@link UserGroup} and then redirect to the
	 * {@link UserGroup} page
	 *
	 * @param hasgrant
	 */
