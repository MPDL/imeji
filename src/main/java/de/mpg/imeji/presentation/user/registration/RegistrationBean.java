  ...
  public void register() throws IOException {
    ...
    if (registration_success) {
      ...
      if (FacesContext.getCurrentInstance().getMessageList().size() > 1) {
        BeanHelper.cleanMessages();
        BeanHelper
            .info("User account has been registered, but verification email could not be sent! Please contact service administrators!");
      }
  ...
  private void sendRegistrationEmail(String url, String exprirationDate) {
    ...
    try {
      ...
      emailClient.sendMail(getUser().getEmail(), Imeji.CONFIG.getEmailServerSender(),
          EmailMessages.getEmailOnRegistrationRequest_Subject(getLocale()), EmailMessages.getEmailOnRegistrationRequest_Body(getUser(), url,
              Imeji.CONFIG.getContactEmail(), exprirationDate, getLocale(), getNavigation().getRegistrationUrl()));
    } catch (final Exception e) {
