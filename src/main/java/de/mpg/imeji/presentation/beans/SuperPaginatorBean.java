...
public abstract class SuperPaginatorBean<ListElementType> extends SuperBean {
  ...
  public void readUrlParameters() {
    ...
    if (FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().containsKey("el")) {
      setElementsPerPage(Integer.parseInt(FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("el")));
    }
  }
  ...
  public String getSelectedSortCriterionLabel() {
    Optional<SelectItem> selected = sortMenu.stream().filter(i -> i.getValue().toString().equals(selectedSortCriterion)).findAny();
