...
/**
 * This abstract bean class is used to manage lists with one or two paginators.
 * It can work together with different BaseListRetrieverRequestBeans that are
 * responsible to retrieve the list elements. On a jsp page, at first the
 * BaseListRetrieverRequestBean has to be initialized. It then automatically
 * updates the list in this bean (by calling update) whenever necessary and
 * reads required GET parameters (via readOutParamaters()). This bean has to be
 * managed in the session scope of JSF. The list only refreshes if any GET
 * parameters have changed or new parameters have been added. If you want to
 * refresh the list anyway, please call the hasChanged method.
 *
 * @author Markus Haarlaender (initial creation)
 * @author $Author: haarlaender $ (last modification)
 * @version $Revision: 3185 $ $LastChangedDate: 2009-11-20 14:30:15 +0100 (Fri,
 *          20 Nov 2009) $
 * @param <ListElementType>
 *            The Type of the list elements managed by this bean
 * @param <FilterType>
 *            The type of filters managed by this bean that are usable for every
...
public abstract class SuperPaginatorBean<ListElementType> extends SuperBean {
	...
	/**
	 * The total number of elements that are in the complete list (without any limit
	 * or offset filters). corresponding BaseListRetrieverRequestBean.
	 */
	...
	public void readUrlParameters() {
		if (FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().containsKey("page")) {
			currentPageNumber = Integer.parseInt(
		...
		if (FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().containsKey("el")) {
			setElementsPerPage(Integer.parseInt(
					FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("el")));
		}
	}
	...
	/**
	 * This method is called by the corresponding BaseListRetrieverRequestBean
	 * whenever the list has to be updated. It reads out basic parameters and calls
	 * readOutParamters on implementing subclasses. It uses the
	 * BaseListRetrieverRequestBean in order to retrieve the new list and finally
	 * calls listUpdated on implementing subclasses.
	 */
	public void update() {
		try {
	...
	public void setCookieElementPerPage() {
		CookieUtils.updateCookieValue(numberOfItemsPerPageCookieName, Integer.toString(elementsPerPage));
	}
	...
	/**
	 * Returns a list with the paginator pages. Used from jsf to iterate over the
	 * numbers
	 *
	 * @return
	 */
	...
	/**
	 * Returns the number of the paginator page button that should be displayed as
	 * first button of the paginator in order to always display exactly seven
	...
	/**
	 * Inner class pf which an instance represents an paginator button. Used by the
	 * iterator in jsf.
	 *
	 * @author Markus Haarlaender (initial creation)
	 * @author $Author: haarlaender $ (last modification)
	...
	public class PaginatorPage {
		...
		public String goToPage() {
			try {
				if (getNumber() > 0 && getNumber() <= getPaginatorPageSize()) {
	...
	/**
	 * WARNING: USE THIS METHOD ONLY FROM UPPER GO TO INPUT FIELD MENU IN JSPF. For
	 * setting the value manually, use setGoToPage().
	 *
	...
	/**
	 * WARNING: USE THIS METHOD ONLY FROM UPPER GO TO INPUT FIELD MENU IN JSPF. For
	 * getting the value manually, use getGoToPage().
	 *
	 * @param elementsPerPageTop
	 */
	...
	/**
	 * Whenever this method is called, an updated list with elements of type
	 * ListelementType has to be returned
	 *
	 * @param offset
	 *            An offset from where the list must start (0 means at the beginning
	 *            of all records)
	 * @param limit
	 *            The length of the list that has to be returned. If the whole list
	 *            has less records than this paramter allows, a smaller list can be
	 *            returned.
	 * @param additionalFilters
	 *            Additional filters that have to be included when retrieving the
	 *            list.
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Must return the total size of the retrieved list without limit and offset
	 * parameters. E.g. for a search the whole number of search records
	 *
	 * @return The whole number of elements in the list, regardless of limit and
	 *         offset parameters
	 */
