...
public class AdminBean extends SuperBean {
	...
	/**
	 * Reindex the Files, i.e., parse the fulltext and the technicale metadata and
	 * index it
	 *
	 * @return
	 */
	...
	private void cleanSubscriptions() throws ImejiException {
		...
		for (Subscription s : subscriptions) {
			...
			if (c != null && user != null && !SecurityUtil.authorization().read(user, c)) {
				LOGGER.error("User " + s.getUserId() + " is not allowed to subscribe to collection " + collectionUri
