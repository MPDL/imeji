...
public class SPARQLEndpointServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    if (!"".equals(q) && session.getUser() != null && SecurityUtil.authorization().isSysAdmin(session.getUser())) {
      ...
      resp.sendError(HttpServletResponse.SC_FORBIDDEN, "imeji security: You need administration priviliges");
    } else if (SecurityUtil.authorization().isSysAdmin(session.getUser())) {
      resp.sendError(HttpServletResponse.SC_UNAUTHORIZED, "imeji security: You need to be signed-in");
    }
  }
