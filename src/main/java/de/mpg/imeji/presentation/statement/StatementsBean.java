...
public class StatementsBean extends SuperBean {
  ...
  public void delete() {
    try {
      final String uri = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("uri");
  ...
  public String getDefaultStatementsString() {
    return defaultStatements.stream().distinct().filter(s -> statements.containsKey(s)).map(s -> statements.get(s).getIndex())
