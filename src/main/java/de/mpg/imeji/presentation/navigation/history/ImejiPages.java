...
public enum ImejiPages {
	 ...
	 */
	ITEM_DETAIL("", ".*/item/([A-Za-z0-9-_]+).*"), COLLECTIONS("history_collections", "/collections"), COLLECTION(
			"collection", ".*/collection/([A-Za-z0-9-_]+).*"), ADMINISTRATION("admin", "/admin"), USER("user",
					".*/user\\?id=(.*)&?"), USERS("admin_info_users", "/users"), USER_GROUP_CREATE(
							"admin_userGroup_new",
							"/createusergroup"), USER_CREATE("admin_user_new", "/createuser"), USER_GROUP(
