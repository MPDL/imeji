...
public class CreateFacetBean extends SuperBean {
	...
	public List<Statement> retrieveStatements() throws ImejiException {
		return new StatementService().retrieveAll().stream().filter(s -> s.getType() != StatementType.GEOLOCATION)
	...
	public List<String> getTypeList() {
		return Stream.of(StatementType.values()).filter(t -> t != StatementType.GEOLOCATION).map(Enum::name)
				.collect(Collectors.toList());
	}
	...
	public enum SystemFacets {
		COLLECTION("Collection", SearchFields.collection.name(), StatementType.TEXT.name()), AUTHORS_OF_COLLECTION(
				"Collection's authors", SearchFields.collection_author.getIndex(),
