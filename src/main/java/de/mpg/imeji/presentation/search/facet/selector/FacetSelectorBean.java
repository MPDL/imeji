...
public class FacetSelectorBean extends SuperBean {
  ...
  public List<FacetSelectorEntryValue> getSelectedValues() {
    return entries.stream().flatMap(e -> e.getValues().stream()).filter(v -> v.isSelected()).collect(Collectors.toList());
  }
  ...
  private String createAddQuery(FacetSelectorEntryValue entryValue) {
    try {
      return getCurrentPage().copy()
          .setParamValue("fq", SearchQueryParser.transform2URL(factory.clone().and(entryValue.getEntryQuery()).build())).getCompleteUrl();
  ...
  private String createRemoveQuery(FacetSelectorEntryValue entryValue) {
    return getCurrentPage().copy()
        .setParamValue("fq", SearchQueryParser.transform2URL(factory.clone().remove(entryValue.getEntryQuery()).build())).getCompleteUrl();
  }
