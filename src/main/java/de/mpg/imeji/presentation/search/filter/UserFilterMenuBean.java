...
public class UserFilterMenuBean extends SuperFilterMenuBean {
	...
	private SearchQuery getCreatedByMeQuery() throws UnprocessableError {
		return SearchQuery.toSearchQuery(
				new SearchPair(SearchFields.creator, SearchOperators.EQUALS, getSessionUser().getEmail(), false));
	}
	...
	private SearchQuery getSharedWithMeQuery() {
		return SearchQuery.toSearchQuery(
				new SearchPair(SearchFields.collaborator, SearchOperators.EQUALS, getSessionUser().getEmail(), false));
	}
