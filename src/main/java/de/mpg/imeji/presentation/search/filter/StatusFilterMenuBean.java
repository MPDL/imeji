...
public class StatusFilterMenuBean extends SuperFilterMenuBean {
	...
	private List<SelectItem> initMenu(boolean isCollectionFilter) throws UnprocessableError {
		...
		menu.add(new SelectItem(
				SearchQuery.toSearchQuery(new SearchPair(SearchFields.status, SearchOperators.EQUALS, "public", false)),
				Imeji.RESOURCE_BUNDLE.getLabel("published", getLocale())));
		menu.add(new SelectItem(
				SearchQuery
						.toSearchQuery(new SearchPair(SearchFields.status, SearchOperators.EQUALS, "discarded", false)),
