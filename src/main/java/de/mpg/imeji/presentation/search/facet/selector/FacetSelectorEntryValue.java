...
public class FacetSelectorEntryValue implements Serializable, Comparable<FacetSelectorEntryValue> {
	...
	public FacetSelectorEntryValue(FacetResultValue facetResultValue, Facet facet, SearchQuery facetsQuery,
			Locale locale) {
	...
	private void initRangeEntry(FacetResultValue resultValue, Facet facet, SearchQuery facetsQuery) {
		...
		this.min = facet.getType().equals(StatementType.DATE.name())
				? DateFormatter.format(resultValue.getMin())
				: resultValue.getMin();
		this.max = facet.getType().equals(StatementType.DATE.name())
				? DateFormatter.format(resultValue.getMax())
				: resultValue.getMax();
		if (label == null) {
	...
	/**
	 * Read the value of the facet according to the query (and not from the facet
	 * result value)
	 * 
	 * @param resultValue
	 * @param facet
	...
	private SearchQuery buildMetadataDateQuery(Facet facet, String value) throws UnprocessableError {
		String mdIndex = facet.getIndex().replace("md.", "").split("\\.")[0];
		SearchMetadata smd = new SearchMetadata(mdIndex, SearchMetadataFields.date, SearchOperators.EQUALS, value,
				false);
	...
	private SearchQuery buildMetadataNumberQuery(Facet facet, String value) throws UnprocessableError {
		String mdIndex = facet.getIndex().replace("md.", "").split("\\.")[0];
		SearchMetadata smd = new SearchMetadata(mdIndex, SearchMetadataFields.number, SearchOperators.EQUALS, value,
				false);
		return new SearchFactory().and(smd).build();
	}
	...
	private SearchQuery buildSystemQuery(Facet facet, String value) {
		try {
			return new SearchFactory()
					.addElement(new SearchPair(SearchFields.valueOfIndex(facet.getIndex()), value), AND).build();
	...
	/**
	 * Compare FacetSelectorEntryValues for construction a sort order First level:
	 * sort by document count, descending Second level: sort by label,
	 * alphabetically
	 */
	@Override
