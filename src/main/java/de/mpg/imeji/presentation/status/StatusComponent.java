...
public class StatusComponent extends UINamingContainer {
  ...
  private CollectionImeji getLastParent(Properties p) {
    ...
    return uri != null ? ImejiFactory.newCollection().setUri(uri).setCreatedBy(findOwner(uri)).build() : (CollectionImeji) p;
  }
  ...
  private List<User> findAllUsersWithReadGrant(Properties p) {
    ...
    if (p instanceof Item) {
      l.addAll(uc.searchAndRetrieveLazy(getReadQuery(((Item) p).getCollection().toString()), null, Imeji.adminUser, 0,
          COLLABORATOR_LIST_MAX_SIZE));
    } else {
      l.addAll(uc.searchAndRetrieveLazy(getReadQuery(p.getId().toString()), null, Imeji.adminUser, 0, COLLABORATOR_LIST_MAX_SIZE));
    }
    return l;
  }
  ...
  private List<UserGroup> findAllGroupsWithReadGrant(Properties p) {
    ...
    final List<UserGroup> l =
        ugc.searchAndRetrieveLazy(getReadQuery(p.getId().toString()), null, Imeji.adminUser, 0, COLLABORATOR_LIST_MAX_SIZE);
    if (p instanceof Item) {
      l.addAll(ugc.searchAndRetrieveLazy(getReadQuery(((Item) p).getCollection().toString()), null, Imeji.adminUser, 0,
          COLLABORATOR_LIST_MAX_SIZE));
    }
    return l;
  }
