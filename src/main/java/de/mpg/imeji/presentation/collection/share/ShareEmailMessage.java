...
public class ShareEmailMessage {
	 ...
	 * @param session
	 */
	public ShareEmailMessage(String addresseeName, String sharedObjectName, String sharedObjectLink, String shareToUri,
			String role, User user, Locale locale) {
		this.user = user;
		this.locale = locale;
		body = EmailMessages.getSharedCollectionMessage(user.getPerson().getFirstnameLastname(), addresseeName,
				sharedObjectName, sharedObjectLink, locale);
