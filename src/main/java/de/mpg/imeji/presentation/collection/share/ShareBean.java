...
public class ShareBean extends SuperBean implements Serializable {
  ...
  public void shareWithGroup() throws ImejiException {
    final ShareListItem groupListItem = new ShareListItem(userGroup, uri.toString(), null, getSessionUser(), getLocale());
  ...
  private void sendEmail(String email, String subject, String body) {
    try {
      new EmailService().sendMail(email, null, EmailMessages.replaceInstanceNameVariable(subject, instanceName), body);
    } catch (final Exception e) {
  ...
  private void sendEmailForShare(ShareListItem item, String subject) {
    for (final User subscribingUser : item.getUsers()) {
      final ShareEmailMessage emailMessage = new ShareEmailMessage(subscribingUser.getPerson().getFirstnameLastname(), title,
          getLinkToSharedObject(), getShareToUri(), item.getRole(), getSessionUser(), getLocale());

      sendEmail(subscribingUser.getEmail(), EmailMessages.replaceInstanceNameVariable(subject, instanceName), emailMessage.getBody());
    }
  }
