		...
		for (final String grantString : user.getGrants()) {
			try {
				...
				if (grant.getGrantFor().contains("/collection/")) {
					final CollectionImeji col = collectionController.retrieveLazy(URI.create(grant.getGrantFor()),
							sessionUser);
					if (!col.isSubCollection()) {
						roles.add(new ShareListItem(user, col.getId().toString(), col.getTitle(), sessionUser, locale,
								false));
					}
				}
