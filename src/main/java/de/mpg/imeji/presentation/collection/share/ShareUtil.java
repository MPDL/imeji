...
public class ShareUtil {
   ...
   * @throws Exception
   */
  public static List<ShareListItem> getAllRoles(User user, User sessionUser, Locale locale) throws ImejiException {
    ...
    for (final String grantString : user.getGrants()) {
      try {
        ...
        if (grant.getGrantFor().contains("/collection/")) {
          final CollectionImeji col = collectionController.retrieveLazy(URI.create(grant.getGrantFor()), sessionUser);
          if (!col.isSubCollection()) {
            roles.add(new ShareListItem(user, col.getId().toString(), col.getTitle(), sessionUser, locale, false));
          }
        }
   ...
   * @throws Exception
   */
  public static List<ShareListItem> getAllRoles(UserGroup group, User sessionUser, Locale locale) throws ImejiException {
    final CollectionService collectionController = new CollectionService();
    final List<ShareListItem> roles = new ArrayList<ShareListItem>();
    for (final String grantString : group.getGrants()) {
      try {
        final Grant grant = new Grant(grantString);
        final CollectionImeji col = collectionController.retrieveLazy(URI.create(grant.getGrantFor()), sessionUser);
        roles.add(new ShareListItem(group, col.getId().toString(), col.getTitle(), sessionUser, locale));
      } catch (final Exception e) {
        LOGGER.error("Error reading grants of user ", e);
      }
    }
