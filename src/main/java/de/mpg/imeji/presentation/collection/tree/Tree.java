...
public class Tree implements Serializable {
  ...
  public Tree(List<CollectionImeji> collections) {
    map = collections.stream().collect(Collectors.toMap(c -> c.getId().toString(), Function.identity()));
  ...
  private List<Node> initTopNodes(List<CollectionImeji> collections) {
    return collections.stream().filter(c -> !c.isSubCollection()).map(c -> new Node(c, map)).collect(Collectors.toList());
  }

  public void showChildren(Node node) {
    int index = nodes.indexOf(node);
    nodes.addAll(index, new HierarchyService().getFullHierarchy().getTree().get(node.getCollection().getId().toString()).stream()
        .filter(id -> map.get(id) != null).map(id -> new Node(map.get(id), map)).collect(Collectors.toList()));
  }
