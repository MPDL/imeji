...
public class ShareListItem implements Serializable {
  ...
  private void init(String role, String uri, Locale locale) {
    this.role = role;
    this.locale = locale;
    this.rolesMenu = Arrays.asList(new SelectItem(ShareRoles.READ.name(), Imeji.RESOURCE_BUNDLE.getLabel("collection_share_read", locale)),
  ...
  public void updateInvitation() throws ImejiException {
    if (invitation != null) {
      ...
      if (role == null) {
        ...
        final Invitation newInvitation = new Invitation(invitation.getInviteeEmail(), invitation.getObjectUri(), role);
