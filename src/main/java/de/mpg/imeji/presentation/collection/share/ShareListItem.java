...
public class ShareListItem implements Serializable {
	 ...
	 * @param roles
	 */
	public ShareListItem(User user, String uri, String title, User currentUser, Locale locale, boolean creator) {
		this.user = user;
		this.title = title;
		this.currentUser = currentUser;
	...
	private void init(String role, String uri, Locale locale) {
		...
		this.rolesMenu = Arrays.asList(
				new SelectItem(ShareRoles.READ.name(), Imeji.RESOURCE_BUNDLE.getLabel("collection_share_read", locale)),
	...
	/**
	 * Update {@link Grants} the {@link ShareListItem} according to the new roles.
	 * Return true if the user grant have been modified
	 *
	 * @return
	 */
	...
	public void updateInvitation() throws ImejiException {
		if (invitation != null) {
			...
			if (role == null) {
				...
				final Invitation newInvitation = new Invitation(invitation.getInviteeEmail(), invitation.getObjectUri(),
						role);
	...
	/**
	 * Return all users in this items. This might be many user if the item contains
	 * a group
	 *
	 * @return
	 */
