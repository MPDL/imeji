...
public class Node {
  ...
  public void initChildren() {
    List<String> childrenIds = new HierarchyService().getFullHierarchy().getTree().get(collection.getId().toString());
    if (childrenIds != null) {
      children = childrenIds.stream().filter(id -> collectionsMap.get(id) != null)
          .map(id -> new Node(this, collectionsMap.get(id), collectionsMap)).collect(Collectors.toList());
    }
  }
