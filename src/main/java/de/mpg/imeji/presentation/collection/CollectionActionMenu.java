...
public class CollectionActionMenu implements Serializable {
	...
	public void searchItemsWihoutLicense() {
		...
		itemsWithoutLicense = controller.search(collection.getId(),
				SearchQuery.toSearchQuery(
						new SearchPair(SearchFields.license, SearchOperators.EQUALS, ImejiLicenses.NO_LICENSE, false)),
				null, user, 0, 0).getNumberOfRecords();
	}
	...
	private void addProvidedDOI(String newDOI) {
		...
		try {
			...
			if (imejiException instanceof UnprocessableError) {
				...
				LOGGER.error(Imeji.RESOURCE_BUNDLE.getMessage("error_doi_creation", locale) + " "
						+ imejiException.getMessage());
			}
	...
	/**
	 * Redirect to the parent collection. If no parent collection, go back to the
	 * collections page
	 * 
	 * @param collection
	 * @throws IOException
