...
public class ShareInput implements Serializable {
	...
	public void sendInvitations() {
		...
		for (final String invitee : unknownEmails) {
			try {
				...
				emailService.sendMail(invitee, null,
						EmailMessages.getInvitationEmailSubject(this.user, this.locale, this.instanceName),
	...
	public List<ShareListItem> asShareListItem() {
		...
		for (final String email : validEmails) {
			final ShareListItem item = new ShareListItem(retrieveUser(email), objectUri, null, user, locale, false);
	...
	/**
	 * Parse the Input to a list of Emails. Add Unknown emails to externaluser list
	 * and invalid Emails to invalideEntries
	 *
	 * @return
	 */
