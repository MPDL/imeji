...
/**
 * With this listener, it can be avoided that Faces Messages are lost when doing
 * an redirect instead of applying a navigation rule
 *
 * @author Markus Haarlaender (initial creation)
 * @author $Author: mfranke $ (last modification)
 * @version $Revision: 1891 $ $LastChangedDate: 2008-12-23 11:13:59 +0100 (Di,
 *          23 Dez 2008) $
 */
public class FacesMessagesPhaseListener implements PhaseListener {
	...
	// private Map<String, Collection<FacesMessage>> messageCache =
	// Collections.synchronizedMap(new
	// HashMap<String,
	// Collection<FacesMessage>>());
	/**
	 * Caches Faces Messages after the Invoke Application phase and clears the cache
	...
	private Map<String, Collection<FacesMessage>> getMessageCache(FacesContext context) {
		if (context.getExternalContext().getSessionMap().get(sessionToken) != null) {
			...
			final Map<String, Collection<FacesMessage>> messageCache = Collections
