...
/**
 * Class to load a {@link Resource} into a java object. This java object must
 * have been annotated with j2j annotations
 *
...
public class Jena2Java {
	...
	private static final Logger LOGGER = LogManager.getLogger(Jena2Java.class);
	...
	/**
	 * Load a {@link Resource} into a Java Object. The Java Object must define a
	 * {@link j2jResource} and have and id non null (defined by {@link j2jId})
	 *
	 * @param item
	 *            : The uri of the resource
	 * @param o
	 *            : The java Object in which the resource should be written
	 * @return
	 */
	...
	/**
	 * Load all {@link Field} defined in the Object. The Object must be a
	 * {@link Resource} (defined in Java by {@link j2jResource})
	 *
	 * @param javaObject
	 * @return
	 */
	...
	/**
	 * Load the Object of a Triple. The {@link Field} defines the relation between
	 * the subject and the object (i.e. the field contains predicate information)
	 *
	 * @param subject
	 * @param f
	 * @param object
	...
	private Object loadObject(Resource subject, Field f, Object object, int position, Statement st) {
		if (J2JHelper.isResource(object)) {
		...
		} else if (J2JHelper.isURIResource(object, f) || (J2JHelper.isLazyURIResource(object, f) && !lazy)) {
			...
			// field of a list which has not been handle so for (i.e not an URI or not a
			// localized String)
			object = loadObjectLiteral(f, subject, object, position, st);
		}
		return object;
	...
	private Object setField(Object subject, Field f, Object object) {
		try {
			...
			throw new RuntimeException("Error writing " + object + " to " + subject + " for field " + f, e);
		}
	}

	/**
	 * Load the object the the statement (subject - predicate - object). The object
	 * must be a {@link Resource}. The position defines which object should be
	 * loaded in case of many statement with the same subject and predicate.
	 *
	 * @param subject
	 * @param object
	 * @param position
	...
	/**
	 * Load the object the the statement (subject - predicate - object). The object
	 * must be a {@link Literal}. The position defines which object should be loaded
	 * in case of many statement with the same subject and predicate.
	 *
	 * @param f
	 * @param subject
	 * @param object
	 ...
	 * @return
	 */
	private Object loadObjectLiteral(Field f, Resource subject, Object object, int position, Statement statement) {
	...
	/**
	 * Load the object the the statement (subject - predicate - object). The object
	 * must be a {@link j2jResource} and an {@link URI}. The position defines which
	 * object should be loaded in case of many statement with the same subject and
	 * predicate.
	 *
	 * @param f
	 * @param subject
	...
	/**
	 * Load the object the the statement (subject - predicate - object). The object
	 * must be a {@link LocalizedString}. The position defines which object should
	 * be loaded in case of many statement with the same subject and predicate.
	 *
	 * @param f
	 * @param subject
	 * @param object
	 ...
	 * @return
	 */
	private Object readLocalizedString(Field f, Resource subject, Object object, int position, Statement statement) {
		final Statement st = statement == null ? getStatement(subject, RDFS.label.getURI(), position) : statement;
		final LocalizedString ls = new LocalizedString(st.getObject().asLiteral().getValue().toString(),
				st.getLanguage());
		return ls;
	}
	...
	/**
	 * Load the object the the statement (subject - predicate - object). The object
	 * must be a {@link List} The position defines which object should be loaded in
	...
	private Object readList(Field f, Resource subject) {
		...
		int count = 0;
		for (final StmtIterator iterator = subject.listProperties(model.createProperty(predicate)); iterator
				.hasNext();) {
	...
	/**
	 * True if the {@link Resource} is a {@link RDF}.type. This type must define the
	 * Java class of the resource,
	 *
	 * @param r
	 * @return
	 */
	...
	/**
	 * Create an {@link Object} of the type defined in the {@link RDF}.type value
	 * defined for this {@link Resource}
	 *
	 * @param r
	 * @return
	 */
	...
	/**
	 * Load the full statement defined by one subject and one predicate for a
	 * specific position (if many statements with same subject and predicate)
	 *
	 * @param subject
	 * @param predicateUri
	 * @param position
	...
	private Statement getStatement(Resource subject, String predicateUri, int position) {
		int count = 0;
		for (final StmtIterator iterator = subject.listProperties(model.createProperty(predicateUri)); iterator
				.hasNext();) {
