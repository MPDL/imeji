...
/**
 * Write Java objects annotated with j2jannotations as Jena {@link Resource} of
 * {@link Literal}
 *
...
public class Java2Jena {
	 ...
	 * @param model
	 * @param lazy
	 *            - defined whether the write operations will be lazy or not (i.e.
	 *            skip or not the {@link List})
	 */
	...
	public void write(Object o) {
		...
		final Resource type = model.createResource(J2JHelper.getResourceNamespace(o).toString(), RDFS.Class);
		final Resource r = model.createResource(J2JHelper.getId(o).toString(), type);
		addProperties2Resource(r, o);
	}

	/**
	 * Update a {@link RDFResource}. Remove all the properties of the
	 * {@link RDFResource} and write the new again
	 *
	 * @param r
	 */
	...
	/**
	 * True if resource exists, throw {@link NullPointerException} if
	 * {@link RDFResource} has a null ID
	 *
	 * @param rdfR
	 * @return
	 */
	...
	private void addProperties2Resource(Resource s, Object o) {
		for (final Field f : J2JHelper.getAllObjectFields(o.getClass())) {
			try {
				...
				throw new RuntimeException("Error adding property for field " + f + " to object " + o.getClass(), e);
			}
		}
	}
		...
		if (!(lazy && J2JHelper.isLazyList(f))) {
			for (int i = 0; i < list.size(); i++) {
				...
				if (J2JHelper.isResource(listElement) && J2JHelper.getId(listElement) == null) {
					...
					listElement = J2JHelper.setId(listElement, URI.create(subjectId + "/" + objectName + "@pos" + i));
				}
	...
	/**
	 * Update the position according to the current place of this list element in
	 * the list
	 *
	 * @param id
	 * @return
	...
	/**
	 * Add a {@link Property} to a {@link Resource}. The {@link Property} can be
	 * either a new {@link Resource} or a {@link Literal}
	 *
	 * @param s
	 * @param obj
	 */
	private void addProperty(Resource s, Object obj, Field f) {
		try {
			if (obj == null) {
				...
				addLiteral(s, obj, f);
			} else if ((J2JHelper.isURIResource(obj, f) || J2JHelper.isLazyURIResource(obj, f)) && obj != null) {
	...
	/**
	 * Get all Embedded {@link Resource} of an {@link Object} stored in {@link Jena}
	 * as a {@link Resource}
	 *
	 * @param s
	 *            - the {@link Resource}
				...
				try {
					...
					if (J2JHelper.isResource(r2) && exists(r2)) {
						...
						if (isListOfResources) {
							...
							for (final StmtIterator iterator = parent
									.listProperties(model.createProperty(predicate)); iterator.hasNext();) {
