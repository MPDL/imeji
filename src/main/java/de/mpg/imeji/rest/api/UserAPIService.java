...
public class UserAPIService implements APIService<UserTO> {
  ...
  public User read(URI uri) throws ImejiException {
    return adminUser.getId().equals(uri) ? adminUser : new UserService().retrieve(uri, Imeji.adminUser);
  }

  public User read(String email) throws ImejiException {
    return adminUser.getEmail().equals(email) ? adminUser : new UserService().retrieve(email, Imeji.adminUser);
  }
  ...
  @Override
  public SearchResultTO<UserTO> search(String q, int offset, int size, User u) throws ImejiException {
    return null;
  }
