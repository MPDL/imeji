...
public class ItemAPIService implements APIService<DefaultItemTO> {
  ...
  public DefaultItemTO create(DefaultItemTO to, User u) throws ImejiException {
    if (to instanceof DefaultItemWithFileTO) {
      ...
      item = controller.create(item, collection, ((DefaultItemWithFileTO) to).getFile(), filename, u,
          ((DefaultItemWithFileTO) to).getFetchUrl(), ((DefaultItemWithFileTO) to).getReferenceUrl());
  ...
  public DefaultItemTO update(DefaultItemTO to, User u) throws ImejiException {
    ...
    if (to instanceof DefaultItemWithFileTO) {
      ...
      if (tof.getFile() != null) {
        ...
        item = controller.updateWithExternalFile(item, collection, getExternalFileUrl(tof), to.getFilename(),
  ...
  @Override
  public SearchResultTO<DefaultItemTO> search(String q, int offset, int size, User u) throws ImejiException {

    final List<DefaultItemTO> tos = new ArrayList<>();
    final SearchResult result =
        SearchFactory.create(SEARCH_IMPLEMENTATIONS.ELASTIC).search(SearchQueryParser.parseStringQuery(q), null, u, null, offset, size);
  ...
  private CollectionImeji getCollection(String collectionId, User u) throws ImejiException {
    if (!StringHelper.isNullOrEmptyTrim(collectionId)) {
      return new CollectionService().retrieveLazy(ObjectHelper.getURI(CollectionImeji.class, collectionId), u);
    }
  ...
  public String getFilename(DefaultItemWithFileTO to) {
    return firstNonNullOrEmtpy(to.getFilename(), (to.getFile() != null) ? FilenameUtils.getName(to.getFile().getName()) : "",
