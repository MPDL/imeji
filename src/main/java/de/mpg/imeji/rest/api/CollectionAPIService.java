...
public class CollectionAPIService implements APIService<CollectionTO> {
	...
	/**
	 * Read all the items of a collection according to search query. Response is
	 * done with the default format
	 *
	 * @param id
	 * @param u
	 * @param q
	...
	public SearchResultTO<DefaultItemTO> readItems(String id, User u, String q, int offset, int size)
		...
		for (final Item vo : itemService.retrieveBatch(result.getResults(), Search.GET_ALL_RESULTS,
				Search.SEARCH_FROM_START_INDEX, u)) {
			final DefaultItemTO to = new DefaultItemTO();
			TransferVOtoTO.transferDefaultItem(vo, to);
			tos.add(to);
		}
		return new SearchResultTO.Builder<DefaultItemTO>().numberOfRecords(result.getResults().size()).offset(offset)
	 ...
	 * @throws ImejiException
	 */
	public SearchResultTO<CollectionElementTO> readElements(String id, User u, String q, int offset, int size,
			SortCriterion sort) throws ImejiException {
		...
		final List<CollectionElementTO> tos = elements.stream().map(e -> TransferVOtoTO.toCollectionelementTO(e))
				.collect(Collectors.toList());
		return new SearchResultTO.Builder<CollectionElementTO>().numberOfRecords(result.getResults().size())
				.offset(offset).results(tos).query(q).size(size).totalNumberOfRecords(result.getNumberOfRecords())
				.build();
	}
	...
	public SearchResultTO<CollectionTO> search(String q, int offset, int size, User u) throws ImejiException {
		...
		return new SearchResultTO.Builder<CollectionTO>().numberOfRecords(result.getResults().size()).offset(offset)
				.results(tos).query(q).size(size).totalNumberOfRecords(result.getNumberOfRecords()).build();
	}
}
