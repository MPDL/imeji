...
public class CollectionAPIService implements APIService<CollectionTO> {
   ...
   * @throws IOException
   */
  public SearchResultTO<DefaultItemTO> readItems(String id, User u, String q, int offset, int size) throws ImejiException {
    final List<DefaultItemTO> tos = new ArrayList<>();
    final ItemService itemService = new ItemService();
    final SearchResult result = SearchFactory.create(SEARCH_IMPLEMENTATIONS.ELASTIC).search(SearchQueryParser.parseStringQuery(q), null, u,
    ...
    return new SearchResultTO.Builder<DefaultItemTO>().numberOfRecords(result.getResults().size()).offset(offset).results(tos).query(q)
        .size(size).totalNumberOfRecords(result.getNumberOfRecords()).build();
  }
   ...
   * @throws ImejiException
   */
  public SearchResultTO<CollectionElementTO> readElements(String id, User u, String q, int offset, int size, SortCriterion sort)
      throws ImejiException {
    final SearchAndRetrieveFacade facade = new SearchAndRetrieveFacade();
    final SearchResult result =
        facade.search(SearchQueryParser.parseStringQuery(q), ImejiFactory.newCollection().setId(id).build(), u, sort, size, offset);
    final List<CollectionElement> elements = facade.retrieveItemsAndCollections(result.getResults(), u);
    final List<CollectionElementTO> tos = elements.stream().map(e -> TransferVOtoTO.toCollectionelementTO(e)).collect(Collectors.toList());
    return new SearchResultTO.Builder<CollectionElementTO>().numberOfRecords(result.getResults().size()).offset(offset).results(tos)
        .query(q).size(size).totalNumberOfRecords(result.getNumberOfRecords()).build();
  }
  ...
  public CollectionTO release(String id, User u) throws ImejiException {
    ...
    final CollectionImeji vo = controller.retrieve(ObjectHelper.getURI(CollectionImeji.class, id), u);
  ...
  @Override
  public SearchResultTO<CollectionTO> search(String q, int offset, int size, User u) throws ImejiException {
    final CollectionService cc = new CollectionService();
    final List<CollectionTO> tos = new ArrayList<>();
    final SearchResult result = SearchFactory.create(SearchObjectTypes.COLLECTION, SEARCH_IMPLEMENTATIONS.ELASTIC)
    ...
    return new SearchResultTO.Builder<CollectionTO>().numberOfRecords(result.getResults().size()).offset(offset).results(tos).query(q)
        .size(size).totalNumberOfRecords(result.getNumberOfRecords()).build();
  }
}
