...
public class TransferTOtoVO implements Serializable {
	...
	/**
	 * Transfer the list of ContainerAdditionalInformationTO to List of
	...
	public static void transferItemMetadata(DefaultItemTO to, Item vo, User u, TRANSFER_MODE mode)
		...
		for (final MetadataTO mdTO : to.getMetadata()) {
			final Metadata mdVO = new MetadataFactory().setStatementId(mdTO.getIndex()).setText(mdTO.getText())
					.setNumber(mdTO.getNumber()).setUrl(mdTO.getUrl())
					.setPerson(transferPerson(mdTO.getPerson(), new Person(), mode)).setLatitude(mdTO.getLatitude())
					.setLongitude(mdTO.getLongitude()).setDate(mdTO.getDate()).setName(mdTO.getName())
					.setTitle(mdTO.getTitle()).build();
	...
	public static Person transferPerson(PersonTO pto, Person p, TRANSFER_MODE mode) {
		if (pto != null) {
			if (mode == TRANSFER_MODE.CREATE) {
				final String id = pto.getIdentifiers() == null || pto.getIdentifiers().isEmpty()
						? null
	...
	public static void transferCollectionContributors(List<PersonTO> persons, CollectionImeji vo, User u,
			TRANSFER_MODE mode) {
