...
public class RestProcessUtils {
	...
	/**
	 * This method builds exception response. Based on the error Code, local title
	 * and message (which can be localized through the Language Bundles) are built
	 *
	 * @param errorCode
	 * @param e
	 * @return
	...
	/**
	 * This method builds the response for successfully created, updated, deleted,
	 * released etc. object. It is a convenience method to save 3 lines of code
	 * every time the HTTP Response needs to be built after success
	 *
	 * @param statusCode
	...
	/**
	 * This method checks the exception type and returns appropriate JSON Response
	 * with properly set-up HTTP Code.
	 *
	 *
	 * @param eX
	 * @param message
	...
	public static JSONResponse localExceptionHandler(Exception eX, String message) {
		...
		if (eX instanceof AuthenticationError) {
			...
			resp = RestProcessUtils.buildJSONAndExceptionResponse(Status.INTERNAL_SERVER_ERROR.getStatusCode(),
					localMessage);
