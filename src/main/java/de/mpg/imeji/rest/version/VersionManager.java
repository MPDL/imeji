...
public class VersionManager {
  ...
  public void checkVersion(String path) throws UnknowAPIVersionException, DeprecatedAPIVersionException {
    ...
    if (!isAnExistingVersion()) {
      throw new UnknowAPIVersionException("API version v" + requestedVersion + " is not a valid version. Please use the latest version v"
          + ImejiRestService.CURRENT_VERSION + ". For more information see " + linkToAPIDoc());
    } else if (isOldVersion()) {
      throw new DeprecatedAPIVersionException(
          "API version v" + requestedVersion + " is no longer supported. Please use the latest version v" + ImejiRestService.CURRENT_VERSION
